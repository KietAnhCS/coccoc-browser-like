...
|---|---|---|
| **Nhất quán truy hồi ↔ xếp hạng** | 10/10 | Nới lỏng dùng `df`, chấm điểm dùng `idf(df)` — cùng một đại lượng, hệ thống không tự mâu thuẫn |
| **Trung thực với người dùng** | 10/10 | `droppedTerms` trả ra ngoài thay vì âm thầm đổi câu hỏi — rất hiếm ở mức đồ án |
| Ranh giới kiến trúc | 10/10 | Quy tắc "có posting list ⇒ cây, siêu dữ liệu ⇒ filter" rõ ràng, kiểm chứng được |
| Chống trôi lệch bản sao | 10/10 | Lý do tách lớp được ghi lại đúng: bảo vệ **giá trị của bằng chứng**, không chỉ bảo vệ mã |
| Bất biến "chỉ rút lại thứ mình tự thêm" | 10/10 | Cưỡng chế bằng cấu trúc (`ParsedQuery` mới chỉ thay `mustTerms`), không chỉ bằng lời |
| Thoát sớm | 9/10 | `isUnmatchable` + `break` khi rỗng + gộp `df = 0` — ba phép cắt tỉa đúng chỗ |
| Chất lượng kiểm thử | 9/10 | 12 ca phủ gần hết nhánh, có ca "test giả định"; thiếu tổ hợp `site:` + nới lỏng |
| Khả năng mở rộng | 8/10 | Thêm filter = một dòng; nhưng thêm `QueryNode` mới thì phải sửa `QueryParser.buildAst` |
| Hiệu năng khi nới lỏng | 6/10 | Mỗi `attempt` đánh giá lại **toàn bộ** cây, kể cả nhánh không đổi |
| **Cưỡng chế ràng buộc thứ tự filter** | **4/10** | `MaxCandidatesFilter` **phải** đứng cuối, nhưng `List.of(...)` không có gì ngăn đặt sai |
| Kiểm tra đối số | 4/10 | `resolve` không kiểm `null`; lỗi lộ ra sâu trong `evaluate` với stack trace khó lần |
| Khả năng quan sát | 3/10 | Javadoc nói "có thể bọc timer quanh `apply` để in bảng" — nhưng **chưa làm**; không có log nào cho biết nới lỏng đã chạy |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Nhớ đệm posting list trong một lần `resolve`.** Nới lỏng 9 term có thể đánh
   giá lại `TermNode` giống hệt tới 9 lần. Một `Map<String, List<Integer>>` cục
   bộ truyền qua `evaluate` cắt chi phí tệ nhất từ $O(k \cdot L)$ về $O(L + k
   \cdot n)$:
   ```java
   // truyen mot ngu canh danh gia thay vi truyen tran SearchIndex
   public record EvalContext(SearchIndex index, Map<String, List<Integer>> postingCache) {
       public List<Integer> postings(String term) {
           return postingCache.computeIfAbsent(term, index::getPostingDocIds);
       }
   }
   ```
   Đồng thời nó cũng loại luôn việc gọi `getDocumentFrequency` lặp lại trong
   `Comparator` ở bước 2 (cạm bẫy ⑤ mục 7.4).

2. **Cưỡng chế ràng buộc "`MaxCandidatesFilter` đứng cuối" bằng mã, không bằng
   niềm tin.** Hiện tại đặt sai thứ tự sẽ mất phần lớn kết quả mà không có cảnh
   báo nào (xem [`filter/MaxCandidatesFilter.md`](./filter/MaxCandidatesFilter.md)
   mục 5.3). Một khối `static` kiểm tra lúc nạp lớp là đủ, và nó thất bại **lúc
   khởi động** chứ không phải lúc phục vụ người dùng:
   ```java
   static {
       if (!(FILTERS.get(FILTERS.size() - 1) instanceof MaxCandidatesFilter)) {
           throw new IllegalStateException(
                   "MaxCandidatesFilter phai la tang CUOI cua duong ong: cat truoc "
                 + "khi loc se vut bo ung vien ma cac tang sau dang le giu lai.");
       }
   }
   ```

3. **Ghi nhật ký việc nới lỏng — nó là tín hiệu chất lượng corpus, không phải chi
   tiết cài đặt.** Tỉ lệ truy vấn phải nới lỏng, và term nào hay bị bỏ nhất, là
   hai con số nói thẳng rằng corpus đang thiếu gì. Hiện không có gì ghi lại:
   ```java
   if (attempt != null) {
       log.info("relax: query={} dropped={} candidates={}",
                parsed.mustTerms(), dropped, attempt.candidateDocIds().size());
       return attempt;
   }
   ```
   Đưa con số này vào [`../analytics/UsageAnalyticsService.md`](../analytics/UsageAnalyticsService.md)
   thì bảng điều khiển quản trị có thêm một chỉ số thật sự hành động được: *"18%
   truy vấn phải nới lỏng ⇒ cần crawl thêm chủ đề X"*.
