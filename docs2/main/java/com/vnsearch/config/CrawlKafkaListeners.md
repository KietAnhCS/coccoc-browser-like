...
|---|---|---|
| **Kể lại sự cố chia đôi luồng kèm cách kiểm chứng** | 10/10 | Không chỉ nêu nguyên nhân mà còn ghi thẳng **lệnh chẩn đoán** và **con số** dùng để lộ diện |
| **Quy tắc trong khung được phát biểu tổng quát** | 10/10 | *"Listener nào ghi vào trạng thái trong bộ nhớ của tiến trình API thì chỉ được chạy ở tiến trình API"* |
| **Ngoại lệ để bay ra, kèm lý do** | 10/10 | Chỉ rõ `try/catch` ở đây biến *hoãn xử lý* thành **mất vĩnh viễn** — và nó trông y hệt mã cẩn thận |
| **Chiến lược "lớp mỏng để ba service không biết Kafka"** | 10/10 | Đường biên vẽ rõ, và kiểm chứng được bằng cách đọc sáu thân phương thức |
| Chuyển tiếp qua `CrawlJobManager` | 10/10 | Xung đột vòng đời được nêu chính xác, và biện minh bằng một rò rỉ **đã xảy ra** |
| Nêu hậu quả của `groupId` chung bằng con số | 9/10 | *"Hai phần ba công việc âm thầm không được làm"* — đúng mức cụ thể cần thiết |
| Bình luận ngăn phép sửa hợp lý ở `onImage` | 9/10 | Chặn đúng suy nghĩ tự nhiên nhất của người đọc sau |
| **Kiểm thử phần rủi ro nhất** | **0/10** | Rủi ro nằm trong **thuộc tính chú giải**, đọc được bằng phản chiếu, và **không cái nào có test** |
| **`autoStartup` chưa áp cho nhóm analytics** | **4/10** | `onPageForAnalytics`/`onImage` chạy ở cả hai tiến trình ⇒ group `analytics` đang bị chia đôi |
| `autoStartup` mặc định `true` | 5/10 | Đúng cho chạy thử cục bộ, nhưng quên đặt ở worker sẽ **tái tạo nguyên sự cố**, và không gì kiểm |
| Không nêu chi phí bộ nhớ của phát tán | 6/10 | Cùng một trang nằm trong heap **ba lần** cùng lúc; đó là cái giá của một-tới-nhiều |
| `onImage` chia group với `onPageForAnalytics` | 6/10 | Đúng (khác topic nên không chia luồng) nhưng trông như thiếu nhất quán và không được giải thích |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Test các thuộc tính chú giải bằng phản chiếu — không cần broker.** Toàn bộ
   rủi ro của lớp này nằm ở phần **không phải mã**, đúng chỗ mà lập luận *"lớp
   mỏng nên không cần test"* không bao phủ. Hai sự cố đã xảy ra thật đều kiểm được
   trong vài mili-giây:
   ```java
   class CrawlKafkaListenersTest {

       private static final Method[] PHUONG_THUC = CrawlKafkaListeners.class.getMethods();

       @Test
       void baListenerTrenTopicPagesPhaiCoBaGroupIdPHAN_BIET() {
           List<String> group = Arrays.stream(PHUONG_THUC)
                   .map(m -> m.getAnnotation(KafkaListener.class))
                   .filter(Objects::nonNull)
                   .filter(a -> a.topics()[0].contains("topic.pages"))
                   .map(a -> a.groupId())
                   .toList();

           assertEquals(3, group.size());
           assertEquals(3, Set.copyOf(group).size(),
                   "Dung CHUNG groupId tren cung mot topic thi moi trang chi den MOT"
                           + " trong ba service, chon ngau nhien — hai phan ba cong viec"
                           + " am tham khong duoc lam, va khong loi nao duoc nem.");
       }

       @Test
       void listenerGhiVaoTrangThaiTrongBoNhoPhaiCoAutoStartupTheoVaiTro() {
           Set<String> canChanTheoVaiTro = Set.of(
                   "onDiscoveredUrl", "onOutlinks", "onPageForAnalytics", "onImage");

           for (Method m : PHUONG_THUC) {
               KafkaListener a = m.getAnnotation(KafkaListener.class);
               if (a != null && canChanTheoVaiTro.contains(m.getName())) {
                   assertTrue(a.autoStartup().contains("app.crawler.role.is-api"),
                           m.getName() + " ghi vao trang thai trong bo nho cua tien trinh API."
                                   + " Thieu autoStartup thi ca backend lan worker cung chay no,"
                                   + " Kafka CHIA DOI luong, va mot nua bi bo lang le.");
               }
           }
       }

       @Test
       void khongThanPhuongThucNaoDuocChuaTryCatch() throws Exception {
           String nguon = Files.readString(Path.of(
                   "src/main/java/com/vnsearch/config/CrawlKafkaListeners.java"));
           assertFalse(nguon.contains("try {"),
                   "Bat ngoai le o day vo hieu hoa thu lai + dead-letter, va bien"
                           + " 'hoan xu ly' thanh 'mat vinh vien' — vi offset VAN duoc chot.");
       }
   }
   ```

2. **Áp quy tắc trong khung cho cả nhóm analytics.** Quy tắc đã được phát biểu rất
   rõ ở dòng 100–103, nhưng hiện chỉ áp cho hai listener frontier.
   `onPageForAnalytics` và `onImage` ghi vào `CrawlAnalyticsService` — một trạng
   thái trong bộ nhớ tiến trình — nhưng không có `autoStartup`, nên group
   `vnsearch-analytics` đang bị chia giữa backend và worker:
   ```java
   @KafkaListener(
           topics = "${app.crawler.kafka.topic.pages}",
           groupId = "${app.crawler.kafka.group.analytics}",
           containerFactory = "crawlListenerContainerFactory",
           // Cung quy tac voi hai listener frontier: CrawlAnalyticsService ghi vao
           // MeterRegistry cua TUNG tien trinh. Chay o ca hai thi group bi chia doi,
           // va /api/admin/analytics cua backend chi thay MOT NUA so lieu.
           //
           // Prometheus thu thap ca hai tien trinh nen tong VAN dung — nhung bat ky
           // ai doc so lieu qua API cua mot tien trinh deu thay so sai.
           autoStartup = "${app.crawler.role.is-api:true}")
   public void onPageForAnalytics(PageEvent event) { analytics.onPage(event); }
   ```
   Nếu quyết định **không** đổi (vì muốn worker cũng đóng góp số liệu vào
   Prometheus), thì lý do đó phải được ghi ngay tại chỗ — hiện trạng "quy tắc có
   ngoại lệ không giải thích" là trạng thái tệ nhất, vì người đọc sau không biết
   đây là thiếu sót hay chủ ý.

3. **Đổi mặc định `autoStartup` thành an toàn, và bắt cấu hình nói ra vai trò.**
   Mặc định `true` đúng cho chạy thử một tiến trình, nhưng nó nghĩa là **quên** cấu
   hình ở worker sẽ tái tạo nguyên sự cố ở mục 3 — tức là hướng hỏng mặc định là
   hướng im lặng. Một phép kiểm lúc khởi động biến nó thành hướng ồn ào:
   ```java
   public CrawlKafkaListeners(..., @Value("${app.crawler.role:}") String vaiTro) {
       if (vaiTro.isBlank()) {
           throw new IllegalStateException(
                   "Thieu app.crawler.role khi app.crawler.bus=kafka. Phai khai ro"
                           + " 'api' hoac 'worker': hai tien trinh cung chay cac listener"
                           + " frontier se lam Kafka CHIA DOI luong URL, mot nua bi bo"
                           + " lang le, va phien crawl ket thuc voi 1 trang trong khi job"
                           + " van bao DONE. Kiem chung bang:"
                           + " kafka-consumer-groups --describe --group vnsearch-frontier-feeder");
       }
       ...
   }
   ```
   Thông báo giữ đúng chuẩn bốn phần của [`SecurityConfig`](./SecurityConfig.md)
   mục 5, và quan trọng hơn: nó mang theo **triệu chứng** và **lệnh chẩn đoán**,
   nên người gặp lỗi nhận ra ngay mình đang ở đúng ca này.
