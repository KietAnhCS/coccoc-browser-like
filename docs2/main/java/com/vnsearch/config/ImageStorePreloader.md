...
|---|---|---|
| **Nêu lỗ hổng bằng bảng hai tab** | 10/10 | Trình bày bất đối xứng dưới dạng thứ **người dùng nhìn thấy**, không dưới dạng thuật ngữ kỹ thuật |
| **"Đúng về hành vi nhưng sai về nguyên nhân"** | 10/10 | Chẩn đoán chính xác loại thông báo nguy hiểm nhất: nó chữa được triệu chứng nên không ai nghi ngờ |
| **Tách theo mức bắt buộc, không theo chức năng** | 10/10 | *"Một tệp ảnh hỏng có thể kéo sập cả phần tìm kiếm văn bản"* — lý do đúng để tách lớp |
| **Nạp gộp, không nạp đè** | 10/10 | Giao hoán + luỹ đẳng ⇒ một lỗi đua phụ thuộc thời điểm trở nên **không biểu diễn được** |
| **Dùng chung khoá cấu hình** | 10/10 | Triệt tiêu khả năng lệch nguồn dữ liệu về mặt **cấu trúc**, kèm hậu quả cụ thể nếu tách |
| Log trường hợp rỗng | 9/10 | Ba việc trong một dòng: đường dẫn, triệu chứng ở giao diện, và khẳng định đây không phải lỗi |
| **Kiểm thử** | **0/10** | Năm tính chất, tất cả kiểm được bằng `@TempDir` + JUnit thuần, và **không cái nào có test** |
| **Tệp hỏng và tệp thiếu cho cùng thông báo** | **4/10** | Một nguyên nhân bất thường và một nguyên nhân bình thường không phân biệt được từ log |
| **Chạy cả ở worker, chiếm ~120 MB vô ích** | **4/10** | Không nhất quán với `ImageStoreListener` ngay bên cạnh, lớp đã có điều kiện theo vai trò |
| Không có đường ghi khi tắt ứng dụng | 5/10 | Ảnh nhận sau khi phiên crawl kết thúc có thể không bao giờ xuống đĩa — cùng loại lỗ hổng, nhỏ hơn |
| Đường dẫn tương đối trong log | 6/10 | Phần lớn các lần gặp lỗi này là chạy sai thư mục, và đường dẫn tuyệt đối sẽ nói ngay điều đó |
| Log không nêu số ảnh đọc được | 6/10 | Chênh lệch giữa `added` và `images.size()` chính là bằng chứng cửa sổ đua ở mục 3 đã xảy ra |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Test tính luỹ đẳng và bảo vệ chống cửa sổ đua.** Hai bất biến quan trọng nhất
   của lớp — *nạp hai lần không sinh trùng* và *không xoá thứ đã có* — đều rẻ để
   kiểm, và bất biến thứ hai nếu hỏng sẽ gây mất dữ liệu **phụ thuộc thời điểm**,
   tức là loại lỗi tốn nhiều giờ nhất để lần ra:
   ```java
   class ImageStorePreloaderTest {

       @Test
       void napHaiLanKhongSinhBanGhiTrung(@TempDir Path thuMuc) throws Exception {
           Path corpus = thuMuc.resolve("crawled.json");
           ImageStorage.save(ImageStorage.pathFor(corpus.toString()),
                   List.of(anh("http://a/1.jpg"), anh("http://a/2.jpg")));

           ImageStore kho = new ImageStore();
           var preloader = new ImageStorePreloader(kho);
           datDuongDan(preloader, corpus.toString());

           preloader.preload();
           preloader.preload();

           assertEquals(2, kho.size(), "khu trung theo imageUrl phai giu kho o dung 2 anh");
       }

       @Test
       void khongXoaAnhMaPhienCrawlDaKipDoVao(@TempDir Path thuMuc) throws Exception {
           Path corpus = thuMuc.resolve("crawled.json");
           ImageStorage.save(ImageStorage.pathFor(corpus.toString()),
                   List.of(anh("http://cu/1.jpg")));

           ImageStore kho = new ImageStore();
           kho.add(anh("http://moi/2.jpg"));      // listener da ghi truoc

           var preloader = new ImageStorePreloader(kho);
           datDuongDan(preloader, corpus.toString());
           preloader.preload();

           assertEquals(2, kho.size(),
                   "preload phai GOP chu khong DE — de se xoa anh ma listener vua ghi,"
                           + " va loi do phu thuoc thoi diem nen KHONG tai hien duoc.");
       }

       @Test
       void tepKhongTonTaiThiKhongNem(@TempDir Path thuMuc) {
           var preloader = new ImageStorePreloader(new ImageStore());
           datDuongDan(preloader, thuMuc.resolve("khong-co.json").toString());
           assertDoesNotThrow(preloader::preload,
                   "nap anh la viec 'co thi tot' — hong no khong duoc chan khoi dong");
       }
   }
   ```

2. **Phân biệt "tệp hỏng" với "chưa có tệp".** Hai tình huống hiện cho ra cùng một
   dòng `log.info`, nhưng một cái là trạng thái bình thường của repo mới clone,
   còn cái kia là **dữ liệu bị hỏng** — thứ đáng cảnh báo và đáng điều tra. Việc
   nuốt lỗi vẫn đúng (mức "có thì tốt"), chỉ là nuốt phải để lại dấu vết khác
   nhau:
   ```java
   @PostConstruct
   public void preload() {
       String path = ImageStorage.pathFor(crawledDataPath);
       Path tuyetDoi = Path.of(path).toAbsolutePath();

       if (!Files.exists(tuyetDoi)) {
           log.info("Chua co kho anh tai {} — tab Hinh anh se trong cho toi phien crawl"
                   + " tiep theo. (Duong dan suy ra tu app.crawler.data-path, la duong dan"
                   + " TUONG DOI voi thu muc lam viec hien tai.)", tuyetDoi);
           return;
       }

       List<ImageFound> images;
       try {
           images = ImageStorage.load(path);          // ban NEM, khong phai loadQuietly
       } catch (IOException e) {
           // Van khong chan khoi dong — nhung day la BAT THUONG, khong phai
           // trang thai binh thuong nhu nhanh tren.
           log.warn("Tep kho anh {} TON TAI nhung khong doc duoc — tab Hinh anh se trong."
                   + " Day KHONG phai trang thai binh thuong: tep co the bi cat cut do"
                   + " tien trinh bi giet giua luc ghi. Xoa tep roi crawl lai de dung lai.",
                   tuyetDoi, e);
           return;
       }
       ...
       log.info("Da nap kho anh tu {}: doc {} anh, them moi {} (tren {} trang, {} ms)",
               tuyetDoi, images.size(), added, imageStore.pageCount(), ...);
   }
   ```
   Dòng log cuối cũng nêu **cả hai** con số: `images.size()` khác `added` chính là
   bằng chứng cửa sổ đua ở mục 3 đã xảy ra thật — một thông tin hiện đang bị mất.

3. **Chỉ nạp ở tiến trình thật sự phục vụ `/api/images`.**
   [`ImageStoreListener`](./ImageStoreListener.md) ngay bên cạnh đã có điều kiện
   theo vai trò với lý do rất rõ; lớp này thì không, nên crawler-worker vẫn đọc
   toàn bộ tệp và giữ ~120 MB siêu dữ liệu ảnh mà nó không bao giờ dùng tới:
   ```java
   @PostConstruct
   public void preload() {
       if (!laApi) {
           log.info("Bo qua nap kho anh (vai tro: worker). Tien trinh nay khong phuc vu"
                   + " GET /api/images, nen giu kho anh trong heap chi lang phi ~120 MB."
                   + " Cung ly do ma ImageStoreListener bi chan bang autoStartup.");
           return;
       }
       ...
   }
   ```
   Điều kiện nên đặt **trong** phương thức chứ không phải `@ConditionalOnProperty`
   trên lớp — giữ đúng nguyên tắc đã được nêu ở
   [`ImageStoreListener`](./ImageStoreListener.md) mục 2: đồ thị bean phải giống
   nhau ở cả hai vai trò, chỉ hành vi lúc chạy mới khác.
