...
|---|---|---|
| **Phân quyền theo vai trò, không theo cơ chế đăng nhập** | 10/10 | Thêm cách xác thực thứ ba không phải sửa một dòng nào trong bảng |
| **Mặc định `denyAll()`** | 10/10 | Chọn chiều sai **ồn ào** thay vì **âm thầm**, và bình luận kể lại lỗi thật đã xảy ra (`/api/images`) |
| **Xử lý `DispatcherType.ERROR`** | 10/10 | Bắt được một lỗi mà toàn bộ test tích hợp bỏ lọt, và giải thích rõ vòng lặp đăng nhập vô tận |
| Thông báo lỗi khi thiếu khoá | 10/10 | Bốn phần đầy đủ: cái gì, ở đâu, **vì sao quan trọng**, làm sao sửa |
| `POST /api/events` ràng buộc theo phương thức | 9/10 | `GET /api/events` sau này sẽ **không** thừa kế quyền công khai — chi tiết dễ sai nhất, làm đúng |
| Lý do tắt CSRF | 9/10 | Ba điều kiện đủ, cả ba đều được ghim tường minh; trừ điểm vì chúng nằm ở hai tệp |
| `FilterRegistrationBean` thay `@Component` | 9/10 | Tránh đăng ký hai lần — với rate limit, hai lần nghĩa là hạn mức còn một nửa mà không ai biết |
| **Kiểm thử các bất biến quan trọng nhất** | **3/10** | `DispatcherType.ERROR`, thứ tự filter, `requireAdminApiKey` — **không** cái nào có test |
| **`MIN_KEY_LENGTH` chỉ kiểm độ dài** | **4/10** | `"aaaaaaaaaaaaaaaa"` qua được; kiểm entropy tối thiểu rẻ hơn nhiều so với cảm giác an toàn sai |
| Phụ thuộc ngầm giữa hai tệp cấu hình | 5/10 | Sửa `allowCredentials(true)` ở `CorsConfig` mở lại CSRF mà tệp này không hề hay biết |
| `/actuator/prometheus` công khai | 5/10 | Javadoc thừa nhận cần chặn ở tầng mạng — một ràng buộc triển khai **không** đọc ra được từ mã |
| Bảng trong Javadoc phải bảo trì tay | 6/10 | Bảng ba cột rất dễ đọc nhưng sẽ lệch với mã ngay lần thêm endpoint đầu tiên |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Một test đầu-cuối thật cho `DispatcherType.ERROR`.** Đây là bất biến duy
   nhất trong tệp mà việc mất nó gây ra **vòng lặp đăng nhập vô tận** cho đúng
   nhóm người dùng hợp lệ, và cũng là bất biến duy nhất mà `MockMvc` **không thể**
   canh giữ. Phải dùng cổng thật:
   ```java
   @SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
   class ErrorDispatchGiuNguyenMaTrangThaiTest {
       ...
       @Test
       void nguoiDungThuongGoiEndpointAdminPhaiNhan403ChuKhong401() {
           users.createAccount("thuong", "mat-khau-du-dai", Role.USER);
           String token = sessions.create("thuong", Role.USER);

           HttpHeaders h = new HttpHeaders();
           h.setBearerAuth(token);
           ResponseEntity<String> r = rest.exchange("/api/admin/stats",
                   HttpMethod.GET, new HttpEntity<>(h), String.class);

           assertEquals(HttpStatus.FORBIDDEN, r.getStatusCode(),
                   "401 o day day nguoi dung vao vong lap dang nhap vo tan");
       }
   }
   ```
   Cùng lớp test đó nên kiểm luôn `anyRequest().denyAll()` (một đường dẫn bịa trả
   401) và thứ tự filter (request mang cả hai header cho ra principal là tên tài
   khoản) — cả ba đều cần cổng thật và không tốn thêm hạ tầng nào.

2. **Ghi rõ rằng `dispatcherTypeMatchers(ERROR)` không mở đường dẫn `/error`.**
   Bình luận hiện tại giải thích rất kỹ *vì sao cần* dòng đó, nhưng không trả lời
   câu hỏi mà mọi người đọc soát bảo mật sẽ hỏi đầu tiên: *"mở `/error` ra thì có
   nguy hiểm không?"*. Câu trả lời — luật khớp theo **kiểu gửi** chứ không theo
   **đường dẫn**, và request từ ngoài luôn mang `DispatcherType.REQUEST` nên vẫn
   rơi vào `denyAll()` — làm cho luật này hẹp hơn nhiều so với vẻ ngoài, nhưng nó
   không hiển nhiên:
   ```java
   // KHONG mo duong dan /error. Luat nay khop theo KIEU GUI:
   // DispatcherType.ERROR chi do chinh container sinh ra trong mot
   // lan forward noi bo. Mot `curl http://host/error` tu ngoai vao
   // mang DispatcherType.REQUEST, khong khop dong nay, va van roi
   // vao anyRequest().denyAll().
   .dispatcherTypeMatchers(DispatcherType.ERROR).permitAll()
   ```

3. **Kiểm entropy tối thiểu, không chỉ độ dài.** `MIN_KEY_LENGTH = 16` cho qua
   `"aaaaaaaaaaaaaaaa"` và `"1234567890123456"` — cả hai đều nằm trong mọi danh
   sách mật khẩu phổ biến. Toàn bộ giá trị của
   [`ApiKeyAuthFilter`](./ApiKeyAuthFilter.md) (so sánh hằng thời gian, `62^32`
   không gian tìm kiếm) sụp đổ nếu khoá thực tế chỉ có vài bit entropy:
   ```java
   private static final int MIN_KEY_LENGTH = 16;
   private static final int MIN_KY_TU_PHAN_BIET = 8;

   private String requireAdminApiKey() {
       ...
       long phanBiet = adminApiKey.chars().distinct().count();
       if (phanBiet < MIN_KY_TU_PHAN_BIET) {
           throw new IllegalStateException(
                   "app.security.admin-api-key chi co " + phanBiet + " ky tu phan biet"
                           + " (toi thieu " + MIN_KY_TU_PHAN_BIET + "). Mot khoa nhu"
                           + " \"aaaaaaaaaaaaaaaa\" dat du dai nhung khong co entropy,"
                           + " va toan bo phep so sanh hang thoi gian tro nen vo nghia."
                           + " Sinh khoa: openssl rand -hex 32");
       }
       return adminApiKey;
   }
   ```
   Đếm ký tự phân biệt là phép kiểm entropy **rẻ nhất có tác dụng** — nó không
   thay được một bộ kiểm tra thật, nhưng nó chặn đúng nhóm khoá mà người vận hành
   gõ vội khi muốn "chạy được đã". Và nó tự giải thích trong chính thông báo lỗi,
   giữ đúng chuẩn bốn phần đã đặt ra ở mục 5.
