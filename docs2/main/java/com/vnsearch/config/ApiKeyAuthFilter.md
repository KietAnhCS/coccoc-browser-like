...
|---|---|---|
| **So sánh hằng thời gian** | 10/10 | Dùng `MessageDigest.isEqual`, và Javadoc **định lượng** được lợi ích: `62^32` → `62*32` |
| **Tách "cấp vai trò" khỏi "từ chối"** | 10/10 | Filter không bao giờ tự trả 401; thêm endpoint công khai chỉ sửa bảng phân quyền |
| Không ghi giá trị khoá vào log | 10/10 | Loại lỗi âm thầm nhất trong nhóm này, và nó được tránh có ý thức |
| Chỉ log khi khoá SAI, không log khi vắng header | 9/10 | Tránh ngập log trên các endpoint công khai — cảnh báo thật giữ được sức nặng |
| Chuyển khoá sang `byte[]` trong hàm dựng | 9/10 | Không sinh rác mỗi request, và tránh chuỗi khoá lọt vào pool |
| Thứ tự filter có lý do ghi rõ | 8/10 | Ưu tiên phiên có danh tính vì **truy vết**, không phải vì bảo mật — lý do đúng, nhưng không có test |
| **Kiểm thử** | **0/10** | **Không một test nào** cho lớp đứng giữa Internet và endpoint điều khiển crawler |
| **Log thiếu IP nguồn** | **3/10** | Hai nghìn dòng cảnh báo không cho biết chặn ai; `RateLimitFilter` đã có sẵn logic lấy IP đúng |
| Javadoc đã lạc hậu | 4/10 | "Hệ thống này không có *người dùng* nào cả" — trong khi gói `auth` đã có đủ tài khoản, phiên, vai trò |
| Không thu hồi/xoay khoá được | 5/10 | Đổi khoá phải khởi động lại; không có khái niệm nhiều khoá hay khoá hết hạn |
| Không phân biệt thất bại theo nguồn | 5/10 | Không đếm số lần sai theo IP nên không có tín hiệu nào để nâng cấp thành chặn |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Ghi IP nguồn vào dòng cảnh báo, và đếm theo IP.** Dòng log hiện tại nói *có
   người đang dò khoá* nhưng không nói *ai*, nên nó không dẫn tới bất kỳ hành
   động nào. [`RateLimitFilter`](./RateLimitFilter.md) đã có hàm lấy IP xử lý
   đúng `X-Forwarded-For` (có kiểm soát bằng `trust-proxy`), nên thông tin đã tồn
   tại trong hệ thống — chỉ là chưa tới được đây:
   ```java
   private final ConcurrentHashMap<String, LongAdder> soLanSai = new ConcurrentHashMap<>();

   } else if (provided != null) {
       String ip = ClientIp.cua(request, trustProxy);
       long lan = soLanSai.computeIfAbsent(ip, k -> new LongAdder()).sum();
       soLanSai.get(ip).increment();
       if (lan >= 10) {
           log.error("Nghi van do khoa: {} da sai {} lan, gan nhat {} {}",
                   ip, lan + 1, request.getMethod(), request.getRequestURI());
       } else {
           log.warn("Tu choi {} {} tu {}: API key khong dung",
                   request.getMethod(), request.getRequestURI(), ip);
       }
   }
   ```
   Việc tách `ClientIp` thành lớp tiện ích dùng chung cũng gỡ đúng kiểu "Feature
   Envy" đã được chẩn đoán ở [`../service/LanguageDetector.md`](../service/LanguageDetector.md)
   mục 1: logic lấy IP hiện là `private` trong `RateLimitFilter` nên nơi thứ hai
   cần nó **không dùng được**.

2. **Cập nhật Javadoc cho khớp với hệ thống hiện tại.** Câu *"Hệ thống này không
   có người dùng nào cả"* đã sai kể từ khi [`UserService`](../auth/UserService.md)
   và [`SessionStore`](../auth/SessionStore.md) ra đời. Kết luận (dùng API key)
   vẫn đúng, nhưng lý do phải đổi — và lý do mới **mạnh hơn** lý do cũ:
   ```
   Truoc: "he thong khong co nguoi dung, nen khong can tai khoan"
   Sau:   "he thong co HAI KIEU chu the voi HAI nhu cau trai nguoc:
             - nguoi that can doi mat khau, dang xuat, khoa tam,
               va danh tinh ghi duoc vao log  → TokenAuthFilter
             - cong cu can khong trang thai, khong het han,
               cau hinh mot lan qua bien moi truong → ApiKeyAuthFilter
           Ep cong cu dang nhap bang tai khoan buoc script phai
           luu MAT KHAU — te hon han luu mot khoa API."
   ```
   Javadoc sai gây hại hơn Javadoc thiếu: người đọc tin nó, rồi kết luận sai về
   phần còn lại của hệ thống.

3. **Viết test cho chính lớp này, kể cả tính chất "không rò rỉ vào log".** Lớp
   đứng giữa Internet và endpoint điều khiển crawler mà không có test nào:
   ```java
   class ApiKeyAuthFilterTest {

       private static final String KHOA = "khoa-quan-tri-32-ky-tu-day-du-oke";

       @AfterEach void donContext() { SecurityContextHolder.clearContext(); }

       @Test
       void khoaDungThiCapVaiTroAdmin() throws Exception {
           var req = new MockHttpServletRequest("GET", "/api/admin/stats");
           req.addHeader(ApiKeyAuthFilter.HEADER, KHOA);
           new ApiKeyAuthFilter(KHOA).doFilter(req, new MockHttpServletResponse(),
                   new MockFilterChain());
       ...
       @Test
       void khoaSaiThiKhongCapGiVaKhongGhiKhoaVaoLog() throws Exception {
           var log = batLogCua(ApiKeyAuthFilter.class);          // ListAppender
           var req = new MockHttpServletRequest("POST", "/api/admin/crawl");
           req.addHeader(ApiKeyAuthFilter.HEADER, "khoa-sai-hoan-toan-nhung-dai");
           new ApiKeyAuthFilter(KHOA).doFilter(req, new MockHttpServletResponse(),
                   new MockFilterChain());

           assertNull(SecurityContextHolder.getContext().getAuthentication());
           assertEquals(1, log.list.size());
           assertFalse(log.list.get(0).getFormattedMessage().contains("khoa-sai"),
                   "log KHONG duoc chua gia tri khoa duoc gui len");
       }

       @Test
       void thieuHeaderThiKhongGhiLogGiCa() throws Exception {
           var log = batLogCua(ApiKeyAuthFilter.class);
           new ApiKeyAuthFilter(KHOA).doFilter(
                   new MockHttpServletRequest("GET", "/api/search"),
                   new MockHttpServletResponse(), new MockFilterChain());
           assertTrue(log.list.isEmpty(), "endpoint cong khai khong duoc lam ngap log");
       }
   }
   ```
   Tính chất "hằng thời gian" thì **không** kiểm được bằng unit test — đo
   nano-giây trên JVM có JIT là vô vọng. Cách canh giữ khả thi duy nhất là một
   luật tĩnh (ArchUnit hoặc một `grep` trong CI) cấm `.equals(` xuất hiện trong
   lớp này, kèm bình luận giải thích vì sao.
