...
## 12. Chấm theo chuẩn doanh nghiệp

| Tiêu chí | Điểm | Nhận xét |
|---|---|---|
| **`@ConditionalOnProperty` cho cả lớp** | 10/10 | *"Một dự án mà chạy thử đòi hỏi dựng broker trước là một dự án không ai chạy thử"* |
| **Kể lại lỗi `KafkaAdmin` kèm triệu chứng bốn dòng** | 10/10 | Nêu rõ **ba trong bốn** nguồn thông tin đều nói "ổn", và vì sao test tích hợp bỏ lọt |
| **Nhận ra Testcontainers che mất lỗi thật** | 10/10 | *"Mọi thứ môi trường test làm hộ là một chỗ môi trường thật sẽ không làm"* — bài học tổng quát |
| **Bảng bốn tham số producer neo vào bài toán thật** | 10/10 | Không một dòng "thực hành tốt" nào; mỗi lý do gắn với một đặc điểm của crawler |
| Dead-letter thay vì "log rồi bỏ qua" | 10/10 | Ba thuộc tính tăng dần: đếm được, xem được, **phát lại được** — biến mất dữ liệu thành hoãn xử lý |
| `retention.ms` là **cửa sổ chạy lại** | 10/10 | Nêu đúng năng lực mà hàng đợi công việc không có, và gọi nó là "một nửa lý do chọn Kafka" |
| `setAddTypeInfo(false)` | 10/10 | Vế *"kể cả những thông điệp **đã nằm sẵn** trên topic"* là vế phân biệt hiểu sâu với hiểu nông |
| Ghi giới hạn Bloom kèm trạng thái quyết định | 10/10 | *"Công việc của bước sau, không phải một lỗ hổng bị bỏ quên"* — biến giới hạn thành quyết định |
| Lập luận `MAX_MESSAGE_BYTES` theo đuôi phân bố | 9/10 | Chỉ ra lỗi có **thiên kiến**: rớt đúng nhóm trang giàu nội dung nhất |
| **Kiểm thử cấu hình** | **2/10** | Sáu tính chất, năm trong sáu kiểm được **không cần broker**, và không cái nào có test |
| **Thang đo bus dùng sai kiểu** | **4/10** | Đại lượng tích luỹ nhưng đăng ký là `Gauge` ⇒ `rate()` tính sai sau mỗi lần khởi động lại |
| Lý do `concurrency=1` ghi chưa chính xác | 5/10 | Kết luận đúng nhưng "mất tính một phân hoạch một luồng" không phải cơ chế thật của Spring Kafka |
| `batch.size` có thể chưa khớp dữ liệu thật | 5/10 | 64 KB < kích thước một thông điệp (~150 KB) ⇒ `linger.ms=20` khó có tác dụng như mô tả |
| Ràng buộc `message.max.bytes` liên tệp | 5/10 | Bốn nơi phải khớp, một nơi ở `docker-compose.yml`, không gì kiểm |
| `AckMode.RECORD` không được giải thích | 6/10 | Lựa chọn hợp lý (ưu tiên ít xử lý lại) nhưng là dòng duy nhất không có lý do, trong một tệp giải thích mọi thứ |
| Không nêu hệ quả vận hành của luật lọc | 6/10 | Ở chế độ Kafka, luật nhập trên giao diện quản trị bị **bỏ qua** — không ghi ở đâu |
| Không ước lượng dung lượng đĩa | 6/10 | Gọi `pages` là "topic tốn đĩa nhất" nhưng không có con số, trong dự án đo bộ nhớ rất kỹ ở nơi khác |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Test cấu hình không cần broker, và sửa kiểu thang đo.** Lỗi `KafkaAdmin` ở
   mục 2 đã xảy ra **thật**, gây ra một hệ thống "xanh toàn tập nhưng không có gì
   đi qua", và nó kiểm được trong vài mili-giây bằng `ApplicationContextRunner` —
   không broker, không Testcontainers, không vùng mù:
   ```java
   class KafkaCrawlConfigTest {

       private final ApplicationContextRunner runner = new ApplicationContextRunner()
               .withUserConfiguration(KafkaCrawlConfig.class)
               .withPropertyValues(
                       "app.crawler.kafka.bootstrap-servers=kafka-that:9092",
                       "app.crawler.kafka.topic.pages=vnsearch.pages",
                       "app.crawler.kafka.topic.urls=vnsearch.urls.discovered",
                       "app.crawler.kafka.topic.outlinks=vnsearch.outlinks",
                       "app.crawler.kafka.topic.images=vnsearch.images");
       ...
       @Test
       void khongBatKafkaThiKhongCoBeanKafkaNao() {
           runner.run(ctx -> assertThat(ctx).doesNotHaveBean(KafkaAdmin.class));
       }

       @Test
       void kafkaAdminPhaiDungDiaChiCuaDuAnChuKhongPhaiLocalhostMacDinh() {
           runner.withPropertyValues("app.crawler.bus=kafka").run(ctx -> {
               KafkaAdmin admin = ctx.getBean(KafkaAdmin.class);
               assertEquals("kafka-that:9092",
                       admin.getConfigurationProperties()
                            .get(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG),
                       "Thieu bean nay thi KafkaAdmin mac dinh tro ve localhost:9092,"
                               + " KHONG topic nao duoc tao, va ung dung VAN khoi dong"
                               + " binh thuong — loi da xay ra that o lan chay Docker dau.");
           });
       }

       @Test
       void namTopicPhaiDuocKhaiVoiDungTen() {
           runner.withPropertyValues("app.crawler.bus=kafka").run(ctx -> {
               var ten = ctx.getBeansOfType(NewTopic.class).values().stream()
                       .map(NewTopic::name).collect(Collectors.toSet());
               assertEquals(Set.of("vnsearch.pages", "vnsearch.urls.discovered",
                       "vnsearch.outlinks", "vnsearch.images", "vnsearch.pages.DLT"), ten);
           });
       }

       @Test
       void objectMapperPhaiDocDuocInstant() throws Exception {
           runner.withPropertyValues("app.crawler.bus=kafka").run(ctx -> {
               ObjectMapper om = ctx.getBean("crawlEventObjectMapper", ObjectMapper.class);
               assertDoesNotThrow(() -> om.writeValueAsString(Map.of("t", Instant.now())),
                       "Thieu JavaTimeModule thi PageEvent nem ngay o thong diep DAU TIEN,"
                               + " va loi do chi lo ra luc chay that.");
           });
       }
   }
   ```
   Song song, `vnsearch.crawl.bus.publish.failures` phải đổi sang `FunctionCounter`
   — nó là đại lượng **tích luỹ**, và `rate()` trên một `Gauge` sẽ đọc cú tụt về 0
   sau mỗi lần khởi động lại thành **tốc độ âm**, làm cảnh báo
   `VnSearchCrawlBusFailing` im lặng đúng lúc cần nhất:
   ```java
   return registry -> FunctionCounter.builder("vnsearch.crawl.bus.publish.failures",
                   crawlEventBus, CrawlEventBus::getPublishFailureCount)
           .description("So lan khong gui duoc su kien len bus ke tu khi khoi dong")
           .register(registry);
   ```

2. **Sửa lý do của `concurrency=1` cho khớp cơ chế thật.** Kết luận đúng nhưng cách
   giải thích sai sẽ dẫn người đọc sau tới quyết định sai: một người biết Spring
   Kafka sẽ thấy rằng `ConcurrentKafkaListenerContainerFactory` **vẫn** giữ đúng
   "một phân hoạch một luồng", kết luận rằng Javadoc nhầm, và tăng `concurrency`
   lên — rồi gặp một lỗi đua trên bộ lọc Bloom mà không hiểu vì sao:
   ```java
   /**
    * <p><b>{@code concurrency}</b> de mac dinh 1 luong moi container. Ly do
    * KHONG phai vi Kafka: {@code ConcurrentKafkaListenerContainerFactory} van
    * giu dung "mot phan hoach mot luong". Ly do nam o phia ung dung: cac bean
    * {@link #workerUrlSeenFilter} va {@link #workerUrlFilter} la MOT ban dung
    * chung cho ca tien trinh, nen nhieu luong listener se cung ghi vao MOT bo
    * loc Bloom. Muon tang concurrency thi phai cap bo loc theo PHAN HOACH
    * truoc da.
    *
    * <p>Thong luong o day duoc dieu chinh bang SO BAN SAO tien trinh, va no do
    * Kubernetes quyet (xem crawler-worker.yaml).
    */
   ```

3. **Đo lại `batch.size` theo kích thước thông điệp thật, và ghim ràng buộc liên
   tệp.** `linger.ms=20` được biện minh bằng việc gom lô để nén tốt hơn, nhưng
   `batch.size=64 KB` nhỏ hơn **một** thông điệp HTML điển hình (~150 KB) — nghĩa
   là lô gần như luôn được gửi ngay khi đầy vì một thông điệp, và cả hai tham số
   đều không làm việc như mô tả:
   ```java
   // Mot PageEvent mang HTML tho, trung binh ~150 KB sau khi tuan tu hoa.
   // batch.size PHAI lon hon nhieu lan con so do thi linger.ms moi co tac dung
   // gom lo — voi 64 KB, moi thong diep tu no da lam day lo va duoc gui ngay,
   // nen viec "gom nhieu trang cung site de nen tot hon" khong xay ra.
   props.put(ProducerConfig.BATCH_SIZE_CONFIG, 1024 * 1024);   // ~6-7 trang mot lo
   ```
   Đồng thời, ràng buộc `MAX_MESSAGE_BYTES ≤ message.max.bytes` của broker nên
   được kiểm lúc khởi động thay vì để lệch âm thầm — `AdminClient.describeConfigs`
   đọc được cấu hình broker, và một `IllegalStateException` với thông báo bốn phần
   theo chuẩn của [`SecurityConfig`](./SecurityConfig.md) mục 5 sẽ biến một lỗi
   `RecordTooLargeException` chìm trong callback thành một lỗi khởi động không thể
   bỏ qua.
