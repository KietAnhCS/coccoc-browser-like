...
- [14. Chấm theo chuẩn doanh nghiệp](#14-chấm-theo-chuẩn-doanh-nghiệp)
...
## 14. Chấm theo chuẩn doanh nghiệp

| Tiêu chí | Trọng số | Điểm | Nhận xét |
|---|---|---|---|
| Đặt đúng câu hỏi cần đo | 15 % | **10/10** | Tách rời "nhanh hơn bao nhiêu" khỏi "khác đi bao nhiêu", và nói thẳng rằng câu hỏi thứ hai quan trọng hơn. Rất ít benchmark sinh viên làm được điều này |
| Cô lập biến (ablation) | 15 % | **7/10** | Ý tưởng giữ nguyên từ điển cho bản đối chiếu là **đúng hoàn toàn**; **trừ 3** vì `flattenToStringSet` bỏ sót `vietnamese-bigrams.txt` và không chuẩn hoá — chính lỗi trộn biến mà thiết kế sinh ra để tránh (mục 2.4) |
| Chất lượng bản đối chiếu | 10 % | **7/10** | Cài lại nguyên văn kể cả chỗ kém hiệu quả, có Javadoc chặn cám dỗ "tối ưu nó" — tư duy đúng; **trừ 3** vì **không một bài test nào** xác nhận nó tái hiện đúng hành vi cũ |
| Chuẩn hoá tách khỏi vùng đo | 10 % | **9/10** | Quyết định kỹ thuật đúng nhất trong lớp, có comment giải thích; **trừ 1** vì đầu ra không nói rằng tỷ số chỉ áp cho một giai đoạn |
| Chống cạm bẫy JIT | 10 % | **6/10** | Warm-up có, biến `sink` chống DCE có, hai hàm đo đối xứng có; **trừ 4** vì số vòng warm-up là hằng số magic không kiểm chứng, warm-up chung một vòng, không fork JVM |
| Xử lý nhiễu đo (GC, phương sai) | 15 % | **3/10** | **Điểm yếu lớn nhất.** Một phép đo duy nhất, không phương sai, không khoảng tin cậy, không xử lý GC, thứ tự đo thiên vị chống lại bên thắng cuộc. Trớ trêu khi `SignificanceTest` cùng gói tồn tại chính để chống lỗi này |
| Đo khác biệt kết quả | 10 % | **8/10** | Bốn đại lượng + 5 ví dụ đọc được bằng mắt là thiết kế xuất sắc, và phần này **tất định** nên đáng tin tuyệt đối; **trừ 2** vì `countDifferingBoundaries` không đối xứng trong khi nhãn ngụ ý đối xứng |
| Chất lượng đầu ra cho báo cáo | 5 % | **8/10** | In đủ ngữ cảnh từ điển để tái lập, chọn đơn vị "âm tiết/giây" đúng, ví dụ định dạng đẹp; **trừ 2** vì `%.2f` mời gọi độ chính xác giả và định dạng số phụ thuộc `Locale` mặc định |
| Chất lượng Javadoc / lập luận | 5 % | **10/10** | Đoạn giải thích vì sao phải giữ nguyên chỗ kém hiệu quả, và vì sao bản đối chiếu phải dùng từ điển mới, là hai đoạn đáng đưa vào giáo trình về phương pháp thực nghiệm |
| Kiểm thử | 5 % | **0/10** | Không có bộ test nào. `segmentGreedy` đã là `static` package-private, tức là đã sẵn sàng cho test — chỉ là chưa ai viết |
| Chất lượng mã / phòng thủ | 5 % | **5/10** | `flattenToStringSet` nhận tham số **không dùng** và **không kiểm `is == null`** trong khi `VietnameseWordDictionary.load()` cùng repo làm đúng; regex chuẩn hoá bị chép hai bản không có gì khoá lại |
| **TỔNG CÓ TRỌNG SỐ** | **100 %** | **≈ 6,8/10** | Một benchmark **suy nghĩ đúng về phương pháp** nhưng **thi hành chưa tới về mặt thống kê** |

```
   ┌──────────────────────────────────────────────────────────────┐
   │  NHẬN ĐỊNH TỔNG QUÁT, NÓI THẲNG                               │
   │                                                              │
   │  ĐIỀU LỚP NÀY LÀM RẤT TỐT:                                    │
   │    Nó biết PHẢI ĐO GÌ. Việc tách hai câu hỏi, việc giữ        │
   │    nguyên từ điển cho bản đối chiếu, việc cô lập bước         │
   │    chuẩn hoá, việc in ví dụ đọc được bằng mắt — bốn quyết     │
   │    định ấy đều ở mức trên trung bình của đồ án tốt nghiệp.    │
   │    Chúng cho thấy tác giả hiểu THIẾT KẾ THÍ NGHIỆM.           │
   │                                                              │
   │  ĐIỀU LỚP NÀY LÀM CHƯA TỚI:                                   │
   │    Nó không biết ĐO XONG THÌ PHẢI NÓI GÌ VỀ ĐỘ TIN CẬY.       │
   │    Một phép đo, không phương sai, không GC, hai chữ số        │
   │    thập phân. Đây không phải thiếu kiến thức — repo này       │
   │    có hẳn 367 dòng SignificanceTest chứng minh điều ngược     │
   │    lại. Đây là NGUYÊN TẮC CHƯA ĐƯỢC ÁP DỤNG NHẤT QUÁN         │
   │    giữa hai lớp nằm cùng một thư mục.                         │
   │                                                              │
   │  VÀ MỘT LỖI CỤ THỂ PHẢI SỬA TRƯỚC KHI TRÍCH SỐ VÀO LUẬN VĂN:  │
   │    Sai lệch từ điển ở mục 2.4 làm con số "tài liệu tách       │
   │    khác nhau" bị THỔI PHỒNG theo một hướng xác định.          │
   │    Con số ấy là con số quan trọng nhất mà lớp này sinh ra.    │
   └──────────────────────────────────────────────────────────────┘
```

### Năm đề xuất, xếp theo thứ tự ưu tiên

**1. Sửa sai lệch từ điển của bản đối chiếu — làm trước tiên, trước mọi thứ khác.**
Đây là khiếm khuyết duy nhất khiến một **con số đã in ra bị sai lệch có hệ
thống**, chứ không chỉ là thiếu độ tin cậy. `flattenToStringSet` đọc mỗi
`/vietnamese-words.txt` trong khi `VietnameseWordDictionary` nạp cả
`/vietnamese-bigrams.txt`, và nó không áp dụng `trim + NFC + toLowerCase` mà
`addWord()` áp dụng. Hệ quả: từ điển của bản tham lam **nhỏ hơn** từ điển của
bản QHĐ, nên một phần "khác biệt kết quả" bị quy nhầm cho thuật toán trong khi
nó đến từ từ điển — đúng cái lỗi trộn hai biến mà Javadoc dựng lên để tránh.
Tệ hơn: 154 mục bị bỏ sót là các cụm **đặc thù đề tài** ("công cụ tìm kiếm",
"an toàn thông tin"), tức là những cụm xuất hiện nhiều trong corpus, nên sai
lệch **không ngẫu nhiên**. Mã sửa đã có sẵn ở mục 11.4, khoảng 30 dòng. Đánh
đổi phải nói rõ: con số "tài liệu tách khác nhau" sẽ **giảm** sau khi sửa, có
thể giảm nhiều, và nếu báo cáo đã trích con số cũ thì phải chạy lại. Đó là cái
giá đúng phải trả — một con số nhỏ hơn nhưng đúng có giá trị hơn hẳn một con số
lớn hơn nhưng lẫn hai nguyên nhân.

**2. Đo nhiều lần, báo trung vị và khoảng, và tách bạch thời gian GC.**
Hiện mỗi thuật toán được đo **đúng một lần** và tỷ số in ra hai chữ số thập
phân. Với nguồn nhiễu đã liệt kê ở mục 6.3 (lập lịch OS, xung nhịp CPU, GC bất
đồng bộ), chữ số thứ hai gần như chắc chắn là nhiễu, mà định dạng `%.2f` lại
mời gọi người đọc chép nguyên nó vào luận văn. Cách sửa: gọi `timeGreedy` và
`timeDp` mỗi bên **5 lần xen kẽ nhau** (`G D G D G D G D G D` chứ không phải
`GGGGG DDDDD` — xen kẽ khử được cả xu hướng trôi theo thời gian lẫn thiên vị
thứ tự), lấy **trung vị** thay vì trung bình (bền hơn với một lần chạy xui gặp
GC lớn), và in kèm min–max. Thêm `ManagementFactory.getGarbageCollectorMXBeans()`
để lấy tổng thời gian GC trước và sau mỗi phép đo, rồi in ra như một cột riêng
— việc này **biến điểm yếu lớn nhất thành một bằng chứng bổ sung**, vì áp lực
GC khủng khiếp của bản tham lam chính là một phần lập luận. Chi phí: khoảng 20
dòng mã và ~12 giây chạy thêm trên một chương trình đã mất 30 giây. Đánh đổi:
đầu ra dài hơn và phức tạp hơn để đọc — chấp nhận được, vì đầu ra hiện tại đang
đơn giản hơn mức dữ liệu cho phép.

**3. Khoá bản đối chiếu và bước chuẩn hoá bằng test.**
Toàn bộ kết luận của lớp dựa trên hai giả định chưa từng được kiểm chứng: (a)
`segmentGreedy` thực sự tái hiện hành vi của `VietnameseTokenizer` cũ, và (b)
`splitIntoSyllables` trong benchmark sinh ra **cùng dãy âm tiết** với bản trong
`VietnameseTokenizer`. Giả định (b) đặc biệt mong manh vì mẫu regex bị chép hai
bản: sửa `NON_WORD` ở một nơi thì benchmark **vẫn biên dịch, vẫn chạy, vẫn ra
số** nhưng đang đo trên một đầu vào khác. Ba bài test ở mục 13 khoá giả định
(a); cho (b), cách rẻ nhất là để `VietnameseTokenizer.splitIntoSyllables` thành
`static` package-private rồi gọi thẳng nó từ benchmark — **xoá hẳn bản sao**
thay vì test nó. Đánh đổi: mở rộng bề mặt của lớp sản phẩm cho một mục đích
kiểm chứng. Với `package-private` và một dòng Javadoc nói rõ lý do, đây là đánh
đổi đáng — nó biến một nợ kỹ thuật thành không tồn tại thay vì chỉ giám sát nó.

**4. In thêm một phép đo `tokenize()` đầu-cuối để đặt cạnh tỷ số giai đoạn.**
Việc cô lập bước ghép từ là quyết định đúng (mục 4.1), nhưng nó tạo ra một rủi
ro trích dẫn thật: con số "nhanh gấp 4 lần" đúng cho **một giai đoạn đã được
cô lập có chủ ý**, còn người đọc báo cáo sẽ hiểu là "tokenizer nhanh gấp 4
lần". Theo định luật Amdahl, nếu ghép từ chiếm 25 % thời gian tokenize thì cải
thiện thật ở mức tokenize chỉ là ~1,23 lần. Cách sửa rẻ: chạy thêm một vòng đo
`new VietnameseTokenizer().tokenize(text(doc))` trên cùng tập tài liệu và in ra
cạnh đó, kèm tỷ lệ phần trăm mà giai đoạn ghép từ chiếm trong tổng. Hai con số
đứng cạnh nhau khiến việc trích dẫn quá đà trở nên khó, và đồng thời trả lời
được một câu hỏi mới đáng giá: *bước nào của tokenize mới thực sự đắt nhất?*
Đánh đổi: thêm một biến thể đo và một vòng warm-up nữa, chương trình chạy lâu
hơn vài giây.

**5. Làm `countDifferingBoundaries` đối xứng và bỏ cấp phát.**
Hàm hiện đếm số mốc **có trong `b` mà không có trong `a`**, tức khoảng một nửa
hiệu đối xứng, trong khi nhãn in ra là "moc gioi han khac nhau" — một cụm từ
mà mọi người đọc sẽ hiểu là đối xứng. Nó cũng đảo tham số thì cho kết quả khác,
một tính chất không ai mong đợi ở một hàm tên như vậy. Vì cả hai mảng đều **tăng
nghiêm ngặt** (bài test `boundariesCoverEveryInputSyllableExactlyOnce` bảo đảm
điều đó), có thể thay bằng một phép hợp nhất hai con trỏ chạy trong
O(|a|+|b|), **không cấp phát gì**, và cho ra hiệu đối xứng thật. Chỉ khoảng 15
dòng, đơn giản hơn bản hiện tại, và bỏ luôn được `HashSet<Integer>` cùng
autoboxing. Đánh đổi: con số in ra sẽ **tăng lên khoảng gấp đôi** so với bản
cũ, nên nếu báo cáo đã trích thì phải cập nhật — nhưng đây là loại thay đổi
đúng, vì đại lượng mới chính là đại lượng mà cái tên vẫn luôn hứa hẹn.
