...
|---|---|---|
| Chọn đúng phương pháp thống kê | 10/10 | Kiểm định **theo cặp** là lựa chọn duy nhất đúng cho cùng-tập-truy-vấn; khử `τ_i` giảm phương sai ~14 lần |
| Chạy hai kiểm định thay vì một | 10/10 | Randomization test là chuẩn ngành IR (Smucker–Allan–Carterette 2007); chạy kèm t-test cho một phép **phân tích độ nhạy miễn phí** |
| Trung thực về giới hạn của phép đo | 10/10 | `(đếm+1)/(B+1)`, `testsDisagree()`, `formatPValue` in `< 0,0001` — ba chỗ đều từ chối nói quá điều đo được |
| Chất lượng lập luận trong Javadoc | 10/10 | Giải thích **vì sao** ở mọi quyết định; đoạn về `(đếm+1)/(B+1)` và đoạn về cặp hoá đáng đưa vào giáo trình |
| Độ chính xác số học | 10/10 | Đổi vế phân số liên tục, `log1p`, `logGamma` thay `Γ` — cả ba cạm bẫy kinh điển đều được né đúng |
| Chất lượng kiểm thử | 10/10 | Đối chiếu **dạng đóng giải tích** thay vì thư viện khác; đây là tư duy trên mặt bằng đồ án một bậc |
| Tính tái lập | 9/10 | Seed cố định đảm bảo báo cáo tái lập; **trừ 1** vì chưa từng kiểm chứng kết luận ổn định qua nhiều seed |
| Xử lý ca suy biến | 8/10 | Hai nhánh `n < 2` và `SE == 0` đều có lý do đúng; **trừ 2** vì nhánh `SE == 0` trả `p = 0,0`, mâu thuẫn với chính nguyên tắc "không bao giờ báo p = 0" |
| Hiệu chỉnh so sánh bội | **5/10** | Chọn trước 6 cặp là phòng vệ đúng và rẻ, nhưng **m = 6 vẫn cho 26,5 % rủi ro có ít nhất một kết luận sai** và không hề được hiệu chỉnh hay ghi chú |
| Báo cáo độ lớn hiệu ứng | **6/10** | Có ΔMRR và KTC 95 % — đủ nguyên liệu; **nhưng không có ngưỡng thực tiễn nào** và không có gì buộc người đọc phân biệt "có ý nghĩa thống kê" với "đáng triển khai" |
| Chất lượng bộ sinh ngẫu nhiên | **6/10** | `java.util.Random` là LCG 48 bit và mã lấy từ **bit thấp** — chính phần yếu nhất; chưa lộ vấn đề ở 20 triệu bit nhưng là nợ kỹ thuật thật |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Thêm hiệu chỉnh so sánh bội tường minh vào chính `Result`, hoặc ít nhất
   ghi số cặp vào báo cáo.** Đây là khiếm khuyết học thuật nghiêm trọng nhất còn
   lại. Chọn trước 6 cặp đã cắt rủi ro từ 98,2 % xuống 26,5 %, nhưng 26,5 % vẫn
   nghĩa là cứ bốn lần chạy báo cáo thì một lần có một dòng "✅" là nhiễu thuần
   tuý — và không có gì trong bảng cho biết điều đó. Cách sửa rẻ nhất là thêm
   một phương thức `isSignificantAt(double alpha)` vào `Result`, rồi để
   `EvaluationRunner` gọi nó với `ALPHA / soCapKiemDinh`. Với m = 6, α' =
   0,00833, hoàn toàn nằm trong phân giải 1e−5 của randomization test. Đánh đổi
   phải nói rõ: Bonferroni sẽ làm vài cặp đang "✅" rơi xuống "❌", và một số
   trong đó là cải tiến có thật — đây là cái giá của việc kiểm soát sai lầm loại
   I trên toàn bộ họ giả thuyết, và nó đáng trả trong một báo cáo học thuật.

2. **Thêm `isPracticallySignificant(double nguongDelta)` và buộc báo cáo in kèm
   ngưỡng ấy.** Hiện `Result` có đủ nguyên liệu để phân biệt "có ý nghĩa thống
   kê" với "đáng triển khai", nhưng không có gì buộc người đọc làm phép phân
   biệt đó — và mục 9 cho thấy đây đúng là chỗ luận văn hay đọc sai nhất. Một
   phương thức trả về `confidenceLow > nguongDelta` (chứ không phải
   `meanDifference > nguongDelta`) sẽ nói đúng điều cần nói: *dữ liệu tương
   thích với việc cải tiến thật sự vượt ngưỡng đáng kể*. Chọn ngưỡng là quyết
   định sản phẩm chứ không phải thống kê, và chính vì thế nó phải được viết
   thành hằng số có tên, có Javadoc giải thích, thay vì để ngầm. Đánh đổi: mọi
   ngưỡng đều tuỳ ý ở mức nào đó, và đặt một con số vào mã sẽ mời gọi tranh cãi
   — nhưng tranh cãi công khai về ngưỡng tốt hơn nhiều so với việc không ai đặt
   câu hỏi.

3. **Thay `java.util.Random` bằng `RandomGenerator.of("L64X128MixRandom")` hoặc
   `SplittableRandom`.** `java.util.Random` là LCG 48 bit từ thời Java 1.0, và
   điểm yếu đã biết của nó nằm đúng ở **các bit thấp** — mà vòng lặp hoán vị lại
   đọc `bits & 1L` rồi dịch phải, tức là tiêu thụ từ bit thấp lên. Với 20 triệu
   bit và một thống kê thô như dấu cộng/trừ thì gần như chắc chắn chưa lộ vấn
   đề, nhưng "gần như chắc chắn" là một câu yếu để đặt vào phần bảo vệ tính hợp
   lệ của cả chương kết quả. `SplittableRandom` là thay thế một dòng, nhanh hơn
   (không có `synchronized`), chất lượng bit đồng đều, và vẫn nhận seed cố định
   nên tính tái lập không đổi. Đánh đổi duy nhất: các con số p-value trong báo
   cáo sẽ đổi ở chữ số thứ tư, nên phải chạy lại và kiểm tra không kết luận nào
   lật — mà đó chính là việc đề xuất 4 yêu cầu làm.

4. **Chạy một lần kiểm chứng đa seed rồi ghi kết quả vào tài liệu.** Seed cố
   định làm sai số Monte Carlo trở nên **tất định**, không làm nó biến mất. Nếu
   p thật của một cặp là 0,0503 và seed 42 tình cờ cho 0,0487, báo cáo sẽ sai
   theo đúng một hướng ở mọi lần build, mãi mãi, và không có cách nào phát hiện
   từ bên trong. Cách phòng rẻ nhất là viết một bài test hoặc một script chạy
   `pairedTest` với năm seed khác nhau trên đúng sáu cặp thật, xác nhận không
   cặp nào đổi kết luận nhị phân, rồi ghi bảng kết quả ấy vào `docs/EVALUATION.md`
   như một phụ lục. Chi phí một lần khoảng vài giây chạy máy và ~30 dòng mã.
   Đánh đổi: nó đòi hỏi `SEED` phải trở thành tham số của `randomizationTwoTailed`
   thay vì hằng số riêng tư, tức là mở rộng bề mặt API cho đúng một mục đích
   kiểm chứng — chấp nhận được nếu phương thức nhận seed để ở mức `package-private`.

5. **Làm cho nhánh `standardError == 0` nhất quán với nguyên tắc sàn p-value.**
   Hiện nhánh này trả `p = 0,0` khi hiệu là hằng số khác 0, trong khi toàn bộ
   phần còn lại của lớp được xây quanh nguyên tắc "không bao giờ báo p = 0 vì đó
   là khẳng định mạnh hơn điều phép đo chứng minh được". Về lý thuyết `p = 0,0`
   ở đây biện minh được (mọi phép hoán vị đều cho `|trung bình|` nhỏ hơn, nên
   kết luận đến từ lập luận chứ không từ lấy mẫu), nhưng nó tạo một điểm không
   nhất quán trong hợp đồng của lớp: người đọc `Result` không có cách nào biết
   `p = 0,0` đến từ nhánh lý thuyết hay từ một lỗi. Trả về đúng sàn
   `1/(PERMUTATIONS+1)` ở nhánh này giữ mọi kết luận nhị phân y nguyên, làm hợp
   đồng trở thành bất biến kiểm chứng được (`0 < p <= 1` luôn đúng), và cho phép
   viết một bài test bao trùm. Đánh đổi thật: bài test
   `constantNonZeroDifferenceDoesNotProduceNaN` hiện đang khẳng định
   `assertEquals(0.0, result.pValueTTest())`, nên thay đổi này buộc phải sửa
   test — và đó là loại sửa test **đúng**, vì hành vi mong đợi thay đổi có chủ ý.
