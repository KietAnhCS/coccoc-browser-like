...
|---|---|---|
| Chọn đúng phương pháp đánh giá cho ràng buộc thực tế | 10/10 | Known-item là lựa chọn đúng khi không có ngân sách gán nhãn; và repo vẫn dựng **song song** đường qrels để phủ phần known-item không phủ được |
| Chất lượng lập luận trong Javadoc | 9/10 | Đoạn giải thích cửa sổ df (dòng 31–40) là đoạn đáng đưa vào giáo trình: nêu đúng ca hỏng, đúng hậu quả, đúng cách chặn. **Trừ 1** vì mô tả `combinedText` gồm thân bài — điều **không xảy ra** trên thực tế |
| Tính tái lập | 10/10 | Ba tầng bảo đảm (seed, `sort` trước `shuffle`, `LinkedHashMap`), trong đó hai tầng sau là loại chi tiết mà đa số bỏ sót |
| Xử lý ca biên | 9/10 | `Math.max(MIN_DF, ...)`, `Math.min(limit, size)`, `doc == null`, `url == null`, `tokenize(null)` — phủ kín. **Trừ 1** vì không có ca nào được kiểm chứng bằng test |
| Chọn cấu trúc dữ liệu | 9/10 | `HashSet` cho `titleTerms` và `usedQueryTexts` (O(1) đúng chỗ), `LinkedHashMap` cho tái lập, local `record` cho phạm vi hẹp. **Trừ 1** cho `List<Integer>` autoboxing |
| Tham số thí nghiệm hiển thị được | 7/10 | `MIN_DF` và `MAX_DF_RATIO` là `public` và **được in ra báo cáo** — đúng chuẩn khoa học. **Trừ 3** vì `TITLE_BOOST` thì `private` và không xuất hiện ở đâu, dù nó ảnh hưởng trực tiếp tới một kết luận của báo cáo |
| **Tính đúng đắn so với ý định đã viết** | **3/10** | `doc.getBodyText()` **luôn null** vì `InvertedIndex` lưu `doc.withoutBodyText()`. Toàn bộ thân bài không bao giờ được xét; `tf` gần như luôn bằng 1; `TITLE_BOOST` gần như vô hiệu. Lỗi im lặng vì ternary `!= null ? ... : ""` |
| **Chất lượng kiểm thử** | **1/10** | **Không một bài test đơn vị nào.** Lớp sinh ra ground truth cho cả chương đánh giá mà chỉ được bảo vệ bởi một `assertFalse(isEmpty())` gián tiếp |
| **Công bố thiên lệch** | **3/10** | Bảy thiên lệch ở mục 8 đều có thật, hai trong số đó (tiêu đề, mô hình chấm điểm) bẻ cong **đúng những so sánh mà báo cáo dùng để kết luận**, và **không thiên lệch nào được ghi** trong `docs/EVALUATION.md` |
| Kiểm chứng vòng lặp tách từ | **2/10** | `replace('_',' ')` giả định `QueryParser` ghép lại đúng term gốc. Trường `terms` được giữ đúng để kiểm chứng điều đó — nhưng **không ai kiểm** |
| Nhất quán giữa các nơi gọi | 6/10 | `EvaluationRunner` và `RankingQualityTest` đặt hằng số có tên; `GinBaselineRunner` dòng 62 cắm thẳng `3` và `42L`, làm seed cố định mất tác dụng bảo vệ nếu ai đó đổi seed |
| Chi phí | 10/10 | `break` sớm làm chi phí gần như độc lập với cỡ corpus; `getPostings` O(1); không tối ưu sớm phép `sort` — đúng ưu tiên |

**Tám đề xuất, xếp theo thứ tự nên làm:**

1. **Viết bộ test đơn vị cho lớp này — bảy tính chất ở mục 12, ưu tiên bài kiểm
   chứng vòng lặp tách từ ở mục 10.5.** Đây là khoảng trống nghiêm trọng nhất và
   cũng rẻ nhất để lấp. Lớp này **định nghĩa** chuẩn đúng cho toàn bộ chương đánh
   giá, nên nó không có gì ở phía trên để đối chiếu — một sai sót ở đây lan xuống
   mọi con số mà không cơ chế nào chặn được. Riêng bài `moiTermNamTrongCuaSoDf`
   khoá lại đúng cái ca hỏng mà Javadoc gọi là "chỗ dễ làm sai nhất": nếu ai đó hạ
   `MIN_DF` hoặc làm hỏng phép tra `df`, mọi bài test khác trong repo vẫn xanh và
   `docs/EVALUATION.md` vẫn in ra một bảng đẹp gồm toàn số 1,0000. Chi phí: khoảng
   120 dòng test, chạy trên corpus seed đã có sẵn trong repo. Đánh đổi: bài
   `boTruyVanTaiLapDuocTheoSeed` sẽ **đỏ mỗi khi ai đó sửa tokenizer hoặc từ điển**
   — và đó chính là hành vi mong muốn, vì lúc đó bộ truy vấn thật sự đã đổi và báo
   cáo cần chạy lại.

2. **Sửa lỗi thân bài (mục 10.3), và chạy lại toàn bộ báo cáo trong cùng một
   commit.** `doc.getBodyText()` trả `null` là một lỗi rõ ràng, không phải một lựa
   chọn thiết kế — Javadoc dòng 130–133 mô tả một hành vi không xảy ra, và ternary
   `!= null ? ... : ""` đã biến một `NullPointerException` dễ thấy thành một mất mát
   dữ liệu không ai thấy. Cách sửa chỉ là một biểu thức, vì `pickDistinctiveTerms`
   đã có sẵn tham số `index`. Đánh đổi phải nói thẳng và không nhỏ: chi phí sinh
   truy vấn tăng khoảng một bậc (thêm giải nén, `combinedText` dài gấp mười sáu
   lần), **mọi con số trong `docs/EVALUATION.md` sẽ đổi**, và thiên lệch tiêu đề ở
   mục 8.5 sẽ bật lên ở mức đầy đủ. Vì thế đề xuất này **phải đi kèm** đề xuất 3
   trong cùng một lần thay đổi; tách ra sẽ tạo một khoảng thời gian mà báo cáo bị
   thiên vị mạnh hơn hiện tại mà không ai biết.

3. **Đưa `TITLE_BOOST` lên `public`, in vào báo cáo, và sinh một bộ truy vấn đối
   chứng với `TITLE_BOOST = 1,0`.** Rò rỉ ở mục 6.2 là vấn đề phương pháp nghiêm
   trọng nhất còn lại: bộ truy vấn thiên về từ khoá tiêu đề đang được dùng để trả
   lời câu hỏi "title boost có đáng không". Không cần bỏ `TITLE_BOOST` — lập luận
   mô phỏng người dùng đứng vững. Cần làm hai việc rẻ: (a) in hệ số ra báo cáo như
   hai hằng số kia, để người đọc biết bộ truy vấn thiên về đâu; (b) chạy thêm một
   lượt với hệ số 1,0 và đối chiếu — nếu kết luận "title boost đáng dùng" còn đứng
   ở cả hai bộ thì nó thật, còn nếu chỉ đứng ở bộ có boost thì nó là ảo giác do
   chính bộ sinh tạo ra. Đánh đổi: cần một tham số hoá nhỏ (biến `TITLE_BOOST`
   thành tham số của constructor, giữ 2,0 làm mặc định) và một lượt chạy nữa,
   khoảng vài giây máy — quá rẻ so với việc bảo vệ được một kết luận của luận văn.

4. **Thêm một mục "Giới hạn của phương pháp" vào `docs/EVALUATION.md`, sinh tự động
   từ `EvaluationRunner`.** Bảy thiên lệch ở mục 8 đều có thật, có hướng xác định,
   và hiện **không cái nào** được ghi ở đâu ngoài tài liệu này. Một người đọc báo
   cáo sẽ thấy "MRR = 0,64" và không có cách nào biết rằng con số ấy được đo trên
   những truy vấn không ai gõ, với ground truth đơn nhất, trên tập tài liệu đã bị
   lọc. Nội dung tối thiểu: bảng 8.8 rút gọn còn ba dòng nghiêm trọng nhất, cộng
   một câu nói rõ "MRR tuyệt đối không so được ra ngoài bối cảnh này; chỉ so sánh
   tương đối giữa các cấu hình mới có ý nghĩa". Đánh đổi duy nhất là nó làm báo cáo
   trông kém ấn tượng hơn — và đó chính xác là điều nên xảy ra, vì một báo cáo tự
   nêu giới hạn của mình là báo cáo khó bác bỏ hơn nhiều so với một báo cáo im
   lặng.

5. **Khử trùng theo tập term chuẩn hoá chứ không theo chuỗi.** Hiện `usedQueryTexts`
   chứa `queryText` nguyên văn, nên "a b c" và "b a c" được coi là hai truy vấn
   khác nhau dù `QueryParser` phân giải chúng thành cùng một tập ứng viên và cùng
   một thứ hạng. Trên thực tế điều này hiếm xảy ra vì term luôn xếp theo điểm giảm
   dần, nhưng "hiếm" không phải "không", và hậu quả thì đúng bằng hậu quả của việc
   không khử trùng — ground truth nhập nhằng, MRR tụt cơ học, giả định độc lập của
   `SignificanceTest` bị vỡ. Cách sửa gọn: đưa `new TreeSet<>(terms).toString()` vào
   `usedQueryTexts` thay vì `queryText`. Đánh đổi: thay đổi này **sẽ làm bộ truy vấn
   hiện tại đổi** nếu có dù chỉ một cặp trùng tập term, nên phải chạy lại báo cáo —
   lại một lý do nữa để gộp chung với đề xuất 2.

6. **Làm `MIN_DF` co giãn theo cỡ corpus, giống cách `MAX_DF_RATIO` đang làm.** Bảng
   ở mục 5.3 cho thấy một bất đối xứng thật: chặn trên là tỷ lệ, chặn dưới là hằng
   số tuyệt đối. Hệ quả là ở corpus 50.000 tài liệu, `df = 3` lại quay về đúng ca
   "term gần như duy nhất" mà `MIN_DF` sinh ra để chặn — tức là biện pháp phòng vệ
   tự vô hiệu hoá khi hệ thống lớn lên, đúng lúc nó cần nhất. Một công thức như
   `max(3, round(totalDocs × 0,001))` giữ ngữ nghĩa "term phải xuất hiện ở ít nhất
   một phần nghìn corpus" ổn định qua mọi cỡ. Đánh đổi: thêm một hằng số nữa để giải
   thích và bảo vệ, và ở cỡ 5.011 hiện tại thì công thức mới cho đúng giá trị cũ
   (`round(5,011) = 5` — thực ra khác chút, nên vẫn phải chạy lại báo cáo). Đây là
   đề xuất về **khả năng mở rộng**, không phải về tính đúng đắn hôm nay, nên xếp
   sau.

7. **Chạy một lần kiểm chứng đa seed và ghi kết quả vào phụ lục báo cáo.** Seed 42
   cố định làm bộ truy vấn tái lập được, nhưng nó không loại bỏ rủi ro "200 truy vấn
   này tình cờ có lợi cho cấu hình X" — nó chỉ làm rủi ro đó **tất định**, tức là ta
   sẽ sai theo đúng một hướng ở mọi lần build và không có cách nào phát hiện từ bên
   trong. Đây đúng là lập luận mà `SignificanceTest.md` mục 4.5 đưa ra cho seed của
   randomization test, và nó áp dụng ở đây **mạnh hơn**, vì seed ở lớp này quyết
   định cả tập truy vấn chứ chỉ riêng nhiễu Monte Carlo. Cách rẻ nhất: chạy
   `EvaluationRunner` với năm seed khác nhau một lần duy nhất, xác nhận thứ tự xếp
   hạng giữa các cấu hình không đổi, ghi bảng ấy vào `docs/EVALUATION.md` như phụ
   lục, rồi cố định lại seed 42. Chi phí: ~30 giây máy và một bảng năm dòng.

8. **In chỉ số sức khoẻ của bộ truy vấn (mục 10.2).** Hiện `generate()` bỏ qua tài
   liệu bằng ba lệnh `continue` im lặng, và `EvaluationRunner` chỉ in số truy vấn
   sinh được. Ba bộ đếm — thiếu URL, không đủ term, trùng truy vấn cũ — cho biết
   ngay thiên lệch 8.1 đang nặng đến mức nào: nếu phải duyệt 900 tài liệu để lấy 200
   truy vấn thì 78 % corpus đã bị loại khỏi bài thi, và MRR đo được nói về một phần
   corpus rất khác với phần mà người dùng thật sẽ đụng tới. Chi phí: ba biến `int`
   và một dòng `printf`. Đánh đổi: không có đánh đổi nào — đây là đề xuất duy nhất
   trong danh sách **không** làm đổi bất kỳ con số nào của báo cáo hiện tại.
