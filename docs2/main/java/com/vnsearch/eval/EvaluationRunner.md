...
- [9. Chấm theo chuẩn doanh nghiệp](#9-chấm-theo-chuẩn-doanh-nghiệp)
...
|---|---|---|
| Tính tái lập | 10/10 | Seed cố định, sắp xếp trước khi index, câu lệnh tái lập tự dán vào báo cáo với **số truy vấn thật** chứ không phải số yêu cầu |
| Thiết kế thí nghiệm (ablation) | 9/10 | Bốn nhóm ánh xạ một–một với bốn câu hỏi; trừ điểm vì nhóm 3 đổi hai biến cùng lúc — dù mã **tự thừa nhận** điều đó |
| Nghiêm ngặt thống kê | 10/10 | Ghép cặp, hai kiểm định độc lập giả định, chọn trước 6 cặp để né so sánh bội, báo p-value dạng ngưỡng — mức này hiếm thấy ở đồ án |
| Tự phê bình phương pháp | 10/10 | `LIMITATIONS` nêu 4 hạn chế trong đó có hạn chế **làm yếu kết luận có lợi nhất** của chính báo cáo (thiên vị chống PageRank) |
| Phát hiện lỗi đo lường | 10/10 | Mục thang đo lật ngược cách diễn giải toàn bộ bảng kết quả; vết sửa `bareTfIdf` được giữ lại kèm lý do đầy đủ |
| Trung thực về giới hạn | 9/10 | Tự nói cột `ms/truy vấn` không đáng tin, tự chỉ sang `GIN-BASELINE.md`; nhưng vẫn **giữ cột đó trong bảng** |
| Nhất quán giữa văn và mã | **3/10** | Ba lỗi cùng lúc: số mục lệch một bậc (4 tham chiếu chéo trỏ nhầm), "13 cấu hình" trong khi có 11, "5.011 tài liệu" trong khi corpus 31.030 |
| Ghép nối lỏng / tra cứu bằng chuỗi | **4/10** | `findByLabel`, `contains("đang dùng")`, `equals("TF-IDF thuần")` — đổi một ký tự làm mất hàng báo cáo mà **không có cảnh báo nào** |
| Tuân thủ nguyên tắc của chính gói | **5/10** | `analyseScoreScales` tự dựng lại `QueryParser`/`Resolver`/`Ranker` — đúng điều `EvaluationHarness` sinh ra để cấm; và tạo thể hiện tokenizer thứ tư |
| Khả năng kiểm thử | **4/10** | `main` 55 dòng trộn I/O với điều phối; 9 hàm tĩnh `private` nên không test được từ ngoài dù logic thuần tuý |
| Hiệu quả tài nguyên | 6/10 | 2.000 lần `resolve` thừa (~18,6 triệu posting); `docs` giữ vài trăm MB chỉ để lấy `size()` |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Đánh số mục của báo cáo bằng bộ đếm thay vì viết tay.** Hiện số mục nằm rải
   rác trong năm hằng chuỗi khác nhau, và bảng mục lục trong `READING_GUIDE` đã
   lệch một bậc so với báo cáo thật — mục "Kiểm định ý nghĩa thống kê", phần công
   phu nhất của cả tài liệu, không hề xuất hiện trong mục lục, còn bốn tham chiếu
   chéo kiểu "xem mục 7" đều trỏ nhầm chỗ. Cách sửa gọn: một `SectionNumberer` giữ
   biến đếm, mỗi hàm sinh mục gọi `next("Kiểm định ý nghĩa thống kê")` để lấy về
   cả tiêu đề lẫn số, và các tham chiếu chéo dùng `ref("thang đo")` tra ngược từ
   khoá. Đánh đổi thật là mất đi khả năng đọc thẳng văn bản trong mã — hiện tại
   `LIMITATIONS` đọc như một đoạn văn, sau khi sửa nó sẽ có `%s` rải rác. Với một
   tài liệu mà giá trị nằm ở chỗ đọc trôi chảy, đây là mất mát có thật; nhưng một
   mục lục trỏ nhầm là thứ hội đồng phát hiện trong ba mươi giây.

2. **Truyền số liệu corpus vào các khối văn bản thay vì viết cứng.** Câu "Gán nhãn
   5.011 tài liệu cho 30 truy vấn là 150.000 lượt đánh giá" nằm cách dòng in
   `| Số tài liệu | 31030 |` đúng hai mươi dòng trong cùng một file xuất ra, và
   mâu thuẫn công khai với nó. Trớ trêu là con số cũ đang **làm yếu** chính luận
   điểm nó phục vụ: với corpus thật, chi phí gán nhãn là 930.618 lượt, tức mạnh
   hơn sáu lần. Sửa bằng cách đổi `METHOD_WHY` từ hằng `String` thành hàm
   `methodWhy(int docCount, int numTopics)` dùng `String.format` với `%,d`. Đánh
   đổi: mọi hằng văn bản khác sẽ chịu áp lực chuyển thành hàm theo, và ranh giới
   giữa "văn bản thuần" với "văn bản có tham số" trở nên mờ. Nên chấp nhận quy tắc
   rõ ràng: khối nào chứa **bất kỳ** con số nào về corpus thì phải là hàm, không
   được là hằng.

3. **Gắn p-value vào câu kết luận ở mục Nhận xét.** Mục 5 tính đủ khoảng tin cậy
   và hai loại p-value, rồi mục 6 viết "Cấu hình này tốt hơn cấu hình đang dùng
   %.1f%% về MRR, nên **đề xuất đổi sang bộ trọng số đó**" mà không hề tra lại
   kết quả kiểm định. Nếu chênh lệch là 0,3% với p = 0,71, câu đó đang khuyên
   người đọc đổi cấu hình sản phẩm dựa trên nhiễu. Sửa: trước khi sinh câu đề
   xuất, chạy `SignificanceTest.pairedTest(best.reciprocalRanks(),
   current.reciprocalRanks())` rồi rẽ nhánh — có ý nghĩa thì "đề xuất đổi", chưa
   có ý nghĩa thì "chênh lệch chưa phân biệt được với nhiễu, giữ nguyên cấu hình
   hiện tại". Đánh đổi: báo cáo sẽ có ít câu kết luận dứt khoát hơn, và một đồ án
   thường muốn kết luận dứt khoát. Nhưng đúng chỗ này là nơi phân biệt một báo cáo
   thực nghiệm với một bài quảng cáo.

4. **Thống nhất một hàm định dạng số cho toàn bộ báo cáo.** Hiện `formatPValue`
   đổi dấu chấm thành phẩy, còn `ΔMRR`, khoảng tin cậy, tỷ lệ phần trăm và mọi
   con số khác đều in bằng `Locale.US` giữ dấu chấm — nên trong cùng một hàng bảng
   có cả `+0.0231` lẫn `0,0037`. Không sai về giá trị, nhưng là loại chi tiết mà
   người chấm nhìn thấy ngay và nó gieo nghi ngờ về mức cẩn thận của phần còn lại.
   Sửa bằng một lớp `VnFormat` nhỏ với `num(double, int)` và `percent(double)`,
   dùng `Locale` Việt Nam hoặc `String.replace` tập trung một chỗ. Đánh đổi duy
   nhất là phải rà lại toàn bộ hai chục lời gọi `String.format` — công việc cơ
   học, rủi ro thấp, nhưng dễ bỏ sót nếu không có test khoá lại định dạng.

5. **Tách `main` thành một lớp điều phối kiểm thử được, và mở giao diện harness đủ
   rộng để bỏ đường đi tự dựng.** Hai vấn đề này nên sửa cùng lúc vì chúng có
   chung gốc: `main` vừa đọc tệp, vừa dựng chỉ mục, vừa điều phối thí nghiệm, vừa
   ghi tệp, nên không có chỗ nào chèn một corpus giả vào để test; và
   `analyseScoreScales` phải tự dựng `QueryParser`/`CandidateResolver`/`ResultRanker`
   vì `harness.search` chỉ trả `List<String>`. Cách sửa: cho `EvaluationHarness`
   thêm `searchDetailed` trả `record Hit(String url, double prScore)`, rồi rút
   phần điều phối vào `EvaluationExperiment` nhận sẵn `index` + `pageRank` +
   `queries` và trả `String` báo cáo — `main` chỉ còn đọc tham số, nạp corpus, gọi,
   ghi tệp. Đánh đổi: thêm một lớp và một `record` cho thứ hiện đang chạy được;
   với một đồ án sắp bảo vệ, đó là chi phí thật. Nhưng nó đồng thời xoá luôn thể
   hiện tokenizer thứ tư — nguồn duy nhất có thể khiến mục 8 lặng lẽ biến mất khỏi
   báo cáo trong khi mục lục vẫn hứa có nó.
