...
|---|---|---|
| Lý do tồn tại được nêu rõ | 10/10 | Javadoc nói thẳng công cụ ra đời để bác một ước lượng cụ thể; hiếm có công cụ đo nào ghi lại được động cơ của chính nó |
| Tính hợp lệ của phương pháp đo | 9/10 | `settle()` bốn vòng, ba mốc trừ nhau, và **buông corpus trước khi đo trạng thái ổn định** — bước quyết định mà phần lớn benchmark bộ nhớ bỏ sót |
| Trung thực về sai số | 10/10 | Javadoc tự nêu điểm yếu của `System.gc()` **trước khi** người đọc kịp hỏi, kèm lập luận vì sao sai số đó vẫn chấp nhận được cho câu hỏi đang hỏi |
| Quy trách nhiệm bằng số đếm | 9/10 | Đếm posting/vị trí thật thay vì ước lượng; nhưng `boxedCost` bỏ sót chi phí `ArrayList` và header `Object[]` nên là cận dưới |
| Chống tối ưu hoá của JIT | 7/10 | Có chú thích và có `printf` giữ tham chiếu, nhưng dùng tác dụng phụ của I/O chứ không dùng `reachabilityFence` — đúng trong thực tế, không được bảo đảm bởi đặc tả |
| Xử lý ca biên | **5/10** | Corpus rỗng chia cho 0 ở dòng 165 → in `Infinity`; corpus thiếu thì in thông báo rồi thoát êm (đúng), nhưng ca rỗng thì không được xét |
| Khả năng kiểm thử | **3/10** | Toàn bộ logic nằm trong `main`, kết quả đi ra `System.out`, không có giá trị trả về nào để khẳng định — **không viết được một bài test nào** mà không sửa cấu trúc |
| Trình bày kết quả | 8/10 | Có tách rõ đỉnh/ổn định và giải thích ngay trong đầu ra; nhưng thứ tự in theo trình tự đo chứ không theo tầm quan trọng |
| Tính lặp lại | 7/10 | Chạy lại được bằng một dòng lệnh, nhưng kết quả phụ thuộc bộ thu gom rác và cờ JVM đang dùng — mà **không cờ nào được ghi lại trong đầu ra** |
| Giá trị đối chứng bên ngoài | 9/10 | Con số 136,5 MB là vế còn lại của phép so với GIN 15,9 MB; không có nó thì so sánh với PostgreSQL chỉ còn phần tốc độ |
| Tài liệu hoá trong mã | 10/10 | Mọi quyết định khó hiểu (`settle` bốn vòng, `documents = null`, `printf` cuối, nhân 2 cho UTF-16) đều có chú thích nêu **lý do**, không mô tả lại mã |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Tách phần đo khỏi phần in, trả về một `record MemorySnapshot`.** Hiện toàn
   bộ 105 dòng của `main` vừa đo, vừa tính, vừa định dạng, vừa in — nên không có
   một điểm nào để một bài test bám vào. Chỉ cần đổi thành `public static
   MemorySnapshot measure(String corpusPath)` trả về `record MemorySnapshot(int
   docCount, long documentsCost, long indexCost, long steadyState, long
   postingCount, long positionCount)` rồi để `main` gọi nó và in, thì cả ba bài
   test ở mục 8 viết được ngay. Quan trọng hơn kiểm thử: một `record` cho phép
   ghi kết quả ra JSON và **so sánh tự động giữa hai lần chạy**, tức là biến công
   cụ này từ "chạy tay rồi chép số" thành thứ có thể đặt vào CI để phát hiện hồi
   quy bộ nhớ. Cái giá là thêm khoảng 20 dòng và một lớp `record` — rất nhỏ so
   với việc mở khoá toàn bộ nhóm khả năng đó.

2. **Ghi lại môi trường đo ngay trong đầu ra.** Cùng một corpus, chạy với
   `-XX:+UseSerialGC` và với G1 mặc định sẽ cho hai con số lệch nhau vài phần
   trăm; chạy với `-Xmx2g` và `-Xmx8g` cũng vậy, vì heap lớn khiến GC lười dọn
   hơn. Hiện đầu ra không nói gì về điều này, nên một bảng số chép từ hai lần
   chạy khác cấu hình sẽ trông như một cải thiện thật. Bốn dòng in
   `Runtime.version()`, `maxMemory()`, tên bộ thu gom rác lấy từ
   `ManagementFactory.getGarbageCollectorMXBeans()` và số nhân CPU là đủ để mọi
   con số về sau tự mang theo ngữ cảnh của nó. Đây là chi phí gần bằng không đổi
   lấy khả năng phát hiện một loại sai lệch mà không cách nào phát hiện về sau.

3. **Tính `boxedCost` cho đủ, hoặc đổi tên để nói rõ nó là cận dưới.** Công thức
   `positionCount × 24` bỏ qua 40 byte của mỗi `ArrayList` và 16 byte header của
   mỗi `Object[]`, mà hai khoản này tính theo **số posting** chứ không theo số vị
   trí — với 1,59 triệu posting chứa trung bình 2,4 vị trí, chúng chiếm tới 89 MB,
   nhiều hơn cả phần đang được tính. Hệ quả: con số in ra (87,5 MB) chỉ bằng
   khoảng một nửa chi phí thật (165,7 MB), nên đề xuất "đổi sang `int[]`" thật ra
   còn **hấp dẫn hơn** những gì công cụ tự nói. Sửa công thức thành
   `positionCount × 24 + postingCount × 56` là một dòng, và nó khiến công cụ
   không còn tự đánh giá thấp chính phát hiện quan trọng nhất của mình.

4. **Dùng `Reference.reachabilityFence(index)` thay cho việc dựa vào `printf`.**
   Dòng cuối cùng của chương trình đang giữ chỉ mục sống bằng tác dụng phụ của
   một lời gọi I/O — cách này hoạt động trên mọi JVM hiện tại, nhưng nó dựa vào
   một tính chất mà đặc tả không hứa hẹn, và nó sẽ **im lặng** khi hỏng: kết quả
   sẽ là một `steadyState` nhỏ bất thường, thứ rất dễ bị đọc nhầm thành "bản mới
   tiết kiệm hơn hẳn". `reachabilityFence` được thêm vào Java 9 đúng cho tình
   huống này, không tốn một chỉ thị máy nào khi chạy, và nó **tự tài liệu hoá ý
   định** — người đọc sau sẽ không xoá nhầm nó như có thể xoá nhầm một dòng
   `printf` trông như thừa.

5. **In bảng theo thứ tự tầm quan trọng, kèm một dòng kết luận tự sinh.** Đầu ra
   hiện tại xếp theo trình tự đo (tài liệu trước, chỉ mục sau, ổn định cuối), tức
   là khoản lớn nhất nằm ở giữa và con số đáng so sánh nhất nằm cuối. Người đọc
   lần đầu — chính là người cần công cụ này nhất — sẽ dừng ở dòng `bodyText` và
   rút ra đúng kết luận sai mà công cụ sinh ra để bác bỏ. Một dòng tự sinh dạng
   *"khoản lớn nhất: vị trí dạng Integer, 29,5% — đổi sang int[] tiết kiệm
   72,9 MB"* đặt ngay đầu báo cáo sẽ khiến kết luận không phụ thuộc vào việc
   người đọc có kiên nhẫn đọc hết bảng hay không. Đánh đổi duy nhất là công cụ
   phải tự xếp hạng các khoản, tức là phải biết danh sách khoản là gì — một cấu
   trúc mà đề xuất 1 vốn đã tạo ra.
