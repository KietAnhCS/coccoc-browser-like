...
|---|---|---|
| Thiết kế thông điệp | 10/10 | Ba phương án được cân nhắc và **ghi lại lý do bác bỏ** — hiếm thấy trong đồ án |
| Bất biến | 10/10 | `record` + mọi trường là kiểu bất biến sẵn ⇒ không có lỗ hổng bất biến nông |
| Kiểm tra đầu vào | 10/10 | Kiểm đúng 3 trường then chốt, và **giải thích vì sao không kiểm 7 trường còn lại** |
| Phòng lỗi vận hành | 10/10 | `toString()` ghi đè + `withoutHtml()` chặn lỗi đổ 2,4 GB log |
| Nhận thức phân tán | 10/10 | `jobId` sinh ra từ một lỗi thật, và chú thích ghi lại nguyên nhân |
| Hiệu năng | 8/10 | 80 KB/thông điệp là cái giá có chủ ý; `htmlSizeBytes()` cấp phát cả mảng mỗi lần gọi |
| Khả năng kiểm thử | 8/10 | Có test bất biến; **thiếu test chống hồi quy `@JsonIgnore`** — đúng lỗi đã từng xảy ra ở lớp anh em |
| Tiến hoá lược đồ | 6/10 | Không có `schemaVersion`; thêm/bớt trường sẽ phá tương thích với thông điệp cũ còn trong topic |

**Bốn đề xuất nâng lên mức sản phẩm:**

1. **Thêm `int schemaVersion` (hoặc header Kafka `schema.version`).** Đây là
   khoảng trống lớn nhất. Topic Kafka giữ thông điệp nhiều ngày; khi triển khai
   phiên bản mới, consumer cũ và mới **cùng đọc một topic**. Không có số phiên
   bản thì cách duy nhất để thêm trường là dừng toàn hệ thống, xoá topic, khởi
   động lại — chấp nhận được cho đồ án, không chấp nhận được cho sản phẩm. Cách
   rẻ nhất: cấu hình Jackson `FAIL_ON_UNKNOWN_PROPERTIES=false` ở consumer để
   thêm trường trở thành thay đổi tương thích ngược.

2. **Nhớ đệm `htmlSizeBytes()`.** Hiện mỗi lần gọi là một lần cấp phát mảng 80
   KB rồi vứt. Với 31.030 trang, nếu Analytics gọi nó vài lần/trang thì đó là
   hàng GB rác cho GC dọn. Cách sửa: tính một lần trong compact constructor và
   lưu vào một trường `transient`… nhưng record không cho phép trường thêm, nên
   phương án thực tế là truyền sẵn `htmlSizeBytes` như một component thứ 11 —
   đánh đổi với vấn đề "trường dẫn xuất có thể lệch" ở mục 5.2. **Chỉ làm nếu
   đo được là nút thắt.**

3. **Cân nhắc `byte[] htmlCompressed` cho ca thông điệp lớn.** Với những trang
   gần trần 4 MB, nén ở tầng ứng dụng trước khi gửi sẽ kéo chúng xuống dưới
   trần thay vì bị vứt. Chỉ áp dụng có điều kiện (`if (size > 2 MB)`), giữ
   nguyên đường thường.

4. **Đưa `contentHash` vào header Kafka thay vì (hoặc cùng với) thân thông
   điệp.** Cho phép consumer khử trùng **mà không phải deserialize** cả 80 KB —
   một tối ưu thật ở phía nhận, nhất là khi một consumer đọc lại topic từ đầu.
