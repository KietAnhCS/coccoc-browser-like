...
|---|---|---|
| Cô lập lỗi | 10/10 | Try/catch mỗi handler, và Javadoc giải thích đó là **mô phỏng ngữ nghĩa Kafka**, không phải phòng thủ tuỳ tiện |
| Chọn cấu trúc dữ liệu | 10/10 | `CopyOnWriteArrayList` khớp chính xác mẫu đọc-nhiều/ghi-hiếm, có lập luận |
| Chất lượng log | 10/10 | Ghi `handlerName` + `url`; ba lớp phòng vệ chống đổ HTML vào log; truyền `Throwable` đúng quy ước SLF4J |
| Nhất quán hợp đồng | 9/10 | Không ném, đếm lỗi — đúng; nhưng hành vi với `null` không được ghi ở interface |
| Quan sát được | 8/10 | Bốn bộ đếm; **thiếu `outlinksPublished`**, và ba bộ đếm không có trong interface |
| API lắp ráp | 9/10 | Fluent `subscribeXxx` đọc như bản khai báo; nhưng `null` bị nuốt im lặng |
| Hiệu năng | 10/10 | 0,0002% thời gian phiên crawl — chi phí gián tiếp không đo được |
| Khả năng kiểm thử | 8/10 | Có test cô lập lỗi và bộ đếm; **thiếu test đa luồng** — mà đó là ngữ cảnh chạy thật |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Cơ chế dead-letter cho chế độ in-process.** Đây là khoảng trống lớn nhất
   (mục 2.3): handler ném ⇒ thông điệp **mất vĩnh viễn**, chỉ còn một dòng WARN.
   Một `Queue<FailedEvent>` giới hạn kích thước (ví dụ 1.000 phần tử gần nhất)
   kèm endpoint `/admin/failed-events` sẽ cho phép xem *cái gì* hỏng chứ không
   chỉ *bao nhiêu* cái hỏng — và làm hai chế độ gần nhau thêm một bậc.

2. **Ném khi `subscribeXxx(null)`.** Hiện `null` bị bỏ qua lặng lẽ. Nhưng khác
   với `publishXxx(null)` (xảy ra lúc chạy, không đáng giết phiên crawl),
   `subscribeXxx(null)` xảy ra **lúc khởi động** — nơi ném là hoàn toàn đúng và
   lỗi cấu hình cần lộ ra ngay. Hiện tại, một service đăng ký nhầm `null` sẽ im
   lặng không bao giờ chạy, và không có cách nào phát hiện ngoài việc nhận ra
   dữ liệu thiếu.

3. **Ghi ngữ nghĩa `null` vào Javadoc của interface.** Cả hai bản cài đều bỏ qua
   `publishXxx(null)`, nhưng hợp đồng không hứa điều đó. Một dòng
   `@param event bỏ qua nếu null` biến hành vi ngầm thành hành vi được bảo đảm.

4. **Đưa `getPagesPublishedCount()` lên interface.** Dashboard muốn vẽ tỷ lệ
   `failures / (published × handlerCount)` — công thức ở mục 5.1 — hiện phải
   `instanceof` để lấy hai trong ba thành phần. Đây đúng là vấn đề mà
   `getPublishFailureCount()` đã được đưa lên interface để tránh.

5. **Thêm `outlinksPublished`.** Ba trong bốn luồng có bộ đếm; luồng nuôi
   PageRank thì không. Nếu `ContentStorage` ngừng nhận outlink, hiện **không có
   số nào** cho thấy điều đó — và triệu chứng duy nhất sẽ là PageRank ra kết quả
   lạ sau khi crawl xong. Một dòng khai báo `AtomicLong` là đủ.
