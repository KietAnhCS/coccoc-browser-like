...
|---|---|---|
| Nhận diện vấn đề | 10/10 | Hai triệu chứng giao diện cụ thể, kèm bằng chứng đo được (logo 100×42 `alt="Fica"`) |
| Chất lượng quyết định | 10/10 | Một ràng buộc dữ liệu thay thế cả một thuật toán đa dạng hoá, **và** giảm 96,1% dung lượng |
| Lập luận đúng đắn | 10/10 | Chứng minh phép chọn dần cho kết quả **giống hệt** chọn toàn cục, dựa trên thứ tự toàn phần |
| Đúng đắn đa luồng | 10/10 | `compute` nguyên tử, kèm mô tả đầy đủ kịch bản đua nếu viết sai |
| Rút gọn mô hình | 10/10 | Nhận ra việc rút gọn làm **biến mất một lớp lỗi phân trang**, không chỉ tiết kiệm bộ nhớ |
| Thiết kế API | 9/10 | Giữ `List` để tương thích ngược, có lập luận; nhưng kiểu trả về nói dối một chút |
| Tận dụng chéo | 10/10 | `forPages` thừa hưởng xếp hạng miễn phí; `addAll` chuyển đổi định dạng cũ miễn phí |
| Quan sát được | 10/10 | `rejected == 0` được thiết kế làm **phép kiểm tính đúng chạy trong sản phẩm** |
| Trung thực về giới hạn | 10/10 | Nêu rõ cái mất (29/30 ảnh của trang thư viện) và chỉ đúng chỗ sửa nếu đổi hướng |
| Khả năng kiểm thử | 7/10 | Có test cơ bản; **thiếu test tính xác định và test đa luồng** — hai tính chất trung tâm |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Test tính xác định** (mã ở mục 6). Toàn bộ lập luận ở mục 2 dựa trên hai
   tính chất của `ImageQuality` (thứ tự toàn phần + `> 0`). Nếu ai đó đổi
   `isBetter` thành `>= 0`, lớp này **âm thầm** trở nên không xác định, và triệu
   chứng chỉ xuất hiện ở chế độ Kafka dưới tải. Một bài test hoán vị là cách duy
   nhất bắt được.

2. **Test `add` đa luồng.** Javadoc hứa thread-safe và đó là hứa hẹn đúng, nhưng
   không có gì kiểm chứng. Khối `compute` với mảng `boolean[1]` là loại mã dễ bị
   "đơn giản hoá" nhầm.

3. **Ghi chú ở `FeedController` rằng ngữ nghĩa đã đổi.** Chữ ký `forPage` giữ
   nguyên nên chỗ gọi không phải sửa — nhưng cũng vì thế, người đọc
   `FeedController` không biết ảnh giờ được **chọn** chứ không phải **lấy đầu
   DOM**. Một dòng chú thích ở chỗ gọi khép lại khoảng trống thông tin này. (Và
   xem thêm đề xuất 3 ở [`ImageQuality`](./ImageQuality.md) — `FeedController`
   có thể vẫn đang dùng đường cũ hoàn toàn.)

4. **Thêm `Gauge` cho `rejected` và `replaced`.** Phép kiểm tính đúng ở mục 4.7
   hiện chỉ đọc được qua `/api/admin/stats`, tức là **phải có người vào xem**.
   Đưa lên Prometheus cho phép đặt cảnh báo *"rejected == 0 sau 1.000 trang"* —
   biến một phép kiểm thủ công thành một phép kiểm tự động.

5. **Cân nhắc giữ top-N thay vì top-1 cho trang thư viện ảnh.** Cái mất ở mục
   1.3 là thật với một số ít trang. Một phương án trung dung: giữ 1 ảnh mặc
   định, nhưng cho phép tới 5 ảnh nếu trang có **nhiều ảnh cùng bậc
   `TIER_SIZED_CONTENT` và cùng cỡ lớn** — dấu hiệu của thư viện ảnh thật. Chi
   phí bộ nhớ tăng không đáng kể (số trang như vậy rất ít), và
   `ImageQuality.compare` đã có đủ thông tin để nhận diện. Chỉ nên làm nếu có
   nhu cầu sản phẩm rõ ràng — hiện quyết định top-1 là đúng cho mục tiêu.
