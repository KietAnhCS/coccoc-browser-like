...
|---|---|---|
| Nhận diện vấn đề | 10/10 | Mô tả đúng triệu chứng người dùng gặp ("hai tab bất đồng, không có lỗi để lần") |
| Lập luận tách tệp | 10/10 | Ba lý do khác loại nhau; lý do "vòng đời khác nhau" là lập luận kiến trúc thật |
| Thiết kế chống lỗi | 10/10 | `pathFor` làm trạng thái sai **không biểu diễn được** thay vì chỉ cảnh báo |
| Ghi nguyên tử | 10/10 | Tmp + rename, có nhánh dự phòng cho ổ mạng, và nêu rõ vì sao checkpoint làm nó bắt buộc |
| Phân biệt rỗng / không có | 10/10 | Ghi cả danh sách rỗng, kèm lý do cụ thể về hai lời khuyên khác nhau |
| Hai hợp đồng nạp | 9/10 | Tách `loadFromJson`/`loadQuietly` đúng; nhưng `loadQuietly` **không ghi log** |
| Ghi chép phụ thuộc ẩn | 10/10 | Chú thích `INDENT_OUTPUT` cứu được một phụ thuộc vô hình với script PowerShell |
| Khả năng kiểm thử | 7/10 | Có test cơ bản; thiếu test cho `INDENT_OUTPUT` và cho ghi nguyên tử |
| An toàn đa luồng | 6/10 | `new ArrayList<>(images)` giảm cửa sổ đua nhưng **không** là ảnh chụp nguyên tử; không được ghi chú |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **`loadQuietly` phải ghi log khi tệp hỏng.** Đây là khoảng trống thật: hiện
   "tệp không tồn tại" (bình thường) và "tệp hỏng" (nghiêm trọng) cho ra **cùng
   một kết quả im lặng**. Một dòng `log.warn` phân biệt hai ca, và nó thuộc về
   lớp này chứ không phải chỗ gọi — vì chỉ lớp này biết đã xảy ra ngoại lệ gì.
   Javadoc nói "người gọi vẫn ghi log", nhưng người gọi **không có** thông tin để
   ghi.

2. **Test chống hồi quy `INDENT_OUTPUT`** (mã ở mục 6). Phụ thuộc giữa một tuỳ
   chọn Jackson và một script PowerShell là loại phụ thuộc **không công cụ nào
   phát hiện được**. Chú thích bảo vệ được người đọc mã; test bảo vệ được cả
   người không đọc.

3. **Ghi chú rõ về ảnh chụp không nguyên tử.** `new ArrayList<>(images)` chống
   được `ConcurrentModificationException` nhưng **không** đảm bảo ảnh chụp nhất
   quán — một ảnh được thêm giữa chừng có thể vào hoặc không vào tệp. Với dữ
   liệu thống kê thì chấp nhận được, nhưng điều đó cần được **ghi ra** như ở
   [`CrawlAnalyticsService`](./CrawlAnalyticsService.md) mục 2.2 đã làm cho một
   cuộc đua khác. Cuộc đua có ghi chép là quyết định; không ghi chép là lỗi.

4. **Cân nhắc JSONL (một bản ghi mỗi dòng) thay vì mảng JSON.** Định dạng hiện
   tại buộc phải nạp **toàn bộ** mảng để đọc một phần tử, và buộc phải ghi lại
   **toàn bộ** tệp để thêm một ảnh. JSONL cho phép **ghi thêm** (append) — hợp
   với bản chất luồng của dữ liệu ảnh, và làm checkpoint gần như miễn phí. Nó
   cũng giải quyết luôn vấn đề `INDENT_OUTPUT` một cách sạch sẽ: mỗi bản ghi tự
   nhiên nằm trên một dòng.

5. **Thêm phiên bản lược đồ vào tệp.** Nếu `ImageFound` thêm một trường, mọi tệp
   cũ sẽ đọc không được (`UnrecognizedPropertyException` theo chiều ngược lại) và
   **toàn bộ ảnh đã lưu bị mất**. Cách rẻ nhất, cùng đề xuất với
   [`PageEvent`](../bus/PageEvent.md): bật
   `FAIL_ON_UNKNOWN_PROPERTIES=false` khi đọc, để thêm/bớt trường thành thay đổi
   tương thích.
