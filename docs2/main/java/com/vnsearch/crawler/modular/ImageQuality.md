...
|---|---|---|
| Phương pháp thực nghiệm | 10/10 | **Bác bỏ một giả định đã ghi trong tài liệu của chính dự án** bằng số liệu (19% vs kỳ vọng ~100%) |
| Trung thực về giới hạn | 10/10 | Nói rõ nó **không** đo chất lượng ảnh, và vì sao tên lớp có thể gây hiểu nhầm |
| Thiết kế thang điểm | 10/10 | Bậc thay vì cộng dồn, kèm phản ví dụ cụ thể ("icon 300px thắng ảnh bài 280px") |
| Xử lý dữ liệu thiếu | 10/10 | `TIER_UNKNOWN` trên `TIER_SMALL` — quyết định phản trực giác nhưng đúng, dựa trên 60,7% |
| Tính xác định | 10/10 | Quy ước `> 0` đảm bảo kết quả ổn định bất kể thứ tự thông điệp, **và** cài luôn tiêu chí phá hoà |
| Chọn ngưỡng | 10/10 | 200px suy từ số đo thật (logo 100×42, ảnh bài hẹp nhất 300–400) kèm biên an toàn |
| Hiệu năng | 9/10 | `Pattern` tĩnh; nhưng `tier`/`estimatedWidth` tính lặp cho ảnh hiện hành |
| Khả năng kiểm thử | 6/10 | Không có bộ test riêng; hai quyết định tinh tế nhất (UNKNOWN>SMALL, tính xác định) **không được máy nào canh** |
| Nhất quán toàn hệ thống | 5/10 | `FeedController` **vẫn** dùng ảnh đầu DOM — đúng cách làm đã bị chứng minh là sai 81% |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Bộ test riêng `ImageQualityTest`** (mã ở mục 7). Đây là lớp có **mật độ
   quyết định tinh tế cao nhất** trong cả gói `modular`, và hiện không có test
   nào của riêng nó. Hai bài quan trọng nhất: `UNKNOWN > SMALL` và tính xác
   định. Cả hai đều là những thứ mà một người "dọn dẹp" có thể phá mà không hiểu
   mình đang phá gì.

2. **Đệm `tier` và `estimatedWidth` cho ảnh hiện hành.** `ImageStore` gọi
   `isBetter(moi, hienHanh)` cho mỗi ảnh đến, và mỗi lần lại tính lại tier của
   `hienHanh`. Với trang 50 ảnh là 50 lần tính thừa. Cách sửa gọn: `ImageStore`
   giữ một bản ghi `(ImageFound, tier, width)` thay vì chỉ `ImageFound`. Chi
   phí hiện không đáng kể, nhưng nó tăng tuyến tính với số ảnh/trang.

3. **Thống nhất `FeedController` dùng `ImageQuality`.** Đây là đề xuất quan
   trọng nhất về mặt sản phẩm: hiện tab "Hình ảnh" dùng phép chọn đã kiểm chứng,
   còn feed dùng phép chọn đã **bị chứng minh sai 81%**. Người dùng thấy hai ảnh
   khác nhau cho cùng một bài, và ảnh ở feed thường là logo toà soạn. Sửa là một
   dòng: thay "ảnh đầu DOM" bằng `ImageStore.imageFor(url)`.

4. **Rà lại cờ `CASE_INSENSITIVE`.** `tier()` hạ chữ thường URL trước khi khớp,
   nhưng `estimatedWidth()` thì **không** (dòng 196). Nên `WIDTH_PARAM` và
   `SIZE_IN_PATH` phụ thuộc vào cờ, còn `DECORATIVE_*` thì không cần. Sự bất
   đối xứng này chưa gây lỗi nhưng rất dễ gây lỗi khi ai đó tái cấu trúc — nên
   hạ chữ thường **một lần** ở đầu cả hai hàm và bỏ cờ đi.

5. **Ghi lại số liệu đo vào một tệp có thể chạy lại.** Bảng ở mục 2 và con số
   19% ở mục 3 hiện chỉ tồn tại trong Javadoc. Nếu corpus đổi (crawl site khác,
   hoặc các site đổi cách nhúng ảnh), các con số đó lặng lẽ trở nên sai — và mọi
   ngưỡng dựa trên chúng cũng vậy. Một script trong `eval/` chạy lại phép đo sẽ
   biến những con số này từ **ghi chép lịch sử** thành **số liệu kiểm chứng
   được**. Đây đúng tinh thần mà `getAverageOutlinksPerPage()` ở
   [`UrlExtractorService`](./UrlExtractorService.md) đã làm cho một con số khác.
