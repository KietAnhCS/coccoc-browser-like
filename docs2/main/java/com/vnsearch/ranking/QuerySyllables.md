...
|---|---|---|
| **Ghi lại nguyên nhân gốc, không chỉ triệu chứng** | 10/10 | "Bỏ dấu là ánh xạ nhiều-một, so khớp trên ảnh thì mất khả năng phân biệt nghịch ảnh" — phát biểu chính xác về mặt toán học |
| **Phân biệt hai khâu có yêu cầu ngược nhau** | 10/10 | Bỏ dấu **đúng** ở tra cứu, **sai** ở hiển thị — hiểu biết chỉ có sau khi gặp lỗi thật |
| Kỹ thuật điểm bất động | 10/10 | Phép kiểm định nghĩa **bằng** phép biến đổi ⇒ không thể trôi lệch |
| Chống nhồi từ khoá | 9/10 | Kẹp `[0,1]` và nêu rõ ví dụ cụ thể "4/2 = 2" |
| `Pattern` biên dịch sẵn | 9/10 | Có định lượng "hàng chục nghìn lần mỗi truy vấn" |
| Lớp ký tự Unicode | 9/10 | `\p{L}\p{N}` thay `[a-zA-Z0-9]` — chọn sai sẽ phá huỷ chính mục đích của lớp |
| Ngắn mạch đúng thứ tự | 9/10 | `exact` trước (rẻ), `loose` sau (đắt), `!loose.isEmpty()` cắt sạch trường hợp phổ biến |
| **Kiểm thử** | **0/10** | **Không một test nào** — kể cả cho lỗi mà file này sinh ra để sửa |
| Bất biến `loose ⊆ exact` | 4/10 | Đúng theo `from()`, nhưng hàm dựng record công khai không cưỡng chế |
| Bảo vệ tính bất biến | 4/10 | Hai `Set` không được sao chép phòng vệ; người gọi sửa được từ bên ngoài |
| Mẫu số của tỉ lệ | 5/10 | Kẹp `min(1,·)` che triệu chứng; mẫu số vẫn không cùng đơn vị với tử số |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết test hồi quy cho đúng lỗi đã sửa — việc cấp thiết nhất.** Cả file này
   tồn tại vì một lỗi cụ thể, và lỗi đó hiện **không có gì chặn nó quay lại**.
   Một lớp test 30 dòng khoá lại toàn bộ bảng quyết định ở mục 3:
   ```java
   class QuerySyllablesTest {

       @Test
       @DisplayName("LỖI ĐÃ SỬA: truy vấn CÓ dấu không được khớp tiếng khác dấu")
       void tiengCoDauChiKhopChinhXac() {
           QuerySyllables s = QuerySyllables.from(Set.of("ngân_hàng"));
           assertTrue(s.matches("ngân"));
           assertFalse(s.matches("ngàn"), "«ngân» KHÔNG được khớp «ngàn»");
           assertFalse(s.matches("ngắn"));
       }

       @Test
       @DisplayName("Truy vấn KHÔNG dấu thì khớp lỏng — cố ý")
       void tiengKhongDauKhopLong() {
           QuerySyllables s = QuerySyllables.from(Set.of("ngan"));
           assertTrue(s.matches("ngan"));
           assertTrue(s.matches("ngân"));
           assertTrue(s.matches("ngàn"));
       }

       @Test
       void tieuDeNhoiTuKhoaBiKepO1() {
           QuerySyllables s = QuerySyllables.from(Set.of("máy_tính"));
           assertEquals(1.0, s.titleMatchRatio("Máy tính và máy tính bảng"), 1e-9);
       }

       @Test
       void stripPunctuationGiuDauTiengViet() {
           assertEquals("ngân", QuerySyllables.stripPunctuation("ngân,"));
       }
   }
   ```

2. **Sửa mẫu số thay vì kẹp kết quả.** `Math.min(1.0, …)` chặn được thao túng
   nhưng làm mất thông tin: tiêu đề khớp 2/2 tiếng và tiêu đề nhồi 10 lần đều cho
   đúng `1,0`. Đếm **tiếng phân biệt đã khớp** cho tỉ số tự nhiên trong `[0,1]`,
   không cần kẹp, và phân biệt được "khớp đủ" với "khớp một phần":
   ```java
   public double titleMatchRatio(String title) {
       if (title == null || title.isBlank() || exact.isEmpty()) return 0.0;
       Set<String> daKhop = new HashSet<>();
       for (String word : WHITESPACE_RUN.split(title.toLowerCase(Locale.ROOT))) {
           String sach = stripPunctuation(word);
           if (matches(sach)) daKhop.add(chuanHoaVeTiengTruyVan(sach));
       }
       return (double) daKhop.size() / exact.size();   // tu nhien thuoc [0,1]
   }
   ```
   Rồi chạy lại đánh giá: nếu MRR không đổi, giữ bản đơn giản hơn; nếu đổi, đó là
   một dòng nữa cho bảng ablation.

3. **Đóng hàm dựng record và sao chép phòng vệ hai tập.** `record` sinh hàm dựng
   công khai không kiểm tra gì, nên bất biến `loose ⊆ exact` — thứ mà `isEmpty()`
   ngầm dựa vào — có thể bị phá từ bên ngoài, và hai `Set` truyền vào vẫn sửa
   được sau khi đối tượng đã tạo. Một hàm dựng gọn khép cả hai lỗ:
   ```java
   public QuerySyllables {
       exact = Set.copyOf(exact);
       loose = Set.copyOf(loose);
       if (!exact.containsAll(loose)) {
           throw new IllegalArgumentException("loose phai la tap con cua exact");
       }
   }
   ```
   `Set.copyOf` vừa sao chép phòng vệ vừa cho tập bất biến thật, và phép kiểm
   biến một giả định ngầm thành một ràng buộc được cưỡng chế.
