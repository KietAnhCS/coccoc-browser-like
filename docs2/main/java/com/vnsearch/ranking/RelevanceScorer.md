...
|---|---|---|
| **Động cơ của pattern** | 10/10 | Nêu rõ Strategy tồn tại để làm **ablation**, và dẫn số đo chứng minh nó đã được dùng thật |
| **Tối ưu không phá vỡ tương thích** | 10/10 | `prepare` là `default` ⇒ mọi cài đặt cũ chạy nguyên, ai muốn thì ghi đè |
| **Quên tối ưu ⇒ chậm, không sai** | 10/10 | Tính chất an toàn quý nhất của thiết kế này |
| Chẩn đoán chi phí | 10/10 | Chỉ đích danh ba thủ phạm và lượng hoá "30.000 phép logarit, 5.000 đối tập băm" |
| `name()` tự ghép | 10/10 | Nhãn sinh ra **từ mã** ⇒ báo cáo không thể lệch với cấu hình thật |
| Tài liệu hoá | 9/10 | Javadoc giải thích *vì sao*, không chỉ *cái gì* — mức hiếm |
| `@FunctionalInterface` | 9/10 | Cưỡng chế bằng trình biên dịch, không bằng quy ước |
| **Kiểm thử hợp đồng giao diện** | **2/10** | Bất biến `prepare ≡ score` **không có test nào** — mà đó là bất biến dễ vỡ nhất |
| Ràng buộc "một `DocumentScorer` cho một truy vấn" | 4/10 | Chỉ nằm trong Javadoc; không có gì ngăn dùng lại sai truy vấn |
| Kiểm định thống kê của số đo | 5/10 | ΔMRR = 0,0452 dẫn ra như sự thật, không kèm khoảng tin cậy hay p-value |
| Ổn định của `name()` như khoá dữ liệu | 5/10 | Là khoá đối chiếu giữa các lần chạy nhưng không có ràng buộc định dạng |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết một test hợp đồng dùng chung cho MỌI cài đặt.** Bất biến
   `prepare(qtf, index).score(d) == score(qtf, d, index)` là điều kiện sống còn:
   vi phạm nó nghĩa là hệ thống sản phẩm (dùng `prepare`) và bộ đánh giá cho hai
   kết quả khác nhau — đúng loại lỗi mà cả kiến trúc này được dựng để tránh. Một
   lớp test trừu tượng ép mọi scorer mới phải qua cửa này:
   ```java
   abstract class RelevanceScorerContractTest {
       protected abstract RelevanceScorer scorer();

       @Test
       void prepareChoKetQuaGiongHetScore() {
           Map<String, Integer> qtf = Map.of("máy", 1, "tính", 2);
           var daChuanBi = scorer().prepare(qtf, index);
           for (int d = 0; d < index.getDocumentCount(); d++) {
               assertEquals(scorer().score(qtf, d, index), daChuanBi.score(d), 1e-9,
                       "prepare va score PHAI cho cung mot diem cho docId=" + d);
           }
       }
   }
   ```
   Rồi `class BM25ScorerTest extends RelevanceScorerContractTest` — mỗi cài đặt
   mới tự động được canh giữ, kể cả các Decorator.

2. **Cho `DocumentScorer` mang theo danh tính truy vấn đã chuẩn bị.** Cạm bẫy ①
   (dùng lại `DocumentScorer` cho truy vấn khác) hiện chỉ được ngăn bằng lời văn,
   và hậu quả là điểm sai im lặng. Rẻ nhất là để `prepare` trả về một record có
   kèm nhãn, và `ResultRanker` khẳng định trước khi dùng:
   ```java
   record PreparedScorer(String queryKey, DocumentScorer scorer) { }
   // trong ResultRanker
   assert prepared.queryKey().equals(khoaCua(queryTermFrequency))
           : "DocumentScorer duoc chuan bi cho mot truy van KHAC";
   ```

3. **Gắn kiểm định ý nghĩa vào chính bảng số trong Javadoc.** Dự án **đã có**
   [`../eval/SignificanceTest.md`](../eval/SignificanceTest.md) nhưng bảng
   TF-IDF/BM25 ở đây không dùng tới nó. Với $n = 200$ truy vấn, chênh lệch MRR
   0,0452 rất có thể có ý nghĩa — nhưng "rất có thể" không phải là kết luận khoa
   học. Một dòng thêm vào Javadoc biến bảng số thành bằng chứng:
   ```
   TF-IDF thuan : MRR 0,8537   Success@1 78,0%
   BM25 thuan   : MRR 0,8989   Success@1 85,0%
   Kiem dinh hoan vi ghep cap, 10.000 lan, p = 0,003  ⇒ khac biet co y nghia
   ```
   Và nên nêu rõ cả **cấu hình** đã dùng (`k1`, `b`) — chính `name()` đã sinh sẵn
   nhãn đó, chỉ việc chép vào.
