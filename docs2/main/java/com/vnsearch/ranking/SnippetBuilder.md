...
|---|---|---|
| **Ghi lại lỗ hổng bảo mật thật** | 10/10 | Mô tả đúng chuỗi sự kiện, và nói rõ **`ContentParser` không sai** — chỗ tinh vi nhất |
| **Phát biểu bất biến vòng lặp** | 10/10 | "`currentMatches` luôn bằng số từ khớp trong `[start, start+w)`" — chứng minh tính đúng đắn tại chỗ |
| Bảo vệ bất biến trái trực giác | 10/10 | Bình luận "đừng bỏ dấu ở đây" chặn đúng chỗ một lần dọn mã sẽ phá |
| Tách trách nhiệm | 10/10 | Điểm số vs văn bản — hai lý do thay đổi khác nhau |
| Thứ tự thoát ký tự | 9/10 | `&` trước tiên — sai thứ tự cho lỗi thoát hai lần |
| Tính trước `isMatch` | 9/10 | Một mảng `boolean` dùng cho cả tìm cửa sổ lẫn bôi sáng |
| Chi tiết trải nghiệm | 9/10 | "..." chỉ thêm khi thực sự bị cắt — nhỏ nhưng ảnh hưởng lòng tin |
| Lá chắn tài liệu ngắn | 8/10 | `Math.min(windowSize, words.length)` |
| **Kiểm thử** | **0/10** | **Không một test nào** — kể cả cho lỗ hổng XSS đã sửa |
| **Phạm vi an toàn của `escapeHtml`** | **4/10** | Không thoát `'`; an toàn cho **một** ngữ cảnh mà ngữ cảnh đó không được ghi |
| Ràng buộc với client | 4/10 | Trả HTML ⇒ client **phải** dùng `innerHTML`; hợp đồng hai chiều không ghi ở đâu |
| Tối ưu đúng điểm nóng | 5/10 | Cửa sổ trượt chiếm 2 % thời gian; 90 % nằm ở `stripPunctuation` + `matches` không được nhắc |
| Ghi lại quyết định `>` vs `>=` | 5/10 | Chọn cửa sổ đầu tiên khi hoà — có chủ đích nhưng không nói ra |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết test cho lỗ hổng XSS — đây là việc cấp thiết nhất trong cả gói.** Javadoc
   mô tả một lỗ hổng bảo mật **thật sự đã tồn tại**, và hiện không có gì ngăn nó
   quay lại. Nguy hiểm hơn nữa, lập luận sai từng dẫn tới lỗ hổng ("`ContentParser`
   đã lọc `<script>` rồi") vẫn nghe rất hợp lý với người đọc mã lần đầu — nên khả
   năng ai đó bỏ `escapeHtml` đi trong một lần dọn dẹp là có thật:
   ```java
   class SnippetBuilderTest {

       @Test
       @DisplayName("XSS: văn bản chứa thẻ script PHẢI được thoát")
       void vanBanChuaTheScriptDuocThoat() {
           String than = "Ví dụ payload: <script>alert(1)</script> rất nguy hiểm";
           String snippet = new SnippetBuilder().build(than, QuerySyllables.from(Set.of("payload")));

           assertFalse(snippet.contains("<script>"), "Thẻ <script> KHÔNG được lọt nguyên vẹn");
           assertTrue(snippet.contains("&lt;script&gt;"));
           assertTrue(snippet.contains("<mark>"), "Thẻ <mark> của CHÍNH hệ thống vẫn phải còn");
       }

       @Test
       void khongThoatHaiLan() {
           String s = new SnippetBuilder().build("a & b", QuerySyllables.from(Set.of("a")));
           assertTrue(s.contains("&amp;"));
           assertFalse(s.contains("&amp;amp;"), "Ký tự & phải được thoát ĐÚNG MỘT LẦN");
       }
   }
   ```
   Ca thứ nhất khoá lại đúng ranh giới tinh tế: thẻ của **dữ liệu** bị thoát, thẻ
   của **hệ thống** thì không. Bổ sung tiếp bốn ca cho các tính chất ②–⑥ ở mục 8.

2. **Ghi rõ hợp đồng an toàn của chuỗi trả về.** `build()` trả một chuỗi **HTML**,
   kéo theo hai ràng buộc mà hiện không nằm ở đâu: client **phải** render bằng
   `innerHTML` (dùng `textContent` sẽ hiện chữ `<mark>`), và chuỗi chỉ an toàn
   trong **nội dung phần tử**, không an toàn trong thuộc tính vì `'` không được
   thoát. Cả hai thuộc về Javadoc của `build`:
   ```java
   /**
    * @return HTML DA THOAT ky tu, chua the {@code <mark>} cua he thong.
    *         <p><b>Ngu canh su dung an toan:</b> chi dat trong NOI DUNG phan tu
    *         ({@code element.innerHTML = snippet}). KHONG an toan khi dat trong
    *         gia tri thuoc tinh HTML — dau nhay don khong duoc thoat.
    */
   ```
   Nếu snippet có khả năng dùng trong thuộc tính, thêm `.replace("'", "&#39;")` —
   một dòng, và hàm an toàn cho mọi ngữ cảnh.

3. **Đo lại rồi tối ưu đúng chỗ.** Cửa sổ trượt là phần được giải thích kỹ nhất
   nhưng chỉ chiếm 2 % thời gian; 90 % nằm ở `stripPunctuation` + `matches` khi
   dựng `isMatch`. Hai phép cắt đơn giản đánh đúng phần đó:
   ```java
   // (a) Bo qua stripPunctuation khi tu KHONG co dau cau — truong hop pho bien
   String sach = coDauCau(w) ? QuerySyllables.stripPunctuation(w) : w;

   // (b) Chi dung snippet cho ket qua THUC SU TRA VE (top-K), khong phai moi ung vien
   ```
   Phép (b) quan trọng hơn hẳn: nó là ràng buộc kiến trúc chứ không phải vi tối
   ưu. Hiện `ResultRanker` gọi đúng như vậy, nhưng không có gì ghi lại — và gọi
   `build()` cho 5.000 ứng viên thay vì 10 kết quả sẽ làm thời gian truy vấn tăng
   500 lần mà không ai lường trước.
