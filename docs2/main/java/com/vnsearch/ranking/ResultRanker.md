...
|---|---|---|
| **Ghi lại lỗi "ba tên một đại lượng"** | 10/10 | Nêu rõ hậu quả cụ thể: `EvaluationRunner` **đã thật sự nhầm** — không phải rủi ro giả định |
| **Đổi thứ tự giai đoạn, không vi tối ưu** | 10/10 | "Làm việc đắt càng muộn càng tốt" — quyết định kiến trúc, đo được 20 lần |
| **Dùng kiểu dữ liệu cưỡng chế thứ tự** | 10/10 | `ScoredCandidate` biểu diễn trạng thái "đã chấm điểm, chưa snippet" ⇒ không thể nhầm |
| Bình luận giải thích vị trí mã | 10/10 | Ba câu về `index.getBodyText` nói cả *vì sao*, *chi phí*, và *vì sao đặt ở đây* |
| Tách trách nhiệm sửa lỗi thật | 10/10 | Dọn kiến trúc làm lộ ra lỗi PageRank 0,1 % — dẫn chứng rằng refactor có giá trị đo được |
| Chọn cấu trúc dữ liệu | 9/10 | `MinHeap.topK` $O(c \log K)$, bộ nhớ $O(K)$ |
| Phân biệt vai trò của `pageRankScore` | 9/10 | "Chỉ để **báo cáo**; việc **dùng** nó thuộc về `PageRankBoostScorer`" |
| Test hồi quy cho lỗi bôi sáng | 8/10 | Hai ca canh giữ đúng lỗi cũ — nhưng ở tầng tích hợp, không phải tầng đơn vị |
| **Kiểm thử tối ưu chính** | **2/10** | Tối ưu hai giai đoạn là lý do tồn tại của lớp, **không có test** ⇒ refactor có thể xoá nó mà mọi test vẫn xanh |
| Hai mặc định khác nhau cho PageRank | 4/10 | `getOrDefault(d, 0.0)` ở đây vs `minPageRank` trong Decorator |
| `getDocument` cho mọi ứng viên | 5/10 | Với kho tài liệu ngoài bộ nhớ, đây là $c$ lượt đi mạng ở **giai đoạn 1** |
| Kiểm tra đối số | 4/10 | `topN`, `candidateDocIds` không được kiểm; `document == null` bỏ qua im lặng |
| `main` trong lớp sản phẩm | 4/10 | Dùng tên lớp đầy đủ ở bốn chỗ thay vì `import` |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết test khoá lại tối ưu hai giai đoạn — nó là lý do tồn tại của lớp và
   hiện không có gì bảo vệ.** Một lần refactor "gộp cho gọn" đưa
   `snippetBuilder.build()` lên vòng chấm điểm sẽ làm truy vấn chậm 20 lần, và
   **mọi test hiện có vẫn xanh** vì kết quả trả về hoàn toàn giống nhau. Đếm số
   lần gọi là cách duy nhất phát hiện:
   ```java
   @Test
   @DisplayName("Snippet CHỈ được sinh cho top-K, không phải mọi ứng viên")
   void snippetChiSinhChoTopK() {
       AtomicInteger soLanGoi = new AtomicInteger();
       SnippetBuilder dem = new SnippetBuilder() {
           @Override public String build(String body, QuerySyllables s) {
               soLanGoi.incrementAndGet();
               return super.build(body, s);
           }
       };
       new ResultRanker(dem).rank(idsCua(500), qtf, index, scorer, prMap, 10);

       assertEquals(10, soLanGoi.get(),
               "Sinh snippet cho moi ung vien lam truy van cham 20 lan");
   }
   ```
   (`SnippetBuilder` hiện là `final` — bỏ `final` hoặc rút một giao diện nhỏ là
   thay đổi nhỏ, và nó mở đường cho cả việc thay chiến lược snippet mà Javadoc của
   [`SnippetBuilder`](./SnippetBuilder.md) đã hứa hẹn.)

2. **Thống nhất mặc định PageRank, và tốt hơn là bỏ hẳn tham số trùng.** Hiện
   `pageRankScores` được truyền vào **hai** đường — một lần vào scorer để xếp
   hạng, một lần vào `rank()` để báo cáo — và hai đường dùng **hai mặc định khác
   nhau** (`0.0` ở đây, `minPageRank` trong Decorator). Không có gì ngăn người gọi
   truyền hai `Map` khác nhau, khiến con số hiển thị cho người dùng không phải con
   số đã dùng để xếp hạng. Cách sạch nhất là để chính chuỗi Decorator công bố giá
   trị nó đã dùng:
   ```java
   // RelevanceScorer bo sung mot phuong thuc mac dinh
   default Map<String, Double> tinHieuDaDung(int docId) { return Map.of(); }
   // ResultRanker doc thang tu scorer, khong nhan pageRankScores nua
   ```
   Cách này cũng mở rộng tự nhiên khi thêm tín hiệu mới: giao diện không phải thêm
   một tham số cho mỗi tín hiệu.

3. **Đưa `getDocument` ra khỏi giai đoạn 1.** Giai đoạn 1 gọi `index.getDocument`
   cho cả 500 ứng viên, nhưng chỉ **10** trong số đó cần đối tượng tài liệu — đúng
   loại lãng phí mà tối ưu hai giai đoạn đã cắt ở khâu snippet, chỉ là ở một tầng
   khác và chưa được nhận ra. Với
   [`PostgresDocumentStore`](../storage/PostgresDocumentStore.md), đó là 500 lượt
   đi mạng cho một truy vấn:
   ```java
   // GD 1: chi giu docId + diem, KHONG cham kho tai lieu
   record ScoredId(int docId, double finalScore) { }
   // GD 3: lay tai lieu CHI cho top-K
   for (ScoredId s : top) {
       WebDocument doc = index.getDocument(s.docId());
       if (doc == null) continue;
       ...
   }
   ```
   Lưu ý một hệ quả cần cân nhắc: lọc `document == null` sẽ chuyển xuống sau
   top-K, nên kết quả có thể trả về **ít hơn** `topN` phần tử khi kho tài liệu
   lệch chỉ mục. Đó lại là điều tốt — nó biến một lỗi đang **im lặng** thành một
