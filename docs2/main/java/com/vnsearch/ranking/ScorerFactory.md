...
|---|---|---|
| **Nhận diện "nợ kỹ thuật vô hình"** | 10/10 | "Strategy chỉ được bộ đánh giá khai thác; sản phẩm thì không" — chỉ ra khoảng cách giữa điều đã chứng minh và điều đang chạy |
| **Cấu hình thay vì biên dịch lại** | 10/10 | Năm dòng properties = năm hàng bảng ablation, không sửa mã |
| Từ chối cấu hình lạ | 10/10 | `default -> throw` với thông báo nêu rõ giá trị hợp lệ và giá trị nhận được |
| Không phụ thuộc khung | 10/10 | Hàm dựng tường minh cho phép dùng ngoài Spring — lõi không dính Spring |
| Bỏ tầng khi tín hiệu tắt | 9/10 | Ba điều kiện cho PageRank, kể cả `isEmpty()` khép lỗ hổng "tín hiệu tắt im lặng" |
| Chuẩn hoá đầu vào cấu hình | 9/10 | `null`, `trim`, `toLowerCase(Locale.ROOT)`, chấp nhận `tf-idf` |
| Nhãn phản ánh đúng cái đang chạy | 9/10 | Không lắp tầng ⇒ không có trong `name()` ⇒ bảng kết quả trung thực |
| **Độ phủ kiểm thử** | **3/10** | Bảy nhánh quyết định, hai ca test — ở lớp quyết định người dùng nhận mô hình nào |
| **Thời điểm phát hiện cấu hình sai** | **4/10** | `createBase()` chạy ở truy vấn đầu tiên, không phải lúc khởi động ⇒ HTTP 500 thay vì từ chối khởi động |
| Trường `final` | 5/10 | `@Value` trên trường ⇒ không `final` ⇒ an toàn nhờ quy ước, không nhờ kiểu |
| Cảnh báo cấu hình bị bỏ qua | 4/10 | `k1`, `b` được đọc kể cả khi `scorer=tfidf`, bỏ qua im lặng |
| Ràng buộc vòng đời | 4/10 | `create()` là $O(N)$; gọi mỗi truy vấn tốn 100 µs vô ích, không có gì cảnh báo |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Công bố scorer thành một bean singleton thay vì gọi `create()` tuỳ nơi.**
   `create()` là $O(N)$ vì `PageRankBoostScorer` duyệt toàn bộ Map ở hàm dựng, và
   không có gì trong lớp nói cho người gọi biết điều đó. Biến nó thành bean giải
   quyết cả chi phí lẫn sự mơ hồ về vòng đời:
   ```java
   @Bean
   public RelevanceScorer relevanceScorer(ScorerFactory factory, PageRankService pageRank) {
       RelevanceScorer scorer = factory.create(pageRank.getScores());
       log.info("Mo hinh xep hang dang phuc vu: {}", scorer.name());
       return scorer;
   }
   ```
   Dòng `log.info` là phần đáng giá không kém: nó in **đúng cấu hình đang chạy**
   vào nhật ký khởi động, nên câu hỏi "hệ thống đang chấm điểm bằng gì" trả lời
   được mà không cần đọc mã hay đoán từ file properties.

2. **Kiểm tra cấu hình lúc khởi động, không đợi truy vấn đầu tiên.** Hiện
   `app.ranking.scorer=bm52` để ứng dụng khởi động bình thường rồi trả HTTP 500
   cho người dùng đầu tiên — tệ hơn hẳn việc từ chối khởi động. Một
   `@PostConstruct` ba dòng chuyển lỗi về đúng thời điểm, và nhân tiện bắt luôn
   các cấu hình bị bỏ qua im lặng:
   ```java
   @PostConstruct
   void kiemTraCauHinh() {
       createBase();                        // nem ngay neu scorerType la
       if (!"bm25".equals(scorerType.trim().toLowerCase(Locale.ROOT))
               && (k1 != BM25Scorer.DEFAULT_K1 || b != BM25Scorer.DEFAULT_B)) {
           log.warn("app.ranking.bm25.k1/b duoc dat nhung scorer={} nen chung bi BO QUA.",
                    scorerType);
       }
   }
   ```

3. **Phủ bảy nhánh quyết định bằng một test tham số hoá.** Lớp này quyết định
   người dùng thật nhận mô hình nào — một lỗi ở đây làm **mọi con số trong báo
   cáo mất giá trị**, đúng loại rủi ro mà
   [`../query/CandidateResolver.md`](../query/CandidateResolver.md) mục 2 mô tả.
   Bảy nhánh gói gọn trong một bảng:
   ```java
   @ParameterizedTest(name = "{0}, beta={1}, gamma={2} ⇒ {3}")
   @CsvSource({
       "bm25,    0.30, 0.10, 'BM25(k1=1.2,b=0.75) + PR x0.30 + title x0.10'",
       "bm25,    0.00, 0.10, 'BM25(k1=1.2,b=0.75) + title x0.10'",
       "bm25,    0.30, 0.00, 'BM25(k1=1.2,b=0.75) + PR x0.30'",
       "bm25,    0.00, 0.00, 'BM25(k1=1.2,b=0.75)'",
       "tfidf,   0.00, 0.00, 'TF-IDF cosine'",
       "tf-idf,  0.00, 0.00, 'TF-IDF cosine'",
       "'  BM25 ',0.00, 0.00, 'BM25(k1=1.2,b=0.75)'",
   })
   void lapDungChuoiTheoCauHinh(String loai, double beta, double gamma, String nhanMongDoi) {
       var f = new ScorerFactory(loai, 1.2, 0.75, beta, gamma);
       assertEquals(nhanMongDoi, f.create(diemPageRankMau()).name());
   }
   ```
   Thêm hai ca cho `pageRankScores` `null` và rỗng — cả hai phải cho nhãn **không
   có** `+ PR`, vì đó là cơ chế duy nhất hiện có để phát hiện PageRank chưa chạy.
