...
|---|---|---|
| **Đúng công thức chuẩn** | 10/10 | Khớp Okapi BM25 nguyên bản, kể cả dạng IDF Robertson–Sparck Jones |
| **Giải thích *vì sao* từng thành phần** | 10/10 | Ba điểm hơn TF-IDF được nêu rõ, mỗi điểm có cơ chế toán học đi kèm |
| **Test canh giữ đúng lời tuyên bố** | 10/10 | Mỗi tính chất Javadoc nêu có một test tương ứng — tài liệu không trôi lệch được |
| Chống NaN lan truyền | 10/10 | Hai lá chắn ở `prepare` + một ở `idf` — NaN trong sort là lỗi thảm hoạ |
| Tối ưu `prepare` | 10/10 | Bốn phép tối ưu độc lập, mỗi phép có lý do rõ ràng, kết quả không đổi |
| Chẩn đoán nguyên nhân gốc | 10/10 | Bình luận về `recomputeDerivedState` chỉ đúng kịch bản sinh ra `avgdl = 0` |
| Kiểm tra tham số | 9/10 | Ném ở hàm dựng, thông báo có kèm giá trị nhận được |
| `name()` đủ tham số | 9/10 | `BM25(k1=1.2,b=0.75)` — tái lập được thí nghiệm từ nhãn |
| **Hạ cánh mềm im lặng** | **4/10** | Chỉ mục hỏng ⇒ mọi điểm 0 ⇒ kết quả rác, **không log, không cảnh báo** |
| Kiểm thử hàm dựng | 3/10 | Hai lệnh `throw` không có ca nào phủ |
| Ràng buộc "chỉ so sánh trong cùng truy vấn" | 5/10 | Đúng nhưng chỉ ngầm hiểu; điểm thô lọt ra API công khai |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Đưa các con số so sánh vào chính lớp này, kèm cấu hình sinh ra chúng.**
   [`RelevanceScorer`](./RelevanceScorer.md) giữ bảng TF-IDF/BM25, nhưng bảng đó
   không nói `k1`, `b` bằng bao nhiêu — mà đúng hai tham số đó quyết định con số.
   Vì `name()` đã sinh sẵn nhãn, chỉ việc chép nó vào Javadoc và cập nhật khi
   chạy lại:
   ```
   * <p>Ket qua tren 200 truy van known-item:
   * <pre>
   *   BM25(k1=1.2,b=0.75) : MRR 0,8989   Success@1 85,0%
   *   BM25(k1=1.2,b=0.00) : MRR 0,8612   Success@1 80,5%   ← tat chuan hoa do dai
   *   BM25(k1=0.0,b=0.75) : MRR 0,8104   Success@1 74,0%   ← bo bao hoa tan suat
   * </pre>
   ```
   Ba dòng đó biến hai tham số từ "con số truyền thống" thành **kết quả đo được
   trên chính corpus này** — và đó là phần một hội đồng chấm đồ án tốt nghiệp sẽ
   hỏi đầu tiên.

2. **Đừng hạ cánh mềm trong im lặng.** Khi `avgDocLength <= 0`, hệ thống trả điểm
   0 cho mọi tài liệu và **trông như đang chạy bình thường** — người dùng nhận
   kết quả xếp theo thứ tự tuỳ ý mà không ai biết. Vì đây là dấu hiệu của một
   chỉ mục nạp thiếu bước, nó xứng đáng một cảnh báo, và cảnh báo phải nói **cách
   sửa**:
   ```java
   if (totalDocs == 0 || avgDocLength <= 0) {
       log.warn("BM25 khong cham diem duoc: totalDocs={}, avgDocLength={}. "
              + "Chi muc co the vua nap tu file ma chua goi recomputeDerivedState().",
                totalDocs, avgDocLength);
       return docId -> 0.0;
   }
   ```
   Tốt hơn nữa: để [`IndexPersistence`](../index/IndexPersistence.md) **luôn** gọi
   `recomputeDerivedState` sau khi nạp, để trường hợp này không thể xảy ra.

3. **Phủ hai lệnh `throw` và bất biến `prepare ≡ score`.** Hàm dựng có hai phép
   kiểm mà không ca test nào chạm tới — nghĩa là nếu ai đó xoá nhầm một dòng
   `if`, mọi test vẫn xanh. Ba dòng test khép kín lỗ hổng đó, và ca thứ ba là ca
   quan trọng nhất của cả lớp:
   ```java
   @Test
   void tuChoiThamSoNgoaiMien() {
       assertThrows(IllegalArgumentException.class, () -> new BM25Scorer(-0.1, 0.75));
       assertThrows(IllegalArgumentException.class, () -> new BM25Scorer(1.2, 1.5));
       assertThrows(IllegalArgumentException.class, () -> new BM25Scorer(1.2, -0.01));
   }

   @Test
   void prepareVaScoreChoCungKetQua() {
       Map<String, Integer> qtf = Map.of("máy", 1, "tính", 2, "khongtontai", 1);
       BM25Scorer s = new BM25Scorer();
       var daChuanBi = s.prepare(qtf, index);
       for (int d = 0; d < index.getTotalDocs(); d++) {
           assertEquals(s.score(qtf, d, index), daChuanBi.score(d), 1e-12);
       }
   }
   ```
   Ca thứ hai đặc biệt đáng giá ở đây vì `prepare` **loại bỏ** term có `df = 0`
   còn `score` thì không — hai đường đi khác nhau về mã, phải chứng minh chúng
   bằng nhau về số.
