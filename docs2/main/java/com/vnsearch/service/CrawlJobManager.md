...
|---|---|---|
| Nhận diện lỗ rò tài nguyên | 10/10 | Bốn lỗ rò khác loại, kèm tính toán cụ thể (367 MB × N) và nhận ra nó là **lỗ hổng DoS** |
| Chọn chính sách pool | 10/10 | Nhận ra `newFixedThreadPool` chỉ **đổi chỗ** tích luỹ; `AbortPolicy` cho phản hồi trung thực |
| Vòng đời đối tượng nặng | 10/10 | Chụp số liệu rồi buông; getter đọc kép đúng khuôn chống TOCTOU |
| Xử lý bean tuỳ chọn | 10/10 | `ObjectProvider` + không có nhánh `if` — quyết định đẩy xuống một tầng duy nhất |
| Khả năng quan sát | 9/10 | `unroutableEvents` được thiết kế để lộ một kiểu hỏng im lặng; nhưng gộp 3 nguyên nhân |
| Ghi chép lý do | 10/10 | Mọi hằng số và mọi quyết định đều kèm lập luận; mã cũ được giữ lại để so sánh |
| Xử lý lỗi | 9/10 | `finally` bắt buộc; nhưng nuốt `IllegalStateException` không đếm |
| An toàn đa luồng | 9/10 | `volatile` + `synchronized transitionTo` + biến cục bộ; `evictExpiredJobs` chạy đồng thời an toàn nhờ `ConcurrentHashMap` |
| Khả năng kiểm thử | 4/10 | **Không có bộ test riêng** — bốn lỗ rò có thể tái phát mà không có gì cảnh báo |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Đưa `MAX_CONCURRENT_JOBS` vào cấu hình, suy từ heap.** Con số 2 hiện là
   hằng số cứng, suy từ giả định "corpus 367 MB, heap 2 GB". Nếu triển khai trên
   máy heap 8 GB thì 2 là quá thận trọng; nếu corpus lớn gấp ba thì 2 là quá
   liều. Công thức
   `max(1, (heapKhaDung * 0.4) / boNhoUocTinhMoiJob)` thích nghi được — và quan
   trọng hơn, nó **ghi lại lập luận** dưới dạng mã thay vì dưới dạng chú thích.

2. **Tách `unroutableEvents` thành ba bộ đếm.** Ba nguyên nhân ở mục 3.3 có mức
   nghiêm trọng **hoàn toàn khác nhau**: `jobId` rỗng là lỗi thật cần cảnh báo
   ngay; job đã bị dọn là hành vi bình thường ở chế độ Kafka. Gộp làm một nghĩa
   là không đặt được cảnh báo cho ca nghiêm trọng — đúng vấn đề mà
   [`ImageDownloadService`](../crawler/modular/ImageDownloadService.md) mục 2.5
   đã giải bằng cách tách `imagesBlocked` khỏi `downloadFailures`.

3. **Bộ test cho bốn lỗ rò** (mã ở mục 6). Bài test #1 và #2 (kiểm crawler được
   thu hồi bằng `WeakReference`) là hai bài đáng giá nhất trong cả tài liệu này:
   chúng bảo vệ một lỗ hổng bảo mật thật, và loại lỗi đó **không thể phát hiện
   bằng mắt** khi đọc mã sau này.

4. **Đếm khi nuốt `IllegalStateException`.** Hiện `catch (IllegalStateException
   ignored)` không để lại dấu vết nào. Nếu nó xảy ra thường xuyên, nghĩa là có
   một đường chuyển trạng thái ngoài dự kiến — thông tin quan trọng. Một
   `AtomicLong invalidTransitions` là đủ, và nó cùng tinh thần với
   `unroutableEvents` mà chính lớp này đã làm đúng ở chỗ khác.

5. **Kiểm `extractDomains` trả về tập rỗng.** Hiện nếu **mọi** seed URL đều dị
   dạng, `domains` rỗng và job vẫn chạy — crawl được 0 trang, trạng thái `DONE`,
   không lỗi. Đây là kiểu hỏng im lặng mà chính lớp này đã cẩn thận tránh ở chỗ
   khác. Nên ném ngay trong `start()` (fail-fast, vì đây là lỗi đầu vào của người
   gọi) kèm thông báo nêu rõ seed nào không phân giải được. Và cân nhắc hỗ trợ
   subdomain — hiện `the-thao.vnexpress.net` có thể bị loại dù cùng site.
