...
|---|---|---|
| **Chẩn đoán "Feature Envy" đúng tên và đúng dấu hiệu** | 10/10 | Hai dấu hiệu kiểm được, và phép thử áp dụng lại được cho cả dự án |
| **Chỉ ra lỗ hổng chức năng do mã không dùng lại được** | 10/10 | Nêu **đích danh** truy vấn đánh giá vô nghĩa mà việc bỏ sót đã tạo ra |
| Kỹ thuật điểm bất động | 10/10 | Phép kiểm định nghĩa **bằng** phép biến đổi ⇒ không thể trôi lệch |
| Chính sách "không kết luận khi thiếu bằng chứng" | 9/10 | Trả `true` khi quá ngắn, kèm ví dụ thật ("Video", "Ảnh") |
| Chiều sai an toàn | 9/10 | Nhận nhầm (giữ thừa) thay vì bỏ sót (mất dữ liệu) — đúng chiều cho bài toán |
| Hằng số có ngữ cảnh | 8/10 | 15 ký tự ≈ 3–4 âm tiết, gắn với tiêu đề mục thật của báo Việt Nam |
| **Kiểm thử** | **0/10** | **Không một test nào** cho hàm quyết định chất lượng toàn bộ corpus |
| **Giới hạn không được ghi** | **4/10** | Tiếng Việt không dấu bị loại nhầm, tiếng Pháp bị nhận nhầm — cả hai đều thật, đều không nêu |
| Tên hàm gộp hai kết quả | 5/10 | `true` nghĩa là "tiếng Việt" **hoặc** "không biết"; người gọi không phân biệt được |
| Chi phí bộ nhớ tạm | 5/10 | Tạo chuỗi mới cỡ bằng đầu vào; ~360 MB rác trên corpus 30.017 trang |
| Chỉ phân biệt hai lớp | 5/10 | `WebDocument.language` cần ba giá trị (`vi`/`en`/`und`), hàm này trả `boolean` |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết test cho heuristic, đặc biệt cho các giới hạn đã biết.** Hàm 12 dòng này
   quyết định trang nào vào corpus và truy vấn đánh giá nào được sinh — tức toàn
   bộ chất lượng dữ liệu của hệ thống phụ thuộc vào nó, và nó **không có test
   nào**. Quan trọng nhất là ghi lại các giới hạn: một test cho hành vi "sai nhưng
   cố ý" biến nó từ **lỗi chưa biết** thành **quyết định đã ghi nhận**:
   ```java
   class LanguageDetectorTest {

       @ParameterizedTest
       @CsvSource({
           "'Máy tính xách tay giá rẻ cho sinh viên', true,  'tiếng Việt có dấu'",
           "'The quick brown fox jumps over the dog', false, 'tiếng Anh'",
           "'柬埔寨国会主席昆索达莉圆满结束对越南的正式访问', false, 'tiếng Trung — ca đã sinh truy vấn rác'",
           "'Video',                                   true,  'quá ngắn ⇒ không kết luận'",
           "'may tinh xach tay gia re cho sinh vien',  false, 'GIỚI HẠN ĐÃ BIẾT: tiếng Việt không dấu bị loại'",
           "'élève français après le déjeuner ici',    true,  'GIỚI HẠN ĐÃ BIẾT: tiếng Pháp nhận nhầm'",
       })
       void heuristicDauThanh(String text, boolean mongDoi, String moTa) {
           assertEquals(mongDoi, LanguageDetector.looksVietnamese(text), moTa);
       }

       @Test
       void ranhGioiNguong15KyTu() {
           assertTrue(LanguageDetector.looksVietnamese("khong dau 1234"));   // 14 ⇒ khong ket luan
           assertFalse(LanguageDetector.looksVietnamese("khong dau 12345")); // 15 ⇒ ket luan
       }
   }
   ```

2. **Trả về ba trạng thái thay vì `boolean`.** Hiện `true` gộp "chắc chắn tiếng
   Việt" với "quá ngắn để biết", nên hai người gọi có nhu cầu khác nhau không tách
   được: [`LanguageFilter`](../crawler/LanguageFilter.md) muốn khoan dung (giữ cả
   trường hợp không chắc), còn
   [`KnownItemQueryGenerator`](../eval/KnownItemQueryGenerator.md) nên chặt chẽ —
   sinh truy vấn từ một tiêu đề "không biết ngôn ngữ" chính là nguồn của truy vấn
   rác ở mục 2:
   ```java
   public enum KetQua { CHAC_CHAN_TIENG_VIET, KHONG_PHAI_TIENG_VIET, KHONG_KET_LUAN }

   public static KetQua nhanDien(String text) { ... }

   /** Giu tuong thich: coi KHONG_KET_LUAN la hop le. */
   public static boolean looksVietnamese(String text) {
       return nhanDien(text) != KetQua.KHONG_PHAI_TIENG_VIET;
   }
   ```
   Ba trạng thái này cũng khớp thẳng với ba giá trị `"vi"`/`"en"`/`"und"` mà
   [`WebDocument.language`](../model/WebDocument.md) đã dùng — hiện `boolean`
   không biểu diễn được `"und"`, nên thông tin bị mất ở ranh giới hai lớp.

3. **Dừng ở ký tự có dấu đầu tiên thay vì bỏ dấu cả chuỗi.** Hàm hiện tạo một bản
   sao toàn bộ văn bản chỉ để phát hiện xem có **một** ký tự có dấu hay không —
   trên corpus 30.017 trang đó là ~360 MB rác cho một câu trả lời `boolean`:
   ```java
   public static boolean coDauThanh(String s) {
       for (int i = 0; i < s.length(); i++) {
           char c = s.charAt(i);
           if (c > 127 && VietnameseTokenizer.stripDiacritics(String.valueOf(c)).charAt(0) != c) {
               return true;      // DUNG NGAY o ky tu co dau dau tien
           }
       }
       return false;
   }
   ```
   Với văn bản tiếng Việt, ký tự có dấu thường xuất hiện trong vài chục ký tự đầu
   ⇒ thoát sớm gần như tức thì. Với văn bản tiếng Anh thì vẫn duyệt hết, nhưng
   phép kiểm `c > 127` loại phần lớn ký tự bằng một phép so sánh — không cấp phát
   gì. Vẫn giữ nguyên tinh thần điểm bất động, chỉ áp ở mức **ký tự** thay vì mức
   **chuỗi**.
