...
|---|---|---|
| **Phân biệt nguồn sự thật với cache dẫn xuất** | 10/10 | Quyết định kiến trúc có hệ quả nhất quán ở **ba** chỗ: đọc, ghi, và sau crawl |
| **"Nguồn rỗng không phải là nguồn"** | 10/10 | Hai lỗi cùng họ, kèm hậu quả cụ thể (container vào vòng khởi động lại vô hạn) |
| **Chỉ số quan sát phát hiện lỗi chưa biết** | 10/10 | `indexSizeBytes` luôn bằng 0 tố cáo một đường nhanh chết tốn 58,5 s/lần khởi động |
| **Ghi lại một lỗi xảy ra ngay dưới bình luận cảnh báo chính nó** | 10/10 | `pageRankScores` bị bỏ sót — bằng chứng "hiểu nguyên tắc" không đủ |
| Nối hai quyết định ở hai lớp | 10/10 | Bình luận về `getDocLength` vs `getBodyText()` ngăn một lỗi làm mọi thống kê thành 0 |
| Chuỗi dự phòng là dữ liệu | 10/10 | Thêm nguồn = một dòng; tầng seed cho người vừa clone chạy được ngay |
| Thông báo log kèm cách sửa | 10/10 | Nói cả "hệ thống có sao không" và "phải làm gì" |
| Tiêm qua hàm dựng | 9/10 | Phụ thuộc là `final`; chỉ cấu hình `@Value` mới dùng trường |
| Bỏ 34 MB thường trú | 9/10 | Và chỉ ra tối ưu nén bị vô hiệu bởi một trường ở lớp khác |
| **Không có cơ chế cưỡng chế chụp trạng thái** | **4/10** | Bốn trường `volatile` phải chụp thủ công; đã bỏ sót một lần, sẽ bỏ sót lần nữa |
| **Kiểm thử `persistIndex`** | **2/10** | Lỗi 58,5 giây quay lại được mà mọi test vẫn xanh |
| Phân trang sâu | 4/10 | `topN = page × size` ⇒ trang 100 xếp hạng 2.000 kết quả để lấy 20 |
| `getIndexSizeBytes` chạm đĩa | 5/10 | `getStats()` gọi nó; bảng điều khiển tự làm mới ⇒ đọc đĩa liên tục |
| Lớp vẫn dài 497 dòng | 6/10 | Đã bỏ bảy trách nhiệm nhưng vẫn ôm cả nạp corpus, phân trang, và sáu accessor thống kê |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Gom bốn trường `volatile` thành một ảnh chụp bất biến — đây là đề xuất quan
   trọng nhất.** Mục 1 cho thấy nguyên tắc "chụp một lần" đã được hiểu, viết thành
   bình luận, **và vẫn bị vi phạm** ở trường thứ tư. Cách duy nhất chắc chắn là
   làm cho việc đọc lệch pha **không biểu diễn được**:
   ```java
   /** Anh chup NHAT QUAN cua moi trang thai dan xuat tu mot chi muc. */
   private record EngineState(SearchIndex index,
                               RelevanceScorer scorer,
                               Map<Integer, Double> pageRankScores,
                               LRUCache<String, SearchResponse> cache,
                               CorpusStats corpusStats) { }

   private volatile EngineState state;      // MOT truong volatile duy nhat

   public SearchResponse search(String rawQuery, int page, int size) {
       EngineState s = state;               // MOT lan doc, KHONG THE lech pha
       ...
   }
   ```
   `refreshDerivedState()` khi đó dựng một `EngineState` mới rồi gán **một lần** —
   phép gán tham chiếu là nguyên tử, nên mọi truy vấn hoặc thấy toàn bộ trạng thái
   cũ, hoặc toàn bộ trạng thái mới, không bao giờ thấy hỗn hợp. Thêm một trường
   dẫn xuất mới sau này cũng tự động an toàn.

2. **Viết test khoá lại `persistIndex` — lỗi 58,5 giây phải không quay lại được.**
   Javadoc dành 11 dòng kể lại lỗi này, kèm số đo và cách phát hiện, nhưng không
   có gì ngăn ai đó xoá lời gọi `persistIndex()` trong một lần dọn dẹp. Hệ thống
   vẫn chạy **hoàn toàn đúng**, chỉ chậm gấp 40 lần khi khởi động:
   ```java
   @Test
   @DisplayName("Dựng chỉ mục từ corpus PHẢI ghi tệp chỉ mục ra đĩa")
   void dungChiMucXongPhaiGhiRaDia(@TempDir Path thuMuc) throws Exception {
       Path chiMuc = thuMuc.resolve("index.json");
       facade.setIndexDataPath(chiMuc.toString());

       facade.reindex();

       assertTrue(Files.exists(chiMuc), "Thieu buoc nay => moi lan khoi dong lap lai chi muc");
       assertTrue(Files.size(chiMuc) > 1000, "Tep chi muc rong cung vo dung — xem loadCorpus");
   }

   @Test
   void ghiChiMucThatBaiKhongLamHongPhienCrawl() {
       // dat indexDataPath vao mot duong dan khong ghi duoc
       assertDoesNotThrow(() -> facade.reindex());
   }
   ```
   Ca thứ hai khoá lại chính sách "cache dẫn xuất" ở mục 3 — cũng là một tính chất
   dễ bị phá khi ai đó "sửa cho chặt chẽ" bằng cách để ngoại lệ nổi lên.

3. **Chặn phân trang sâu, hoặc ít nhất ghi rõ chi phí của nó.** `topN = max(page ×
   size, size)` nghĩa là trang 100 phải chấm điểm và xếp hạng **2.000** kết quả để
   trả về 20 — chi phí tăng tuyến tính theo số trang, và không có gì chặn một
   người dùng (hoặc một bot) gọi `page=10000`:
   ```java
   private static final int MAX_PAGE = 50;   // 50 trang x 20 = 1.000 ket qua

   public SearchResponse search(String rawQuery, int page, int size) {
       int trangDaKep = Math.min(Math.max(page, 1), MAX_PAGE);
       ...
   }
   ```
   Và theo đúng nguyên tắc của
   [`../model/SearchResponse.md`](../model/SearchResponse.md) mục 1, giá trị **đã
   kẹp** phải là giá trị trả về trong `SearchResponse.page` — hiện tại dòng 315
   truyền `page` nguyên văn, tức lặp lại chính lỗi mà trường `pageSize` sinh ra để
   sửa, chỉ ở một tham số khác.
