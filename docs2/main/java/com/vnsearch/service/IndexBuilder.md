...
|---|---|---|
| **Gom tiền đề về một chỗ** | 10/10 | Chỉ rõ tiền đề từng lặp ở **ba** nơi và hậu quả khi quên: "kết quả sai một cách im lặng" |
| **Hai lớp bảo vệ độc lập** | 10/10 | `IndexBuilder` làm lỗi **không xảy ra**, `InvertedIndex` làm lỗi **không thể ẩn** |
| **Phân biệt danh tính hệ thống với thuộc tính vật** | 10/10 | docId là chỉ số vào posting list, không phải thuộc tính của trang web |
| **Chia đúng ranh giới song song / tuần tự** | 10/10 | Nêu tường minh điều kiện an toàn (tokenizer bất biến) trước khi song song hoá |
| Chặn đỉnh bộ nhớ bằng chia lô | 10/10 | Khác biệt giữa "chạy được" và `OutOfMemoryError` ở corpus 30.017 trang |
| Biện minh hai hằng số | 9/10 | Cả `BATCH_SIZE` lẫn `PARALLEL_THRESHOLD` đều có lý do, kể cả lý do về test |
| Bình luận về `toList()` giữ thứ tự | 9/10 | Lớp bảo vệ **duy nhất** cho một bất biến không có test |
| Dựng mới thay vì cập nhật | 9/10 | Lập luận "không đáng đánh đổi tính đúng đắn" được ghi ra rõ |
| **Kiểm thử** | **1/10** | Không một test nào; nhánh song song **chưa từng chạy** trong bộ test |
| **Sửa `docId` của người gọi** | **4/10** | Chính là "tác dụng phụ từ xa" mà [`WebDocument`](../model/WebDocument.md) được viết ra để tránh |
| Dùng `commonPool` chung của JVM | 5/10 | Tranh chấp với mọi tác vụ song song khác; không có bể luồng riêng |
| Ngưỡng theo số lượng, không theo khối lượng | 5/10 | 1.999 tài liệu dài 50.000 từ vẫn chạy tuần tự |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết test cho nhánh song song và bất biến gán token — đây là lỗ hổng nghiêm
   trọng nhất.** Nhánh `buildInBatches` là phần phức tạp nhất lớp, và nó **chưa
   từng chạy** trong bộ test vì mọi test đều dùng corpus dưới 2.000 tài liệu. Bất
   biến "token thứ $i$ ứng với tài liệu thứ $i$" hiện chỉ được bảo vệ bằng một
   dòng bình luận:
   ```java
   @Test
   @DisplayName("Nhánh song song cho kết quả GIỐNG HỆT nhánh tuần tự")
   void songSongGiongHetTuanTu() {
       List<WebDocument> corpus = sinhCorpus(2_500);          // VUOT nguong 2.000

       InvertedIndex songSong = new IndexBuilder(tokenizer).build(sao(corpus));
       InvertedIndex tuanTu   = dungTuanTuThuCong(sao(corpus));

       assertEquals(tuanTu.getTermCount(), songSong.getTermCount());
       for (String term : tuanTu.allTerms()) {
           assertEquals(tuanTu.getPostings(term), songSong.getPostings(term),
                   "Term «" + term + "» gan SAI tai lieu — kiem tra toList() con giu thu tu khong");
       }
   }

   @Test
   void docIdTrungLapKhongLamSapUngDung() {
       List<WebDocument> corpus = List.of(doc(5, "a"), doc(5, "b"), doc(2, "c"));
       assertDoesNotThrow(() -> new IndexBuilder(tokenizer).build(corpus));
   }
   ```
   Ca thứ hai canh giữ đúng lỗi từng làm ứng dụng không khởi động được (mục 2) —
   loại lỗi mà một tệp dữ liệu hơi lệch cũng kích hoạt được.

2. **Đừng sửa `docId` của đối tượng người gọi.** Dòng `doc.setDocId(nextDocId++)`
   sửa **tại chỗ** danh sách mà người gọi truyền vào — chính là "tác dụng phụ từ
   xa" mà [`../model/WebDocument.md`](../model/WebDocument.md) mục 1 mô tả và
   `withoutBodyText()` được viết ra để tránh. Dự án đã nêu nguyên tắc này ở một
   lớp, nên áp dụng không nhất quán ở lớp khác là chỗ dễ gây nhầm cho người đọc:
   ```java
   int nextDocId = 0;
   for (WebDocument goc : sorted) {
       WebDocument choChiMuc = goc.withoutBodyText();   // da la BAN SAO
       choChiMuc.setDocId(nextDocId++);                 // sua BAN SAO, khong sua goc
       index.addDocument(choChiMuc);
   }
   ```
   Cách này còn ăn khớp tự nhiên với `withoutBodyText()`: chỉ mục vốn **đã** không
   cần thân bài, nên bản sao là thứ nên đi vào chỉ mục ngay từ đầu.

3. **Dùng bể luồng riêng thay vì `ForkJoinPool.commonPool`.** `parallelStream` mặc
   định chạy trên bể chung của cả JVM — cùng bể mà mọi `parallelStream` khác trong
   ứng dụng dùng, kể cả các thư viện. Trong một ứng dụng Spring phục vụ HTTP, một
   lần reindex có thể chiếm hết bể chung và làm chậm những phần không liên quan:
   ```java
   private static final int SO_LUONG =
           Math.max(1, Runtime.getRuntime().availableProcessors() - 1);   // chua 1 loi cho HTTP

   private List<List<Token>> tachTuSongSong(List<WebDocument> batch) throws Exception {
       ForkJoinPool be = new ForkJoinPool(SO_LUONG);
       try {
           return be.submit(() -> batch.parallelStream()
                   .map(doc -> tokenizer.tokenize(InvertedIndex.indexableText(doc)))
                   .toList()).get();
       } finally {
           be.shutdown();
   ...
   ```
   Chừa lại một lõi cũng là chi tiết đáng giá: reindex chạy **trong khi** hệ thống
   vẫn phục vụ truy vấn, nên chiếm 100 % CPU sẽ làm độ trễ truy vấn tăng vọt đúng
   lúc người dùng đang dùng.
