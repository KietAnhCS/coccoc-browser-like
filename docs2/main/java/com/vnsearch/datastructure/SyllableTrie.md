...
|---|---|---|
| **Nêu vấn đề bằng số, trung thực cả yếu tố giảm nhẹ** | 10/10 | "15 triệu lần cấp phát", và thừa nhận "chết non nên đời hết lên vài" thay vì phóng đại |
| **Lợi ích vượt ngoài tốc độ: cắt nhánh sớm** | 10/10 | Chỉ ra thông tin mà `HashSet` **không thể** cung cấp — lập luận mạnh nhất |
| **Một bảng băm cho cả cây** | 10/10 | Ý tưởng trung tâm, và giải thích rõ vì sao một bảng lớn hơn hẳn nhiều bảng nhỏ |
| **Cảnh báo bẫy hàm băm** | 10/10 | Vứt 32 bit cao ⇒ $O(1)$ thành $O(n)$ mà **vẫn chạy đúng** — loại lỗi khó tìm nhất |
| **Không chép hằng số từ nơi khác** | 10/10 | 0,55 vs 0,75: nêu công thức $1/(1-\alpha)$ và vì sao chaining khác open addressing |
| Bất biến id $\ge 1$ | 10/10 | Gọi hẳn là "Bất biến", giải thích cả cơ chế lẫn hệ quả nếu vi phạm |
| Định lượng chi phí boxing | 10/10 | "64 byte phụ trội cho một cạnh đáng lẽ chỉ cần 12 byte" |
| Test phủ ba quyết định khó nhất | 9/10 | Cắt nhánh, băm trộn, băm lại — mỗi cái một ca |
| `intern` vs `idOf` | 9/10 | Cảnh báo đúng chỗ, đúng hậu quả |
| Hàm phục vụ báo cáo | 9/10 | `nodeCount`/`edgeCount`/`approximateBytes` |
| **Không có tài liệu về đa luồng** | **3/10** | Lớp **không** an toàn, và ràng buộc "nạp xong mới đọc" không nằm ở đâu — trong khi `MinHeap` và `Trie` đều ghi rõ |
| **Kiểm thử phân bố hàm băm** | **3/10** | Đổi `hash()` thành `(int) key` làm chậm hàng nghìn lần mà **mọi test vẫn xanh** |
| Kiểm tra biên | 4/10 | `weightAt(NONE)` ném ngoại lệ thô; `wordWeight <= 0` tạo "từ ma" im lặng |
| `syllableIds` chưa được tối ưu | 5/10 | Vẫn `HashMap<String,Integer>` với boxing, gọi **cũng nhiều như** `child()` |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Viết test đo **phân bố** hàm băm, không chỉ tính đúng đắn.** Đây là lỗ hổng
   nguy hiểm nhất: `hash()` là phần tinh vi nhất của lớp, Javadoc dành cả một đoạn
   giải thích vì sao nó cần thiết — nhưng nếu ai đó thay nó bằng `(int) key` "cho
   gọn", cấu trúc vẫn trả kết quả **hoàn toàn đúng**, chỉ chậm hàng nghìn lần, và
   toàn bộ 9 ca test vẫn xanh. Phải đo chuỗi thăm dò thì mới bắt được:
   ```java
   @Test
   @DisplayName("Một âm tiết dưới nhiều cha phải phân bố đều, không dồn thành chuỗi dài")
   void hamBamPhanBoDeu() {
       SyllableTrie trie = new SyllableTrie(1 << 16);
       int cua = trie.intern("của");
       for (int i = 0; i < 5_000; i++) {              // 5.000 cha khac nhau
           trie.insert(new String[]{"cha" + i, "của"}, 1.0);
       }
       assertTrue(trie.chuoiThamDoDaiNhat() < 20,
               "Chuoi tham do dai bat thuong — ham bam co dang vut mat 32 bit cao khong?");
   }
   ```
   Cần thêm một phương thức `chuoiThamDoDaiNhat()` (hoặc `thongKeThamDo()`) — nó
   cũng có ích cho báo cáo, cùng nhóm với `approximateBytes()` đã có.

2. **Ghi rõ ràng buộc vòng đời và đa luồng vào Javadoc.** Lớp này **không** an
   toàn đa luồng: `insert`/`intern` sửa `weight[]`, `edgeKey[]`, `syllableIds`,
   trong khi `child`/`idOf` đọc chúng. Mô hình dùng đúng là "nạp xong hết rồi mới
   cho luồng nào đọc" — nhưng điều đó không nằm ở đâu, và [`MinHeap`](./MinHeap.md)
   với [`Trie`](./Trie.md) đều ghi rất rõ phần này:
   ```java
   /**
    * <p><b>KHONG an toan da luong.</b> Vong doi bat buoc gom hai pha tach bach:
    * <ol>
    *   <li><i>Nap</i> — goi {@link #intern} va {@link #insert}. Mot luong duy nhat.</li>
    *   <li><i>Tra cuu</i> — chi goi {@link #idOf}, {@link #child}, {@link #weightAt}.
    *       Nhieu luong SONG SONG duoc, vi khong thao tac nao sua trang thai.</li>
    * </ol>
    * Goi {@link #intern} o pha 2 pha vo bat bien nay — xem canh bao tai
    * {@link #intern}.
    */
   ```
   Cân nhắc thêm một cờ `frozen` cùng phương thức `freeze()` như
   [`SparseMatrix`](./SparseMatrix.md): `intern`/`insert` sau khi đông cứng thì
   ném, biến ràng buộc vòng đời từ lời văn thành phép kiểm chạy được.

3. **Nội suy âm tiết bằng chính bảng phẳng, bỏ nốt `HashMap<String,Integer>`.**
   `idOf()` được gọi **đúng bằng số lần** `child()` được gọi, nhưng nó vẫn đi qua
   `HashMap` với boxing `Integer` và một lần theo con trỏ — tức phần chưa tối ưu
   này có thể chiếm nhiều thời gian hơn cả phần đã tối ưu công phu. Một bảng địa
   chỉ mở thứ hai, khoá là `hashCode` của chuỗi, xoá nốt điểm nghẽn:
   ```java
   // Bang noi suy: syllableHash[] long[], syllableId[] int[], syllableText[] String[]
   public int idOf(String syllable) {
       int slot = hash(syllable.hashCode()) & syllableMask;
       while (syllableText[slot] != null) {
           if (syllableText[slot].equals(syllable)) return syllableId[slot];
           slot = (slot + 1) & syllableMask;
       }
       return NONE;
   }
   ```
   Trước khi làm, nên **đo** đã: nếu `idOf` chỉ chiếm 10 % thời gian tách từ thì
   công sức này không đáng, và bản thân phép đo đó là một dòng số liệu đáng đưa
   vào báo cáo — nó cho thấy tối ưu được dẫn dắt bởi số liệu chứ không bởi cảm
   giác.
