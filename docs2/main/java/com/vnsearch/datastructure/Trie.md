...
|---|---|---|
| **Giải pháp cho vấn đề gõ không dấu** | 10/10 | Một trường `display` giải quyết trọn vẹn, giữ nguyên $O(L)$, không thêm cấu trúc phụ |
| **Đối chiếu khoá với `LRUCache`** | 10/10 | Hai lớp trỏ sang nhau, cùng dạy một nguyên tắc: chọn khoá theo hành vi thật |
| **Nêu kịch bản tranh chấp có thật** | 10/10 | Chỉ đích danh `SearchEngineFacade.search()` vs `/api/suggest`, và dẫn lỗi HashMap Java 7 |
| Chuẩn hoá NFC | 10/10 | Vấn đề rất thật với người dùng macOS, và lỗi **không tái hiện được** trên máy phát triển |
| Giữ khoá ngắn | 10/10 | Phần tính top-K nằm **ngoài** khoá, chỉ làm việc trên bản sao cục bộ |
| Khử trùng đúng ba chi tiết | 10/10 | Khoá gộp là hiển thị, `Math::max` không phải `sum`, `LinkedHashMap` cho tất định |
| Test phủ đúng quyết định thiết kế | 10/10 | Ba quyết định lớn, ba ca test mang tên chính quyết định đó |
| `clear()` $O(1)$ | 9/10 | Tận dụng GC, và giải thích rõ vì sao `root` không `final` |
| Mẫu quay lui trong DFS | 9/10 | Một `StringBuilder` cho toàn cây con |
| **Kiểm thử đa luồng** | **2/10** | Lý do chính khiến lớp có khoá, mà **không có ca nào** — trong khi `LRUCache` thì có |
| **Đệ quy không giới hạn độ sâu** | **3/10** | `insert` nhận khoá dài tuỳ ý **từ truy vấn người dùng** ⇒ `StackOverflowError` là đường tấn công thật |
| Bộ nhớ mỗi node | 4/10 | `HashMap` 48 B cho node chỉ có 1–2 con; chèn hai lần làm gấp đôi |
| Ngữ nghĩa `frequency` vs `display` | 5/10 | Một cộng dồn, một ghi đè — không được ghi rõ ở đâu |
| Bỏ qua đầu vào không hợp lệ trong im lặng | 5/10 | `insert` trả về ngay khi key rỗng hoặc `frequency <= 0`, không log |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Chặn độ sâu cây — `insert` đang nhận dữ liệu người dùng không giới hạn.**
   Đây là lỗ hổng nghiêm trọng nhất: `SearchEngineFacade.search()` gọi
   `trie.insert(truyVan)` cho **mọi** truy vấn, nên một truy vấn 50.000 ký tự tạo
   một nhánh sâu 50.000 tầng, và lần `getSuggestions` tiếp theo đổ ngăn xếp — làm
   sập luồng HTTP đó. Hai dòng khép lỗ, và chúng cũng chặn luôn việc cây phình ra
   vì rác:
   ```java
   /** Do dai khoa toi da. Goi y dai hon muc nay khong co gia tri su dung,
    *  va no bao ve DFS de quy khoi StackOverflowError. */
   private static final int MAX_KEY_LENGTH = 128;

   public void insert(String key, String display, int frequency) {
       if (key == null || key.isEmpty() || frequency <= 0) return;
       if (key.length() > MAX_KEY_LENGTH) return;      // hoac cat bot
       ...
   }
   ```
   Bổ sung: chuyển `collectWords` sang vòng lặp với ngăn xếp tường minh thì loại
   bỏ hẳn rủi ro, nhưng với giới hạn 128 thì đệ quy hoàn toàn an toàn và mã dễ đọc
   hơn — nên giới hạn là lựa chọn đúng ở đây.

2. **Viết test đa luồng, đối xứng với `LRUCache`.** Javadoc dành cả một đoạn (dòng
   39–45) để giải thích vì sao lớp cần khoá, dẫn cả lỗi HashMap Java 7 — nhưng
   không có gì chứng minh khoá thật sự hoạt động. `LRUCache` có
   `concurrentAccessDoesNotCorruptState`; lớp này thì không, dù kịch bản tranh
   chấp ở đây **cụ thể hơn** (hai endpoint HTTP đã được nêu đích danh):
   ```java
   @Test
   void insertVaGetSuggestionsSongSongKhongLamHongCay() throws InterruptedException {
       ...
       assertTrue(xong.await(30, TimeUnit.SECONDS), "Co luong bi TREO — dau hieu cay bi hong");
       assertEquals(5, trie.getSuggestions("tu", 5).size());
   }
   ```
   Phép khẳng định trên `await` là phần quan trọng nhất: triệu chứng của lỗi
   HashMap đồng thời chính là **treo**, không phải ngoại lệ.

3. **Đưa việc chèn hai lần vào chính `Trie`, thay vì để người gọi nhớ.** Hiện
   `SuggestionService` phải tự gọi `insert` hai lần với khoá có dấu và không dấu
   (cạm bẫy ①); quên một lần thì mất hẳn một kiểu gõ, và không có gì báo. Vì đây
   là **cách dùng duy nhất đúng** cho tiếng Việt, nó thuộc về lớp chứ không thuộc
   về người gọi:
   ```java
   /** Chen mot goi y tieng Viet: tu dong chen CA khoa co dau lan khoa khong dau,
    *  ca hai cung tro ve mot chuoi hien thi. */
   public void insertVietnamese(String display, int frequency) {
       insert(display, display, frequency);
       String khongDau = VietnameseTokenizer.stripDiacritics(display);
       if (!khongDau.equals(display)) {
           insert(khongDau, display, frequency);
       }
   }
   ```
   Phép kiểm `!khongDau.equals(display)` là kỹ thuật **điểm bất động** đã dùng ở
   [`../ranking/QuerySyllables.md`](../ranking/QuerySyllables.md) mục 2 — chuỗi
   vốn không dấu thì chỉ chèn một lần, tránh gấp đôi bộ nhớ vô ích cho phần lớn
   khoá ASCII.
