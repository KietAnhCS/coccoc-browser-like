...
|---|---|---|
| **"Dư thông tin là nói sai"** | 10/10 | Phát biểu đúng và phản trực giác: dữ liệu thừa **tạo ra** kết luận sai, không phải vô hại |
| **Lập luận xoá mã dựa trên bằng chứng** | 10/10 | Bốn bước: không ai gọi → chỉ vì Jackson → Jackson không cần → hợp đồng không đổi |
| **Kiểm tra hợp đồng API trước khi refactor** | 10/10 | Xác nhận tên khoá JSON giống hệt **trước** khi quyết định |
| Phân biệt trường nào giữ, trường nào xoá | 10/10 | `pageRankScore` giữ vì giá trị **thật sự khác**; `tfidfScore` xoá vì trùng |
| Chọn kiểu thời gian thật | 9/10 | `Instant` thay chuỗi ⇒ loại bỏ hẳn lớp lỗi múi giờ |
| Bất biến miễn phí | 9/10 | `record` ⇒ không setter ⇒ an toàn đa luồng, `equals` đúng |
| Tài liệu tham số | 8/10 | Nói rõ `pageRankScore` chỉ để **quan sát**, không phải thành phần tính điểm |
| **Kiểu không phân biệt HTML với văn bản thuần** | **4/10** | `snippet` là HTML nhưng khai báo `String` y hệt `title` — không có gì cảnh báo người dùng API |
| Khoá hợp đồng JSON bằng test | 3/10 | Điều kiện then chốt cho phép chuyển đổi, nhưng **không có test** giữ nó |
| Miền giá trị của `score` | 5/10 | Không chuẩn hoá, không so sánh được giữa các truy vấn — chỉ nói trong Javadoc lớp khác |
| Kiểm tra `null` | 5/10 | `record` nhận `null` cho mọi trường tham chiếu |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Khoá hợp đồng JSON bằng một test.** Toàn bộ quyết định chuyển sang `record`
   dựa trên điều kiện "tên khoá JSON sinh ra y hệt bản cũ" — nhưng không có gì
   giữ điều kiện đó lại. Đổi tên một trường (ví dụ `score` → `finalScore` cho rõ
   nghĩa) là thay đổi **phá vỡ** với mọi client, và trình biên dịch sẽ vui vẻ chấp
   nhận:
   ```java
   @Test
   @DisplayName("Hợp đồng JSON của SearchResult không được đổi")
   void hopDongJsonKhongDoi() throws Exception {
       String json = new ObjectMapper().findAndRegisterModules().writeValueAsString(
               new SearchResult("t", "u", "s", 1.0, 0.5, Instant.parse("2026-08-15T10:30:00Z")));

       assertEquals(Set.of("title", "url", "snippet", "score", "pageRankScore", "crawledAt"),
               new ObjectMapper().readTree(json).properties().stream()
                       .map(Map.Entry::getKey).collect(toSet()),
               "Doi ten/them/bot khoa JSON la thay doi PHA VO hop dong API");
   }
   ```
   Ca test này cũng bắt luôn trường hợp ai đó vô tình thêm trường mới mà quên cập
   nhật tài liệu API.

2. **Đặt tên trường nói rõ `snippet` là HTML.** Kiểu `String` không phân biệt được
   "văn bản thuần" với "HTML đã thoát", nên người viết client không có cách nào
   biết phải dùng `innerHTML` ngoài việc đọc tài liệu — và người thêm trường mới
   sau này không có gì nhắc rằng trường đó cũng phải thoát ký tự. Rẻ nhất là để
   tên trường tự nói:
   ```java
   public record SearchResult(String title, String url, String snippetHtml,
                               double score, double pageRankScore, Instant crawledAt) { }
   ```
   Đây **là** thay đổi phá vỡ hợp đồng, nên chỉ làm nếu client cũng do nhóm này
   kiểm soát. Nếu không, tối thiểu ghi vào Javadoc tham số: *"HTML đã thoát ký tự,
   chứa thẻ `<mark>` của hệ thống — render bằng `innerHTML`"*, cùng nội dung với
   [`../ranking/SnippetBuilder.md`](../ranking/SnippetBuilder.md) đề xuất 2.

3. **Nói rõ miền giá trị của `score` ngay trong Javadoc tham số.** Con số này
   không chuẩn hoá, có thể vượt 1 (với TF-IDF, do xấp xỉ chuẩn hoá độ dài — xem
   [`../ranking/TfIdfScorer.md`](../ranking/TfIdfScorer.md) mục 3), và **không so
   sánh được** giữa hai truy vấn khác nhau. Người viết giao diện rất dễ nhân 100
   và hiển thị "độ liên quan 28 %":
   ```java
   /**
    * @param score diem cuoi cung dung de xep hang, DA gom moi tin hieu.
    *              <b>Chi co Y NGHIA THU TU trong cung mot truy van.</b>
    *              Khong chuan hoa ve [0,1], khong so sanh duoc giua hai
    *              truy van, va khong nen hien thi nhu mot ty le phan tram.
    */
   ```
