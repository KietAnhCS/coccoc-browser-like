...
|---|---|---|
| Tính đơn trách nhiệm | 10/10 | Thích ứng một đường dẫn thành `DocumentStore`, không tự phân tích JSON — không trùng lặp với `ContentStorage` |
| Khả năng dùng lại | 10/10 | Một lớp phục vụ ba tầng dự phòng nhờ tham số hoá `(path, label)` |
| Bất biến & an toàn luồng | 10/10 | `final class`, mọi trường `final`, không trạng thái — an toàn luồng miễn phí |
| Trải nghiệm người dùng đầu tiên | 10/10 | Tầng `seed-documents.json` là chi tiết hiếm gặp: repo tự chạy được ngay sau `git clone` |
| Xử lý lỗi | 9/10 | Truyền thẳng `IOException`, không nuốt lỗi; đúng hợp đồng interface |
| Tính đúng đắn của `isAvailable()` | **6/10** | Chặn `null` và chuỗi rỗng rất tốt, nhưng `Files.exists` **nhận nhầm thư mục là corpus** và gộp "thiếu quyền" với "không tồn tại" |
| Chất lượng Javadoc | **6/10** | Giải thích ý đồ xuất sắc, nhưng viết **không dấu** và chứa con số cứng (5.011 trang) đã lệch với corpus hiện tại |
| Độ bao phủ kiểm thử | **4/10** | Không có bài test nào cho ca biên `null` / rỗng / thư mục; ca `""` là lỗi im lặng đúng loại mà test tồn tại để bắt |
| Khả năng quan sát | 7/10 | `describe()` trả đủ nhãn + đường dẫn, nhưng nhãn mặc định `"JSON"` vô nghĩa và không log gì khi bỏ qua một tầng |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Thay `Files.exists` bằng `Files.isRegularFile(p) && Files.isReadable(p)`.**
   Đây là sửa một dòng và nó đóng hai lỗ hổng cùng lúc: một đường dẫn trỏ vào thư
   mục hiện đang được nhận là nguồn hợp lệ, khiến chuỗi dự phòng **dừng sai tầng**
   rồi mới phát hiện qua ngoại lệ; và một tệp tồn tại nhưng thiếu quyền đọc hiện
   bị xếp chung với "tệp không tồn tại", nên một lỗi cấu hình quyền hạn sẽ trôi
   qua hoàn toàn im lặng. Cả hai đều là loại lỗi mà triệu chứng (hệ thống chạy
   bằng 40 tài liệu mẫu) cách xa nguyên nhân (quyền tệp) rất nhiều bước suy luận.

2. **Ghi log ở mức `debug` mỗi khi một tầng bị bỏ qua.** Hiện tại
   `isAvailable() == false` là hoàn toàn im lặng, đúng theo thiết kế của interface.
   Nhưng khi hệ thống chạy bằng seed trong lúc lẽ ra phải chạy bằng corpus thật,
   người vận hành không có manh mối nào về việc **tầng nào đã bị bỏ qua và vì
   sao**. Một dòng `log.debug("Bỏ qua {}: không tìm thấy tệp", describe())` biến
   một cuộc điều tra nửa giờ thành một lần đọc log. Chi phí bằng 0 khi tắt debug.

3. **Bỏ con số cứng khỏi Javadoc, hoặc sinh nó tự động.** Javadoc dòng 16 ghi
   "5.011 trang" trong khi corpus hiện tại là 31.030 trang. Không có cơ chế nào
   giữ hai con số này đồng bộ, và không có test nào phát hiện được sai lệch. Cách
   chữa rẻ nhất là viết mô tả định tính ("corpus đầy đủ đã crawl") và để con số
   thật chỉ tồn tại ở nơi nó được **đo ra** — báo cáo của
   [`PostgresImportRunner`](./PostgresImportRunner.md) và `docs/GIN-BASELINE.md`.
   Nguyên tắc chung: một con số chỉ nên xuất hiện ở đúng một chỗ trong toàn bộ đồ án.

4. **Thêm bài test khoá tầng seed** (mã ở mục 8, bài 4). Đây là bài test có tỉ lệ
   giá trị trên công sức cao nhất trong cả gói: bốn dòng, chạy trong mili giây, và
   nó bảo vệ đúng thứ quyết định ấn tượng đầu tiên của người chấm. Chế độ hỏng mà
   nó bắt — ai đó thêm `data/` vào `.gitignore` để tránh commit corpus 87 MB và vô
   tình kéo theo cả tệp seed — là chuyện rất dễ xảy ra và hoàn toàn vô hình cho
   tới lần clone tiếp theo.

5. **Cân nhắc kiểm tra sơ bộ định dạng trong `isAvailable()` — nhưng có kiểm soát.**
   Hiện một tệp `crawled-multi.json` bị cắt cụt (đứt giữa lúc crawl) vẫn báo "có
   sẵn", rồi `loadAll()` mới ném sau khi đã đọc hàng chục MB. Đọc **byte đầu tiên**
   và kiểm nó là `[` hoặc `{` tốn vài µs và loại được ca tệp rỗng hoàn toàn hoặc
   tệp nhị phân đặt nhầm chỗ. Cần cưỡng lại cám dỗ đi xa hơn: phân tích trọn tệp
   trong `isAvailable()` sẽ biến một phép kiểm µs thành phép kiểm hàng chục giây
   và đọc corpus **hai lần** — đúng cái bẫy đã ghi ở bảng cạm bẫy mục 6.3.
