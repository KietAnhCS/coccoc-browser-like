...
|---|---|---|
| Chọn mẫu thiết kế | 10/10 | Adapter đúng chỗ: `DocumentRepository` giữ nguyên API rộng, hợp đồng hẹp được lộ ra riêng; chiều phụ thuộc một hướng |
| Thiết kế cho chuỗi dự phòng | 10/10 | `isAvailable()` trả `false` thay vì ném — cho phép ứng dụng khởi động trên máy trắng, đúng thứ test và demo cần |
| Phân biệt "rỗng" với "không có" | 10/10 | `countDocuments() > 0` loại được nghịch lý "dựng Docker xong lại tệ hơn không dựng" |
| Quản lý tài nguyên | 10/10 | `try-with-resources` cả hai chỗ; không giữ kết nối giữa các lời gọi nên **không thể** rò rỉ |
| Xử lý & bọc ngoại lệ | 9/10 | Bọc `SQLException` thành `IOException` **có giữ cause**; trừ điểm vì ba lỗi rất khác nhau cùng ra một loại |
| Chất lượng tài liệu | 9/10 | Nêu thẳng "CSDL chỉ là kho, tìm kiếm do chỉ mục tự cài" — câu quan trọng nhất của cả gói; trừ vì viết không dấu |
| Hiệu năng lúc khởi động | **6/10** | Hai kết nối cho một lần khởi động; và **không đặt `loginTimeout`** nên một host chết có thể treo khởi động hàng chục giây |
| Bảo mật thông tin xác thực | **6/10** | Không log `user`/`password` là đúng, nhưng `jdbcUrl` được log nguyên văn và URL JDBC **hợp lệ khi chứa `?password=`** |
| Độ bao phủ kiểm thử | **4/10** | Không có bài test nào cho ca "không có CSDL" — đúng ca quan trọng nhất, và là ca **chạy được mà không cần hạ tầng** |

**Năm đề xuất nâng lên mức sản phẩm:**

1. **Gộp `isAvailable()` và `loadAll()` vào một kết nối duy nhất bằng cách nạp sẵn
   (eager) trong `isAvailable()`.** Hiện hai phương thức mở hai kết nối liên tiếp,
   và giữa chúng còn tồn tại một cửa sổ TOCTOU thật: CSDL có thể biến mất sau khi
   `isAvailable()` trả `true`. Một cách chữa gọn là để `isAvailable()` nạp luôn
   corpus vào một trường `Optional<List<WebDocument>>` rồi `loadAll()` chỉ trả về
   cái đã nạp. Đánh đổi rất rõ: mất tính bất biến của lớp, và nếu nguồn này không
   được chọn thì đã đọc thừa 87 MB. Với **bốn** nguồn và Postgres đứng đầu, phương
   án này chỉ đáng làm nếu chi phí hai kết nối trở thành vấn đề đo được — nên đây
   là đề xuất cần cân nhắc, không phải khuyến nghị vô điều kiện.

2. **Đặt `loginTimeout` cho driver.** Đây là đề xuất có tỉ lệ lợi ích trên công
   sức cao nhất, và là sửa một dòng: `DriverManager.setLoginTimeout(3)` hoặc thêm
   `?connectTimeout=3&socketTimeout=30` vào URL. Hiện tại, nếu `jdbcUrl` trỏ tới
   một host tồn tại nhưng không phản hồi (tường lửa chặn im lặng, container đang
   khởi động), `getConnection` sẽ chờ theo timeout mặc định của hệ điều hành — có
   thể tới 75 giây. Trong thời gian đó ứng dụng **treo hoàn toàn** và người dùng
   không biết chuyện gì đang xảy ra, dù ba tầng JSON có sẵn dữ liệu chỉ cách đó
   vài chục micro giây.

3. **Che tham số nhạy cảm trong `describe()`.** Viết một hàm nhỏ cắt bỏ phần
   query string chứa `password=` trước khi ghép vào chuỗi mô tả. Hiện tại repo
   không dùng dạng URL đó nên chưa rò rỉ, nhưng `describe()` được **ghi thẳng vào
   log** ở `SearchEngineFacade`, và log thường được thu thập tập trung, lưu lâu,
   nhiều người đọc. Một lỗi rò rỉ thông tin xác thực qua log là loại lỗi mà không
   ai phát hiện cho tới khi đã quá muộn, trong khi chi phí phòng ngừa chỉ là năm
   dòng mã và một bài test.

4. **Phân loại lỗi bằng `SQLState` trước khi bọc.** Ba nguyên nhân hoàn toàn khác
   nhau — CSDL biến mất (`08xxx`), bảng không tồn tại (`42P01`), hết bộ nhớ phía
   Java — hiện cùng ra một `IOException` với thông điệp giống nhau. Ca "bảng chưa
   tồn tại" đặc biệt đáng tách ra, vì hành động sửa rất cụ thể và rất khác: chạy
   [`PostgresImportRunner`](./PostgresImportRunner.md). Một thông điệp
   *"bảng `documents` chưa tồn tại — hãy chạy `PostgresImportRunner` trước"* biến
   một lần tra cứu tài liệu thành một dòng log tự giải thích.

5. **Đưa số tài liệu vào `describe()`.** `isAvailable()` vừa chạy `count(*)` xong
   rồi vứt kết quả đi. Ghi lại con số đó và trả về
   `"PostgreSQL @ ... (31.030 tài liệu)"` khiến dòng log khởi động trả lời được
   **hai** câu hỏi thay vì một: dữ liệu đến từ đâu, **và** có phải bản đầy đủ
   không. Với một hệ thống có bốn tầng dự phòng, việc phân biệt "corpus 31.030
   trang" với "40 tài liệu seed" ngay ở dòng log đầu tiên tiết kiệm rất nhiều
   phỏng đoán khi kết quả tìm kiếm trông lạ.
