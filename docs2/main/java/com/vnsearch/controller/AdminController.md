...
|---|---|---|
| **Gọi đúng tên lỗ hổng: SSRF, không phải "thiếu xác thực"** | 10/10 | Phân biệt chính xác giữa một API mở và một máy chủ trở thành proxy có đặc quyền mạng |
| **Ba lớp bảo vệ, mỗi lớp một mô hình đe doạ** | 10/10 | Bỏ lớp nào cũng mất một thứ riêng — phép thử phân biệt phòng thủ chiều sâu thật với lặp lại |
| **Lớp ② không tin lớp ①** | 10/10 | *"Một phép sai sót của chính người vận hành cũng không nên tải được metadata đám mây"* |
| **Kiểm SSRF trước khi vào hàng đợi** | 10/10 | Tính "toàn phần hoặc không" cho một thao tác không có giao dịch, kèm lý do rõ |
| **Đặt trần kèm một lối thoát** | 10/10 | Chuyển nhu cầu lớn sang kênh phù hợp hơn thay vì để người dùng tự tìm cách lách |
| Thông báo lỗi tiếng Việt, nêu đúng con số | 9/10 | Không dùng mặc định của Bean Validation; trừ điểm vì không nói làm gì tiếp |
| **Kiểm thử controller** | **0/10** | Xoá vòng lặp `validate` sẽ mở lại SSRF mà **mọi test hiện có vẫn xanh** |
| **`POST /reindex` chặn ~40 giây** | **4/10** | Vượt timeout mặc định của nhiều client; không jobId, không tra được trạng thái, không chống gọi đồng thời |
| Dừng ở seed sai đầu tiên | 5/10 | 50 seed với 3 seed sai ⇒ ba vòng thử; trái ngược với cách `@Valid` gom mọi lỗi trong **cùng phương thức** |
| Hai trong ba hằng số không có Javadoc | 5/10 | `MAX_DEPTH_LIMIT = 10` gần như vô nghĩa (tăng theo hàm mũ) nhưng trông ngang hàng với `maxPages` |
| Mặc định nằm trần trong thân hàm | 5/10 | `maxDepth=3`, `maxPages=100` là số trần, ngay dưới ba hằng số có tên — thiếu nhất quán trong 40 dòng |
| `GET /stats` trả thẳng map của facade | 6/10 | Hợp đồng API do một lớp khác định nghĩa; rủi ro thấp vì cần ADMIN, nhưng không ai kiểm soát ở đây |
| Lối thoát không tới được người gọi API | 6/10 | `MultiDomainCrawlRunner` chỉ có trong Javadoc; người nhận 400 chỉ thấy *"maxPages tối đa 50000"* |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Test rằng controller **có gọi** `SeedUrlValidator`.** `SsrfProtectionTest` kiểm
   validator rất kỹ, nhưng nó kiểm lớp đó **độc lập** — nên xoá ba dòng vòng lặp ở
   `crawl()` sẽ mở lại nguyên vẹn lỗ hổng SSRF trong khi toàn bộ bộ test vẫn xanh.
   Đây là khoảng cách kinh điển giữa *"bộ lọc hoạt động"* và *"bộ lọc được dùng"*:
   ```java
   ...
   class AdminControllerTest {
       ...
       @ParameterizedTest
       @ValueSource(strings = {
               "http://169.254.169.254/latest/meta-data/",   // metadata dam may
               "http://localhost:5432/",                      // PostgreSQL noi bo
               "http://10.0.0.5/admin",                       // mang noi bo
               "http://127.0.0.1:8080/api/admin/stats",       // chinh no
               "file:///etc/passwd"})                         // giao thuc khac
       void seedTroVaoMangNoiBoPhaiBiChanVaKHONGtaoJob(String seedXau) throws Exception {
           mockMvc.perform(post("/api/admin/crawl")
                          .contentType(APPLICATION_JSON)
                          .content("{\"seedUrls\":[\"" + seedXau + "\"]}"))
                  .andExpect(status().isBadRequest());

           verify(facade, never()).startCrawl(any(), anyInt(), anyInt());
       }

       @Test
       void motSeedXauTrongDanhSachTotPhaiChanCA_JOB() throws Exception {
           mockMvc.perform(post("/api/admin/crawl")
                          .contentType(APPLICATION_JSON)
                          .content("{\"seedUrls\":[\"https://vnexpress.net\","
   ...
   ```
   Dòng `verify(facade, never())` quan trọng ngang mã trạng thái: nó ghim đúng tính
   chất *"một seed xấu không được đi vào hàng đợi rồi mới bị phát hiện"* mà bình
   luận ở dòng 88–89 đặt ra.

2. **Gom mọi seed sai vào một phản hồi, thay vì dừng ở cái đầu tiên.** Trong cùng
   một phương thức đang có hai cơ chế hợp thức hoá với hai hành vi báo lỗi khác
   nhau: `@Valid` gom mọi trường sai, còn vòng lặp SSRF dừng ngay. Với 50 seed cho
   phép mỗi lần, chênh lệch này là thật:
   ```java
   ...
   for (String seedUrl : request.seedUrls()) {
       try {
           ...
           loi.add(seedUrl + " — " + e.getMessage());
       }
   }
   if (!loi.isEmpty()) {
       // Gom TAT CA, cung cach @Valid xu ly cac truong sai: nguoi gui 50 seed
       // khong nen phai sua va gui lai nhieu lan.
       throw new IllegalArgumentException(
               "Cac seed URL bi tu choi (" + loi.size() + "/" + request.seedUrls().size()
                       + "): " + String.join("; ", loi));
   }
   ```

3. **Cho `POST /reindex` cùng mô hình bất đồng bộ với `POST /crawl`, và đưa lối
   thoát vào thông báo lỗi.** Hiện `reindex` chặn khoảng 40 giây — vượt timeout mặc
   định của nhiều client HTTP — nên người gọi có thể nhận timeout trong khi thao tác
   **vẫn đang chạy**, không có cách nào biết nó thành công hay không, và không gì
   ngăn họ gọi lại lần thứ hai chồng lên lần thứ nhất:
   ```java
   @PostMapping("/reindex")
   public ResponseEntity<Map<String, String>> reindex() {
       return reindexDangChay.compareAndSet(false, true)
               ? ResponseEntity.accepted().body(Map.of(          // 202, khong phai 200
                       "jobId", facade.startReindex(),            // cung khuon voi /crawl
                       "status", "STARTED"))
               : ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of(   // 409
                       "status", "DANG_CHAY",
                       "message", "Mot lan reindex khac dang chay. Tra trang thai tai"
                               + " GET /api/admin/crawl/{jobId}/status."));
   }
   ```
   Song song, thông báo của `@Max` nên mang theo lối thoát mà Javadoc đã nêu — nó
   hiện chỉ tồn tại trong mã nguồn, nơi người gọi API không bao giờ đọc tới:
   ```java
   @Max(value = MAX_PAGES_LIMIT,
        message = "maxPages toi da " + MAX_PAGES_LIMIT + " qua API. Muon crawl lon hon,"
                + " dung MultiDomainCrawlRunner tu dong lenh: no chay o tien trinh rieng"
                + " nen khong an tai nguyen cua may chu dang phuc vu tim kiem, va nguoi"
                + " chay nhin thay tien do de dung lai khi can.")
   ```
