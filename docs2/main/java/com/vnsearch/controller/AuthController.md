...
|---|---|---|
| **Ba mức đóng phiên cho ba nỗi lo khác nhau** | 10/10 | Mỗi endpoint gắn với một tình huống người dùng cụ thể, không có endpoint nào thừa |
| **Đổi mật khẩu đóng mọi phiên khác** | 10/10 | *"Không đóng thì kẻ kia vẫn ở trong, và người dùng **tưởng mình đã an toàn**"* — lỗi phổ biến nhất về đổi mật khẩu |
| **`/register` không có trường `role`** | 10/10 | Chống mass assignment bằng **kiểu dữ liệu**, không bằng chú giải hay kiểm tra lúc chạy |
| **Token không nằm trong `Authentication`** | 10/10 | *"Một thứ bí mật nằm trong đối tượng được truyền khắp nơi thì sớm muộn cũng vào log"* |
| **`/me` lấy nguồn sự thật từ máy chủ** | 10/10 | Nêu đúng hậu quả của việc tin `localStorage`: nó **che mất** việc quyền đã bị thu hồi |
| Trường `via` phân biệt hai đường xác thực | 9/10 | Trung thực thay vì bịa ra một `User` giả cho khoá API |
| `closedOtherSessions` trả về số phiên đã đóng | 9/10 | Biến một thao tác vô hình thành bằng chứng người dùng dùng được |
| Nêu quan hệ nhân quả với việc tắt CSRF | 9/10 | *"Có cơ sở chứ không phải cho tiện"*; trừ điểm vì không nêu đánh đổi với XSS |
| **Kiểm thử controller** | **0/10** | **Không một test nào**, kể cả `/register` từ chối trường `role` — hai dòng test |
| **Lập luận `@Size` chống DoS sai cơ chế** | **4/10** | BCrypt chỉ dùng **72 byte đầu** nên chi phí là hằng số; chặn độ dài không chống được tấn công CPU |
| Giới hạn 72 byte của BCrypt không được nêu | 4/10 | Hai mật khẩu khác nhau từ ký tự thứ 73 được coi là như nhau — hành vi gây bất ngờ thật sự |
| Ba nhánh `authentication == null` là mã chết | 5/10 | Vô hại nhưng che giấu giả định "luật ở tệp khác đang bảo vệ tôi", và ba nhánh có hai hành vi |
| `/me` trả hai hình dạng cho cùng một trường | 5/10 | Giao diện phải xử lý cả hai, và `Map<String, Object>` không báo gì |
| Không có hạn mức riêng cho `/login` | 5/10 | Endpoint đắt nhất hệ thống (~200 ms CPU, không cache được, công khai) dùng chung hạn mức với `/api/search` |
| `Credentials` dùng chung hai endpoint | 6/10 | Hiện hợp lý, nhưng một trường thêm vì `/login` sẽ tự động xuất hiện ở `/register` |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Test bất biến chống mass assignment — hai dòng, và nó bảo vệ thứ quan trọng
   nhất trong tệp.** Javadoc nói rõ vì sao `role` không có trong `Credentials`, nhưng
   không gì ngăn ai đó thêm nó vào sau này vì một nhu cầu hợp lý ở `/login`:
   ```java
   ...
   class AuthControllerTest {
       ...
       @Test
       void dangKyPhaiBoQuaTruongRoleGuiLen() throws Exception {
           when(users.register(any(), any())).thenReturn(nguoiDung("ke-thu", Role.USER));

           mockMvc.perform(post("/api/auth/register").contentType(APPLICATION_JSON)
                          .content("{\"username\":\"ke-thu\",\"password\":\"mat-khau-dai\","
                                  + "\"role\":\"ADMIN\"}"))
                  .andExpect(status().isCreated())
                  .andExpect(jsonPath("$.role").value("USER"));

           // UserService.register KHONG nhan tham so role — day moi la bao dam that.
           verify(users).register("ke-thu", "mat-khau-dai");
       }

       @Test
       void doiMatKhauDongMoiPhienKHACnhungGIUphienHienTai() throws Exception {
           mockMvc.perform(post("/api/auth/password")
                          .header("Authorization", "Bearer token-hien-tai")
                          .contentType(APPLICATION_JSON)
                          .content("{\"currentPassword\":\"cu\",\"newPassword\":\"moi\"}")
                          .with(user("an")))
                  .andExpect(status().isOk());

           verify(sessions).revokeAllForExcept("an", "token-hien-tai");
       }

       @Test
       void dangXuatVoiTokenKhongTonTaiVanTra204() throws Exception {
           mockMvc.perform(post("/api/auth/logout")
                          .header("Authorization", "Bearer token-da-chet"))
                  .andExpect(status().isNoContent());
       }
   }
   ```
   Test thứ ba đáng có riêng: lời hứa *"trả 204 kể cả khi token đã hết hạn"* đã từng
   bị [`SecurityConfig`](../config/SecurityConfig.md) phủ quyết âm thầm, và không có
   gì ngăn điều đó tái diễn — dù test này chỉ bắt được nửa vấn đề (nó bỏ qua chuỗi
   filter), nên nên có thêm một test đầu-cuối cho phần còn lại.

2. **Hạn mức tần suất riêng cho `/login`, và sửa lại lập luận về `@Size`.**
   `/login` tốn ~200 ms CPU thuần, không cache được, và công khai — ba tính chất
   cộng lại làm nó thành mục tiêu tự nhiên cho một cuộc tấn công cạn CPU. Việc chặn
   độ dài **không** giúp gì, vì BCrypt chỉ đọc 72 byte đầu nên chi phí băm là hằng
   số bất kể đầu vào dài bao nhiêu:
   ```java
   // Trong SecurityConfig
   @Bean
   public FilterRegistrationBean<RateLimitFilter> loginRateLimitFilter(
           @Value("${app.security.rate-limit.login-per-minute:10}") int soMoiPhut, ...) {
       var dangKy = new FilterRegistrationBean<>(new RateLimitFilter(soMoiPhut, bat, tinProxy));
       dangKy.addUrlPatterns("/api/auth/login", "/api/auth/register");
   ...
   ```java
   /**
    * <p><b>Vi sao chan do dai o day.</b> KHONG phai de chan tan cong CPU: BCrypt
    * chi dung 72 BYTE DAU cua mat khau, nen chi phi bam la HANG SO (~200 ms) du
    * dau vao dai 8 hay 100.000 ky tu. Chan do dai chi tiet kiem chi phi phan
    * tich JSON — nho hon nhieu.
    *
    * <p>Thu that su chan duoc tan cong can CPU la han muc tan suat RIENG cho
    * {@code /api/auth/login} (xem {@code SecurityConfig.loginRateLimitFilter}):
    * 200 ms CPU thuan × 120 request/phut = 24 giay CPU moi phut tu MOT dia chi.
    *
    * <p><b>Gioi han 72 byte cung la mot bat ngo can biet:</b> hai mat khau khac
    * nhau tu ky tu thu 73 duoc BCrypt coi la NHU NHAU.
    */
   ```
   Lưu ý ranh giới: khoá tạm tài khoản của [`UserService`](../auth/UserService.md)
   chặn theo **tài khoản**, nên nó không giúp gì khi kẻ tấn công bắn vào 1.000 tên
   khác nhau — hai cơ chế bổ sung cho nhau chứ không thay thế.

3. **Định kiểu phản hồi `/me`, và xoá ba nhánh mã chết.** Trường `user` hiện có hai
   hình dạng tuỳ `via`, nên giao diện phải xử lý cả hai mà không có gì trong hợp đồng
   nói ra điều đó:
   ```java
   public record MeResponse(boolean authenticated, String via, User.PublicView user) {

       /** Phien do API key cap: khong co tai khoan dung sau, nen dung mot PublicView
        *  toi thieu thay vi mot hinh dang JSON KHAC — giao dien chi phai xu ly MOT kieu. */
       static MeResponse quaApiKey(String ten) {
           return new MeResponse(true, "api-key", User.PublicView.choApiKey(ten, Role.ADMIN));
       }
   }

   @GetMapping("/me")
   public MeResponse me(Authentication authentication) {
       // KHONG con nhanh `authentication == null`: /api/auth/** da la .authenticated()
       // trong SecurityConfig, nen nhanh do la ma CHET. Giu no lai che giau giay to
       // rang tinh dung cua endpoint nay phu thuoc vao mot luat o tep khac.
       String name = authentication.getName();
       return users.find(name)
               .map(u -> new MeResponse(true, "session", u.toPublic()))
               .orElseGet(() -> MeResponse.quaApiKey(name));
   }
   ```
   Việc xoá ba nhánh `authentication == null` là chủ ý: chúng vô hại nhưng tạo cảm
   giác sai rằng controller tự bảo vệ mình, trong khi thực tế nó dựa hoàn toàn vào
   bảng phân quyền — và ba nhánh đó hiện có **hai** hành vi khác nhau (401 ở hai
   chỗ, 200 với `closedSessions: 0` ở chỗ thứ ba), tức là chúng còn không nhất quán
   với nhau.
