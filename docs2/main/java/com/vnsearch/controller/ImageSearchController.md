...
|---|---|---|
| **Xếp hạng gián tiếp qua trang chứa ảnh** | 10/10 | Không một dòng mã xếp hạng mới, và mọi cải tiến ở tầng văn bản tự động cải thiện tab ảnh |
| **Kể lại hằng số lạc hậu sau một lần đổi thiết kế** | 10/10 | *"Con số đó có căn cứ đúng vào lúc ấy"* — mẫu lỗi đáng học nhất gói, và triệu chứng trông như corpus nhỏ |
| **Ba bước đầu không phụ thuộc `page`** | 10/10 | Bất biến trung tâm, nhắc lại ba lần, kèm mô tả chính xác kiểu hỏng *"vừa lặp vừa thiếu"* |
| **Ca biên của `hasMore`** | 10/10 | Nêu đúng chỗ suy luận phía client sai: khi tổng chia hết cho `pageSize` |
| **`pagesScanned` phân biệt hai ca rỗng** | 10/10 | Hai ca trông giống hệt nhau nhưng cần hai lời khuyên khác hẳn |
| Lý do dùng `LinkedHashMap` được ghi | 10/10 | Lần duy nhất trong dự án lý do được viết ra — và cũng là lần mất nó gây hậu quả nặng nhất |
| Sắp xếp ổn định, chỉ phân hoạch hai nhóm | 9/10 | *"Không phải thay thế phép xếp hạng của máy tìm kiếm"* — ranh giới đúng cho một tín hiệu yếu |
| Chi phí nâng trần được lập luận | 9/10 | Hai lý do cụ thể (cache + tra bảng băm), không phải "chắc là rẻ" |
| **Kiểm thử** | **0/10** | **Không một test nào**, kể cả cho bất biến phân trang — thứ đã hỏng thật một lần |
| **Hai hằng số phải bằng nhau, hai khai báo độc lập** | **4/10** | Ràng buộc được ghi nhưng không được thực thi — đúng loại lỗi vừa xảy ra, chỉ ở dạng khác |
| **Javadoc trỏ vào hằng số không tồn tại** | **4/10** | `{@link #FETCH_PAGE_MULTIPLIER}` sẽ làm `javadoc` báo lỗi, và cho thấy tài liệu lệch với mã |
| `totalResults` hứa nhiều hơn thực tế | 5/10 | Bị chặn ở 300; giao diện hiện "300 ảnh" cho một truy vấn khớp 5.000 trang |
| Trả `Map<String, Object>` | 5/10 | Tám khoá trong `results` + sáu khoá siêu dữ liệu, không hợp đồng nào — trong khi `/api/search` dùng record |
| Không gì ngăn thêm tiêu chí sắp xếp | 6/10 | Bình luận cảnh báo, nhưng một dòng `.thenComparing(...)` sẽ vứt bỏ toàn bộ xếp hạng mà không lỗi nào |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Thực thi ràng buộc `MAX_SCANNED_PAGES == MAX_TOTAL_IMAGES` thay vì chỉ ghi
   nó.** Javadoc phát biểu rất rõ rằng hai đại lượng này có quan hệ một-một kể từ
   khi [`ImageStore`](../crawler/modular/ImageStore.md) chỉ giữ một ảnh mỗi trang —
   nhưng chúng vẫn là hai khai báo độc lập, tức là đúng cấu hình đã tạo ra lỗi ở mục
   2, chỉ chuyển sang một dạng khác:
   ```java
   /**
    * Tran so anh gom cho mot truy van.
    *
    * <p>Tu khi {@link ImageStore} chi giu MOT anh moi trang, so trang quet va
    * so anh toi da la CUNG MOT dai luong — nen chi khai bao MOT lan. Giu hai
    * hang so rieng chinh la cau hinh da lam MAX_SCANNED_PAGES=60 tro thanh mot
    * tran cung cho ca tab anh sau khi thiet ke doi.
    */
   private static final int MAX_IMAGES = 300;
   ```
   Nếu quan hệ một-một có ngày không còn đúng (ví dụ `ImageStore` quay lại giữ nhiều
   ảnh mỗi trang), việc phải **tách** một hằng số thành hai là một hành động có ý
   thức — trong khi giữ hai hằng số sẵn khiến việc chúng lệch nhau xảy ra âm thầm.

2. **Test bất biến phân trang — thứ chỉ kiểm được ở tầng controller.** Tính ổn định
   của phân trang là tính chất của cách **bốn bước phối hợp**, không thuộc về lớp
   nào riêng lẻ, nên không test hiện có nào chạm tới nó. Và theo chính Javadoc của
   [`FeedController`](./FeedController.md), đây là lỗi tab Hình ảnh **đã vấp**:
   ```java
   ...
   class ImageSearchControllerTest {
       ...
       void haiLoLienTiepKhongLapVaKhongThieu() throws Exception {
           dungDuLieuGia(87);   // 87 anh

           List<String> lo1 = urlCuaLo(1, 30);
           List<String> lo2 = urlCuaLo(2, 30);
           List<String> lo3 = urlCuaLo(3, 30);

           List<String> tatCa = Stream.of(lo1, lo2, lo3).flatMap(List::stream).toList();
           assertEquals(87, tatCa.size(), "khong duoc thieu anh nao");
           assertEquals(87, Set.copyOf(tatCa).size(), "khong duoc lap anh nao");
       }

       @Test
       void hasMoreSaiKhiTongChiaHetChoPageSize() throws Exception {
           dungDuLieuGia(60);   // 60 anh, pageSize 30 => chia het
           mockMvc.perform(get("/api/images").param("q", "a").param("page", "2"))
                  .andExpect(jsonPath("$.hasMore").value(false));
           // Neu de giao dien tu suy tu results.length === pageSize, no se tuong
           // con nua va goi them mot lan vo ich — ca bien ~3% moi truy van.
       }

       @Test
       void pagesScannedPhanBietHaiCaRong() throws Exception {
           when(facade.search(any(), anyInt(), anyInt())).thenReturn(phanHoiCo(142));
           when(imageStore.forPages(any(), anyInt())).thenReturn(List.of());

           mockMvc.perform(get("/api/images").param("q", "a"))
                  .andExpect(jsonPath("$.results").isEmpty())
                  .andExpect(jsonPath("$.pagesScanned").value(142));
           // pagesScanned > 0 + results rong => "hay chay crawl", KHONG phai
           // "khong tim thay" — hai loi khuyen khac han nhau.
       }
   }
   ```

3. **Định kiểu phản hồi, và sửa tham chiếu Javadoc hỏng.** Endpoint này trả về một
   `Map` với **sáu** khoá siêu dữ liệu và **tám** khoá cho mỗi kết quả — hợp đồng
   phức tạp nhất trong cả API, và là hợp đồng duy nhất không được định kiểu, trong
   khi `/api/search` ngay bên cạnh dùng [`SearchResponse`](../model/SearchResponse.md):
   ```java
   public record ImageSearchResponse(
           String query, List<ImageItem> results,
           int page, int pageSize, int totalResults, boolean hasMore,
           int pagesScanned, long timeTakenMs) {

       public record ImageItem(
               String imageUrl, String pageUrl, String pageTitle, String host,
               String altText, Integer width, Integer height, boolean missingAlt) {}
   }
   ```
   Đồng thời, `{@link #FETCH_PAGE_MULTIPLIER}` ở dòng 42 trỏ vào một hằng số **không
   tồn tại** — nó sẽ làm `javadoc` báo lỗi, và quan trọng hơn, nó là dấu hiệu Javadoc
   lớp chưa được cập nhật cùng lúc với mã. Câu đó nên trỏ sang cơ chế thật đang giảm
   nhẹ thiên lệch:
   ```java
   /**
    * <p><b>He qua phai biet:</b> mot trang rat lien quan nhung khong co anh se
    * khong dong gop gi. Chieu nguoc lai — mot trang nhieu anh chiem het luoi —
    * da duoc {@link ImageQuality} xu ly tu luc crawl: moi trang chi giu MOT anh,
    * nen quan he trang-anh la mot-mot va khong trang nao chiem duoc nhieu hon
    * mot o.
    */
   ```
