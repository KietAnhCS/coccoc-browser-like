...
|---|---|---|
| Chất lượng ý tưởng | 10/10 | "Chứng minh trường thừa" và "tổng tích luỹ biến dãy bất kỳ thành đơn điệu" — hai ý tưởng ở tầm nguyên tắc, không phải thủ thuật |
| Ép bất biến | 10/10 | Hàng rào ném **kèm giải thích vì sao nguy hiểm** — thông điệp lỗi mẫu mực |
| Nhận diện mẫu chung | 10/10 | Chỉ ra `rowPtr` của CSR xuất hiện hai lần độc lập trong đồ án — đúng dấu hiệu của kỹ thuật nền tảng |
| Lập luận kiến trúc | 10/10 | Từ chối GZIP để giữ truy cập ngẫu nhiên; và nêu rõ [`CompressedText`](./CompressedText.md) chọn ngược lại vì bài toán ngược lại |
| Thiết kế không dữ liệu thừa | 10/10 | `sizes` suy từ `offsets`, `offsets` có canh biên, `positions` không cần vị trí byte — ba mảnh khớp nhau không dư gì |
| Hiệu quả cài đặt | 9/10 | Một lượt, không sắp xếp, không sao chép mảng vị trí (hưởng lợi từ quyết định `int[]` ở [`Posting`](./Posting.md)) |
| Bất biến của chính record | 5/10 | Ba mảng `byte[]` sửa được từ bên ngoài; `equals`/`hashCode` sinh sẵn so theo tham chiếu — **cùng lỗi mà `Posting` đã sửa nhưng ở đây chưa** |
| Khả năng truy cập ngẫu nhiên | 5/10 | Lớp tồn tại để giữ truy cập ngẫu nhiên **theo term**, nhưng bên trong một term vẫn phải giải nén toàn bộ |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Tự viết `equals`/`hashCode` như [`Posting`](./Posting.md) đã làm.** Record
   này có **ba** trường mảng, nên `equals` sinh sẵn so cả ba theo tham chiếu —
   hai `CompressedPostings` nén từ cùng dữ liệu sẽ "khác nhau". Đây đúng là lỗi
   mà `Posting` đã nhận ra và sửa; ở đây nó chưa được sửa, dù rủi ro giống hệt
   (test lưu–nạp của [`IndexPersistence`](./IndexPersistence.md) sẽ so nhầm):
   ```java
   @Override
   public boolean equals(Object o) {
       return o instanceof CompressedPostings that
               && count == that.count
               && Arrays.equals(docIds, that.docIds)
               && Arrays.equals(offsets, that.offsets)
               && Arrays.equals(positions, that.positions);
   }
   @Override
   public int hashCode() {
       return 31 * (31 * (31 * count + Arrays.hashCode(docIds))
               + Arrays.hashCode(offsets)) + Arrays.hashCode(positions);
   }
   ```
2. **Thêm phương thức `docIdAt(int i)` giải mã một phần.** Hiện muốn biết docId
   thứ 5 phải gọi `toPostings()` và dựng lại toàn bộ. Một hàm giải mã chỉ mảng
   `docIds` (bỏ qua `positions` — phần chiếm hơn nửa dung lượng) sẽ phục vụ giai
   đoạn **giao posting list**, vốn không cần vị trí:
   ```java
   /** Chỉ giải nén docId — đủ cho phép giao, không đụng tới positions. */
   public int[] docIdsOnly() {
       return VByteCodec.decodeSorted(docIds, count);
   }
   ```
   Tiết kiệm ~70% công giải nén cho đường đi phổ biến nhất.
3. **Thêm điểm nhảy để [`PostingCursor`](./PostingCursor.md) đọc thẳng dạng
   nén.** Đây là bước còn thiếu để khép kín chuỗi tối ưu của cả gói: hiện tầng
   truy vấn vẫn phải gọi `toPostings()` và cấp phát 1,59 triệu đối tượng
   `Posting` — đúng thứ mà cursor sinh ra để tránh. Lớp này đã cố ý giữ mỗi term
   là một đơn vị độc lập (mục 4); bước tiếp theo là giữ mỗi **khối 128 posting**
   trong một term cũng độc lập. Xem [`VByteCodec`](./VByteCodec.md) đề xuất 3.
