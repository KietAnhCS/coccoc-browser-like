...
|---|---|---|
| Ép bất biến | 10/10 | `lastDocId` biến một giả định thành phép kiểm tra; giữ bất biến **miễn phí**, không tốn sort nào |
| Chất lượng thông điệp lỗi | 10/10 | Nói cả cái gì sai, bất biến nào bị đe doạ, và phải làm gì |
| Tài liệu hoá sự cố thật | 10/10 | Ví dụ "không trung thực" là sự cố **đã xảy ra**, ghi lại đầy đủ — hiếm và rất đáng giá |
| Thiết kế cho song song hoá | 10/10 | Tách `indexableText` public để tách từ song song, nạp tuần tự — đúng ranh giới |
| Ranh giới cấu trúc dữ liệu | 10/10 | `List<Integer>` cho dữ liệu sống ngắn, `int[]` cho dữ liệu sống lâu |
| Khử trùng lặp | 10/10 | Gộp ba bản sao `binarySearch` về một; ghi rõ lý do |
| Quản lý trạng thái dẫn xuất | 9/10 | `recomputeDerivedState` gom một chỗ, có cảnh báo hậu quả nếu quên |
| Bảo vệ đóng gói | 6/10 | `getAllDocuments`/`getAllTerms` bọc unmodifiable, nhưng **`getPostings` và `getPositions` vẫn trả cấu trúc sửa được** |
| An toàn đa luồng | 7/10 | Chỉ đọc thì an toàn; ghi thì không, và điều này **không được ghi trong Javadoc lớp** |
| Khả năng kiểm thử | 6/10 | 82 dòng test cho 458 dòng mã; bất biến trung tâm và trạng thái dẫn xuất chưa có ca riêng |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Bọc `getPostings` bằng `Collections.unmodifiableList`.** Đây là lỗ hổng
   cuối cùng của việc bảo vệ bất biến: lớp đã ép rất chặt ở **cửa vào**
   (`lastDocId`) nhưng vẫn phát ra danh sách **sửa được** ở cửa ra. Một lời gọi
   `sort()` hay `remove()` vô ý ở tầng truy vấn phá bất biến vĩnh viễn cho tới
   lần build lại — và phá theo cách im lặng. Chi phí: một lớp bọc mỏng,
   ~0 ns. Cũng là đề xuất 2 của [`SearchIndex`](./SearchIndex.md).
2. **Cho `addDocument` gọi chung một nguồn sự thật với `recomputeDerivedState`.**
   Hiện `addDocument` cập nhật `totalTokens`/`lastDocId` **trực tiếp** còn
   `importData` gọi `recomputeDerivedState` — hai đoạn mã phải luôn khớp nhau,
   nhưng không có gì bảo đảm. Một test bất biến rẻ hơn nhiều so với việc tái cấu
   trúc:
   ```java
   @Test
   void hopCapNhatTangDanKhopVoiTinhLaiToanBo() {
       InvertedIndex idx = dungChiMuc50TaiLieu();
       double truoc = idx.getAverageDocLength();
       idx.recomputeDerivedStateForTest();          // package-private cho test
       assertEquals(truoc, idx.getAverageDocLength(), 1e-9);
   }
   ```
3. **Ghi rõ hợp đồng đa luồng vào Javadoc lớp.** Hiện thông tin này nằm rải rác:
   [`TermDictionary`](./TermDictionary.md) nói "chỉ dùng trong `addDocument`, mà
   việc dựng chỉ mục luôn đơn luồng", [`MaxWeightSegmenter`](./MaxWeightSegmenter.md)
   nói tầng truy vấn chạy đa luồng. Nhưng chính `InvertedIndex` — nơi bốn
   `LinkedHashMap` không thread-safe nằm — lại không nói gì:
   > *"**Đồng thời:** an toàn khi chỉ đọc sau khi build xong. Mọi phương thức
   > ghi (`addDocument`) phải chạy đơn luồng. Mẫu dùng đúng: dựng xong một chỉ
   > mục mới hoàn chỉnh rồi gán bằng tham chiếu `volatile`."*
