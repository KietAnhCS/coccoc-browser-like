...
|---|---|---|
| Chất lượng thuật toán | 10/10 | Galloping là lựa chọn đúng — thích nghi theo khoảng cách thật, không bao giờ tệ hơn two-pointer |
| Đúng mẫu thiết kế | 10/10 | Iterator có `skipTo` là mẫu chuẩn của mọi máy tìm kiếm nghiêm túc (Lucene dùng `DocIdSetIterator.advance`) |
| Không cấp phát | 10/10 | Toàn bộ giao diện thiết kế quanh việc không tạo rác; chỉ `of()` cấp phát một lần |
| Chọn giá trị lính canh | 10/10 | `NO_MORE = Integer.MAX_VALUE` khiến điều kiện dừng của vòng giao viết được thành một dòng |
| Tài liệu hoá | 10/10 | Javadoc có **số liệu so sánh cụ thể** (4.005 → 48 bước) và giải thích cả hai pha của galloping |
| Hợp đồng được ghi rõ | 8/10 | Bốn tình huống của `skipTo` được ghi, nhưng "không lùi" chỉ suy ra được từ test chứ Javadoc không nói |
| Khả năng kiểm thử | 10/10 | 9 ca, có kiểm chứng đối sánh với cài đặt ngây thơ — chuẩn mực để noi theo |
| Bề mặt API | 8/10 | 6 phương thức là hợp lý; `size()` hơi lệch trách nhiệm nhưng mở khoá tối ưu lớn |

**Ba đề xuất nâng lên mức sản phẩm:**

1. **Ghi hợp đồng "không lùi" vào Javadoc của `skipTo`.** Hiện nó chỉ tồn tại
   trong `skipToBackwardsIsNoOp` — người viết cài đặt mới đọc giao diện sẽ không
   biết, và một cài đặt cho phép lùi có thể gây vòng lặp vô hạn ở tầng giao:
   ```java
   /**
    * …
    * <p>Nếu {@code targetDocId} nhỏ hơn hoặc bằng vị trí hiện tại, cursor
    * <b>giữ nguyên</b> và trả về {@code true} — cursor là con trỏ MỘT CHIỀU.
    */
   ```
2. **Thêm bộ test hợp đồng dùng chung.** Hiện chỉ có một cài đặt, nhưng giao
   diện được thiết kế để có nhiều (một cursor đọc thẳng từ
   [`CompressedPostings`](./CompressedPostings.md) là bước tiếp theo tự nhiên và
   sẽ xoá bỏ hoàn toàn việc giải nén thành `List<Posting>`). Chuyển 9 ca hiện có
   thành lớp trừu tượng nhận cài đặt qua một hàm là ~10 dòng công.
3. **Cho `skipTo` một biến thể trả về docId thay vì `boolean`.** Mẫu ở mục 4.1
   phải viết `if (!c.skipTo(x) || c.docId() != x)` — hai lời gọi cho một câu hỏi.
   Lucene giải bằng `int advance(int target)` trả thẳng docId đã đứng vào. Nó bỏ
   được một nhánh trong vòng lặp nóng nhất của cả hệ thống:
   ```java
   /** Nhảy tới và trả về docId đã đứng vào, hoặc NO_MORE. */
   default int advance(int targetDocId) {
       return skipTo(targetDocId) ? docId() : NO_MORE;
   }
   ```
