...
**Phạm vi:** **9 file** trong `com.vnsearch.eval`.

> **Tự kiểm chứng:**
> ```bash
> find search-engine/src/main/java/com/vnsearch/eval -name "*.java" | wc -l
> ```
...
| **8** | **`MemoryBreakdown`** | **174** | Đo **bộ nhớ thật** của chỉ mục, tách theo từng thành phần |
| **9** | **`TokenizerBenchmark`** | **294** | Đo **trực tiếp** hai thay đổi của bộ tách từ trên corpus thật |

> ⚠️ **Hai dòng 8–9 bị thiếu trong bản trước của bảng này**, và bản trước ghi
> *"7 file"* trong khi package có **9**. Cả hai file bị bỏ sót đều là **công cụ
> đo**, tức đúng thứ mà một tầng tên là "đánh giá" phải có.

**Vì sao hai file này đáng nằm trong bảng, không phải phụ lục.** Chúng đại diện
cho hai loại đo khác hẳn phần còn lại của nhóm:

| | Nhóm 1–7 đo gì | `MemoryBreakdown` | `TokenizerBenchmark` |
|---|---|---|---|
| Đối tượng | **chất lượng** kết quả trả về | **bộ nhớ** chiếm | **tốc độ + độ lệch kết quả** |
| Câu hỏi | "xếp hạng có đúng không?" | "phần nào chiếm nhiều nhất?" | "nhanh hơn bao nhiêu, và **tách khác đi ở đâu**?" |

Mỗi file mang một bài học phương pháp luận đáng ghi vào báo cáo:

- **`MemoryBreakdown`** ra đời vì một ước lượng **chưa đo**: bản rà soát trước
  đoán rằng bỏ `bodyText` khỏi chỉ mục sẽ giảm 60–70 % bộ nhớ. Javadoc ở
  `MemoryBreakdown.java:6-12` nói thẳng: *"quyết định tối ưu dựa trên con số chưa
  đo là cách nhanh nhất để bỏ công vào đúng chỗ không đâu"*. Nó cũng **thừa nhận
  giới hạn của chính phép đo** (`:14-20`): `System.gc()` chỉ là một lời đề nghị,
  nên sai số là ngẫu nhiên — đủ để trả lời *"phần nào lớn nhất"*, không đủ để
  công bố con số tuyệt đối.

- **`TokenizerBenchmark`** tách **hai** thay đổi bị trộn lẫn: thuật toán mới
  (quy hoạch động) **và** từ điển mới (185.000 mục thay cho 154). Nó cài lại
  nguyên văn Longest Matching cũ nhưng cho chạy trên **cùng từ điển mới**
  (`TokenizerBenchmark.java:19-24`), nhờ đó bảng kết quả đo **riêng phần thuật
  toán**. Đây chính là nguyên tắc *"không bao giờ đổi hai biến cùng lúc"* mà
  [`02-index/00-SO-DO-TU-DUY.md`](../02-index/00-SO-DO-TU-DUY.md) §8.3 nêu ra —
  ở đây nó được áp dụng thật.

Và câu hỏi thứ hai của `TokenizerBenchmark` mới là câu quan trọng:
**"nhanh hơn mà kết quả y hệt thì không đáng đổi thuật toán."**
...
> Đây chính là lý do `CandidateResolver` phải được **tách thành lớp riêng** thay vì để private trong `SearchEngineFacade` — xem [sơ đồ tư duy tầng truy vấn §9](../03-query/00-SO-DO-TU-DUY.md).
...
Câu 2 và 3 dẫn thẳng tới phát hiện lỗi thang đo 1000× — xem [sơ đồ tư duy tầng xếp hạng §5](../04-ranking/00-SO-DO-TU-DUY.md).
...
| **Thứ đang được đo** — các cấu hình xếp hạng | [Sơ đồ tư duy tầng xếp hạng](../04-ranking/00-SO-DO-TU-DUY.md) |
| **Đường đi được dùng lại nguyên si** | [Sơ đồ tư duy tầng truy vấn](../03-query/00-SO-DO-TU-DUY.md) |
