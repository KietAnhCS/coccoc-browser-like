...
**Phạm vi:** 5 file trong `com.vnsearch.datastructure`, cộng `index/ArrayPostingCursor` + `index/PostingCursor` (được phân tích ở nhóm này vì bản chất là một cấu trúc duyệt).
...
> Cùng ý tưởng sentinel với phần tử canh biên `offsets[count+1]` của [`CompressedPostings`](../03-index/CompressedPostings.md).
...
| Các cấu trúc này được dùng ở đâu | [Crawler](../01-crawler/00-SO-DO-TU-DUY.md) · [Chỉ mục](../03-index/00-SO-DO-TU-DUY.md) · [Truy vấn](../04-query/00-SO-DO-TU-DUY.md) · [Xếp hạng](../05-ranking/00-SO-DO-TU-DUY.md) |
