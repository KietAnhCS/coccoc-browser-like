...
**Đây là một điểm yếu thiết kế thật:** lớp không tự bảo vệ bất biến của mình, mà phụ thuộc người gọi. Cùng loại vấn đề với [InvertedIndex §4.2](../03-index/InvertedIndex.md).
...
> Có thể duy trì `nnz` như một biến cộng dồn trong `set` để về $O(1)$ — cùng kỹ thuật `totalTokens` ở [InvertedIndex §3.2](../03-index/InvertedIndex.md). Nhưng vì `nnz()` chỉ gọi vài lần để báo cáo, $O(N)$ là hợp lý.
