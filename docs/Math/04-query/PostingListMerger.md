...
### 5.1 Phân tích độ phức tạp — và hai chỗ lãng phí

$$O\bigl(\underbrace{p_1}_{\text{vị trí của term đầu}} \times \underbrace{k}_{\text{số term}} \times (\underbrace{\log n}_{\texttt{getPositions}} + \underbrace{p_i}_{\texttt{contains}})\bigr)$$

**Lãng phí 1: `getPositions` gọi lại trong vòng lặp trong.**

`index.getPositions(phraseTerms.get(i), docId)` cho ra **cùng một kết quả** ở mọi lần lặp của `start` — nó không phụ thuộc `start`. Với term đầu xuất hiện 20 lần và cụm 3 từ, đó là $20 \times 2 = 40$ lần binary search thay vì 2.

**Cách sửa (rất rẻ):** lấy tất cả `positions` ra **trước** vòng lặp ngoài.

```java
List<List<Integer>> allPositions = new ArrayList<>(phraseTerms.size());
for (String term : phraseTerms) {
    allPositions.add(index.getPositions(term, docId));
}
```

**Lãng phí 2: `positions.contains(start + i)` là $O(p_i)$ tuyến tính.**

`List.contains` trên `ArrayList` quét tuần tự. Nhưng `positions` **đã sắp xếp tăng dần** (được tạo theo thứ tự token tăng dần trong `addDocument`) — nên có thể dùng `Collections.binarySearch` để về $O(\log p_i)$.

**Cách tốt nhất:** dùng chính **two-pointer** trên các danh sách vị trí, giống hệt §1 nhưng so sánh $pos_i$ với $pos_0 + i$. Khi đó độ phức tạp về $O(\sum_i p_i)$ — tuyến tính thay vì tích.

> Đây là một chỗ mà **cùng một kỹ thuật (two-pointer trên danh sách đã sắp xếp) được dùng ở tầng docId nhưng không được dùng ở tầng position** — một cơ hội tối ưu rõ ràng và nhất quán về mặt thiết kế.

### 5.2 Vì sao trong thực tế chưa thành vấn đề

Phrase search chỉ chạy trên các ứng viên **đã qua phép giao**, tức thường vài chục tài liệu. Và người dùng hiếm khi dùng dấu ngoặc kép. Đây là ví dụ tốt về **tối ưu theo tần suất sử dụng thật** — nhưng khác với `remove` $O(n)$ có chủ ý ở nhiều cấu trúc khác, ở đây sự lãng phí không có lợi ích bù lại nào (code không đơn giản hơn), nên nó là **nợ kỹ thuật** chứ không phải đánh đổi.
...
- Bất biến mà lớp này phụ thuộc: [InvertedIndex §4](../03-index/InvertedIndex.md)
- Người gọi: [CandidateResolver.md](CandidateResolver.md)
- Nguồn của `phraseTerms`: [QueryParser.md](QueryParser.md)
- Cùng dùng bất biến sắp xếp: [TfIdfScorer.md](../05-ranking/TfIdfScorer.md)
