...
Đây cũng là lý do `getAverageDocLength()` phải là $O(1)$ (xem [InvertedIndex §3.2](../02-index/InvertedIndex.md)) — nó được gọi cho **mọi** ứng viên.
...
Lớp thứ hai đúng là nơi lỗi "quên tính lại `totalTokens` khi nạp file" biểu hiện — xem [IndexPersistence §6](../02-index/IndexPersistence.md).
...
6. ~~**Không dùng trong sản phẩm.**~~ ✅ **Đã khắc phục.** Đoạn dưới là mã **cũ** — Facade từng chôn cứng lớp cụ thể:
   ...
   ```
   Xem [**02-FACTORY.md**](../08-design-patterns/02-FACTORY.md).
