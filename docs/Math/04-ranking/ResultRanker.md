...
`ResultRanker` nay **không còn kết hợp tín hiệu**. Nó chỉ gọi scorer và nhận về một điểm duy nhất — **`ResultRanker.java:97-109`**:
...
for (int docId : candidateDocIds) {
    ...
    scored.add(new ScoredCandidate(doc, prepared.score(docId), pageRank));  // pageRank chỉ để HIỂN THỊ
}
```

> ⚠️ **Bản trước của trang này ghi `scorer.score(queryTermFrequency, docId, index)`
> gọi thẳng trong vòng lặp.** API đã tách làm **hai pha**, và lý do là một tối ưu
> thật (`ResultRanker.java:94-96`):
>
> > Xem `RelevanceScorer#prepare`: **idf, trọng số truy vấn và tập tiếng của truy
> > vấn** trước đây đều bị tính lại cho **TỪNG** ứng viên.
>
> Đây là dạng bất biến vòng lặp kinh điển: phần **chỉ phụ thuộc truy vấn** bị kẹt
> trong vòng lặp **duyệt tài liệu**. Với $c$ ứng viên và $q$ term, chi phí idf đi
> từ $O(c \cdot q \log n)$ xuống $O(q \log n)$ — tức **chia cho $c$**, mà $c$
> thường là vài trăm.

**Ba trường điểm gộp còn một** (`ResultRanker.java:56-64`): record `RankedResult`
từng có `finalScore`, `relevanceScore` và hàm `tfidfScore()` — **cả ba trả về
cùng một số**. Javadoc nói thẳng hậu quả:

> Ba tên cho một đại lượng không phải là sự tiện lợi: nó khiến tầng gọi tưởng
> mình đang đọc ba thành phần độc lập, và `EvaluationRunner` **đã thật sự nhầm**
> — nó đo "thang đo của TF-IDF thô" bằng chính điểm tổng.
...
> `ResultRanker` từ **ba việc** (chấm điểm + kết hợp tín hiệu + sinh snippet) xuống **một việc**: chấm điểm và lấy top-K. Sinh snippet chuyển sang `SnippetBuilder`, kết hợp tín hiệu chuyển sang Decorator. Xem [**03-DECORATOR.md**](../08-design-patterns/03-DECORATOR.md).
...
>
> Phân tích đầy đủ: [**03-DECORATOR.md**](../08-design-patterns/03-DECORATOR.md).
...
Chi tiết thuật toán và chứng minh ở [MinHeap §5](../05-datastructures/MinHeap.md).
...
- Cấu trúc top-K: [MinHeap.md](../05-datastructures/MinHeap.md)
- Nguồn ứng viên: [CandidateResolver.md](../03-query/CandidateResolver.md)
- Phép bỏ dấu: [VietnameseTokenizer §4](../02-index/VietnameseTokenizer.md)
