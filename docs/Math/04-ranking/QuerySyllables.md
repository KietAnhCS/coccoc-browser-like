...
Đây là lý do `InvertedIndex` giữ **chỉ mục kép** (có dấu + không dấu) trong cùng một `HashMap` — xem [InvertedIndex §6](../02-index/InvertedIndex.md).
...
> Cùng kỹ thuật được dùng ở [`LanguageDetector.looksVietnamese`](../05-datastructures/Trie.md): *"văn bản có ít nhất một ký tự mang dấu ⟺ $\sigma(s) \ne s$"*.
...
- Người dùng thứ hai — chấm điểm: [03-DECORATOR.md §5](../08-design-patterns/03-DECORATOR.md)
- Phép bỏ dấu và bẫy chữ `đ`: [VietnameseTokenizer](../02-index/VietnameseTokenizer.md)
- Chỉ mục kép có dấu / không dấu: [InvertedIndex](../02-index/InvertedIndex.md)
