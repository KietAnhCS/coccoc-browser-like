...
**Nhưng điều kiện `<= 0` (không phải `== 0`) che một vấn đề lớn hơn.** Nếu df **vượt quá** $N$ — điều **có thể xảy ra** với chỉ mục kép có dấu/không dấu (xem [InvertedIndex §6](../02-index/InvertedIndex.md)) — thì:
...
Chi tiết `>>>` chống tràn: xem [InvertedIndex §5.1](../02-index/InvertedIndex.md).

**Trùng lặp:** hàm này gần như y hệt ở `BM25Scorer` và `InvertedIndex.getPositions` — ba bản sao, xem [InvertedIndex §5.2](../02-index/InvertedIndex.md).
...
- Bất biến được tận dụng: [InvertedIndex §4](../02-index/InvertedIndex.md)
