...
**Phạm vi:** **14 file** trong `com.vnsearch.index`, cộng `service/IndexBuilder` là cửa vào duy nhất và `datastructure/SyllableTrie` là cấu trúc nền của bộ tách từ.

> **Tự kiểm chứng:**
> ```bash
> find search-engine/src/main/java/com/vnsearch/index -name "*.java" | wc -l
> ```

> 📊 **Số đo trong trang này thuộc mốc A** — corpus **5.011 trang**, 136.768 term.
> Repo có **bốn mốc corpus**; mốc hiện hành là **D — 31.030 trang, 138.507 term**.
> Bảng quy chiếu ở đầu [`DSA-REPORT.md`](../../DSA-REPORT.md).
...
## 1. Bản đồ toàn cảnh — 14 file chia thành 4 nhóm
...
### Bảng tra nhanh — cả 14 file, mỗi file một câu
...
| **12** | **`MaxWeightSegmenter`** | **2** | **Quy hoạch động cực đại trọng số** — thay Longest Matching tham lam; đây mới là nơi thuật toán tách từ thật sự nằm |
| **13** | **`VietnameseWordDictionary`** | **2** | **Từ điển từ ghép CÓ TRỌNG SỐ**, nạp vào `SyllableTrie`. Công thức trọng số lấy đúng bản production của Coc Coc |
| **14** | **`CompressedText`** | **3** | Nén thân bài bằng `Deflater` thô. Chỉ dùng cho **snippet của top-K**, nhưng phải có sẵn từ trước khi biết top-K là ai |

> ⚠️ **Ba dòng 12–14 bị thiếu trong bản trước của bảng này**, và bản trước còn
> ghi *"11 file"* trong khi package có **14**. Hai file bị bỏ sót là hai file
> quan trọng nhất của nhóm 2:
>
> - `MaxWeightSegmenter` **là nơi thuật toán tách từ nằm** — `VietnameseTokenizer` chỉ điều phối.
> - `VietnameseWordDictionary` là thứ biến bài toán từ *"đoán bằng heuristic"* thành *"tối ưu có hàm mục tiêu"*.
>
> Nói cách khác, bảng cũ mô tả tầng chỉ mục mà **bỏ mất phần lõi của bộ tách từ**.

**Vì sao `SyllableTrie` nằm ở `datastructure/` chứ không ở `index/`.** Nó là một
cấu trúc dữ liệu thuần — trie trên đơn vị **âm tiết**, lưu bằng **mảng phẳng** —
nên thuộc về nhóm cấu trúc dữ liệu dùng chung, xem
[`05-datastructures/`](../05-datastructures/00-SO-DO-TU-DUY.md). Đây là cùng
nguyên tắc đã đặt `BloomFilter` ở `datastructure/` chứ không ở `crawler/`.
...
Mảng `offsets` có `count + 1` phần tử chứ không phải `count`: **phần tử canh biên (sentinel)** làm công thức hiệu số đúng cho **mọi** `i`, kể cả phần tử cuối, nên khỏi cần một nhánh `if` riêng — **cùng lý do** với sentinel node của [`LRUCache`](../05-datastructures/LRUCache.md).

> 🔗 **Đây chính là** kỹ thuật `rowPtr` mà [`SparseMatrix.freeze()`](../05-datastructures/SparseMatrix.md) dùng để nén ma trận liên kết cho PageRank. Cùng một ý tưởng xuất hiện **hai lần ở hai chỗ hoàn toàn không liên quan** trong đồ án — đó là dấu hiệu nó là một **kỹ thuật nền tảng**, không phải thủ thuật riêng lẻ.
...
| **Chỉ mục được dùng thế nào** — two-pointer, filter-and-refine | [03-query/](../03-query/) |
| Điểm BM25 dùng `avgdl` và `tf` ra sao | [04-ranking/BM25Scorer.md](../04-ranking/BM25Scorer.md) |
