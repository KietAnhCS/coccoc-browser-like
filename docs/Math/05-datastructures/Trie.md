...
**Điều kiện `if (!withoutDiacritics.equals(phrase))`** tránh chèn hai lần cùng một khoá khi cụm vốn không có dấu. Cùng kỹ thuật với `InvertedIndex` (xem [InvertedIndex §6](../02-index/InvertedIndex.md)) — và cùng dựa trên tính chất **điểm bất động**:
...
Chi tiết về NFC/NFD ở [VietnameseTokenizer §3](../02-index/VietnameseTokenizer.md).

> **Một điểm chưa nhất quán:** `Trie.normalize` chỉ NFC, **không** `toLowerCase`. Còn `VietnameseTokenizer.normalizeForLookup` làm cả hai. Việc hạ chữ thường được đẩy sang người gọi (`SearchEngineFacade.suggest` gọi `prefix.trim().toLowerCase()`). Hoạt động được, nhưng lại là một bất biến phụ thuộc người gọi nhớ — cùng loại vấn đề với [InvertedIndex §4.2](../02-index/InvertedIndex.md).
...
Lại một lần nữa dùng **điểm bất động** của `stripDiacritics` — cùng kỹ thuật với §4 và với [ResultRanker §3](../04-ranking/ResultRanker.md).
...
- Nguồn dữ liệu và phép bỏ dấu: [VietnameseTokenizer.md](../02-index/VietnameseTokenizer.md)
- Bản cài đặt song song bằng TypeScript: [BookmarkTrie.md](../07-frontend/BookmarkTrie.md)
