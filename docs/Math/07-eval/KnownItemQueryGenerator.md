...
**Dùng lại đúng `TfIdfScorer.tf` và `TfIdfScorer.idf`** của hệ thống thật — không viết lại công thức. Đây là cùng nguyên tắc "một cài đặt duy nhất" mà [CandidateResolver](../04-query/CandidateResolver.md) thể hiện.
...
Nếu lấy cả `noDiacriticTerm`, truy vấn sẽ thành `máy tính may tinh` — lặp cùng một khái niệm và làm sai trọng số. Đây là hệ quả trực tiếp của thiết kế chỉ mục kép ở [InvertedIndex §6](../03-index/InvertedIndex.md).
...
Và nó **an toàn** vì `QueryParser` dùng đúng tokenizer đã dùng lúc index (xem [QueryParser §1](../04-query/QueryParser.md)) nên `máy tính` sẽ được ghép lại thành `máy_tính` — khớp chính xác khoá trong chỉ mục.
...
>
> Bộ lọc `looksVietnamese` mà [Trie](../06-datastructures/Trie.md) dùng cho gợi ý **chưa** được áp dụng ở đây — một cải tiến rẻ và rõ ràng.
...
- Công thức được dùng lại: [TfIdfScorer.md](../05-ranking/TfIdfScorer.md)
- Bất biến được kiểm chứng gián tiếp: [QueryParser §1](../04-query/QueryParser.md)
